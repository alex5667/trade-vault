...
def build_runtime_from_inputs(inp: Dict[str, Any]) -> RuntimeStub:
    ...
    # OFI (optional advanced)
    if _bool(inp.get("ofi_stable", 0)) or _bool(inp.get("ofi_dir_ok", 0)):
        rt.last_ofi_event = {
            "ts_ms": ts_ms,
            "dir": direction,
    ...
    # FP edge absorb (optional advanced)
    if _bool(inp.get("fp_edge_absorb", 0)):
        rt.last_fp_edge = {
            "ts_ms": ts_ms,
            "dir": direction,
            "strength": _f(inp.get("fp_edge_strength", 1.5), 1.5),
