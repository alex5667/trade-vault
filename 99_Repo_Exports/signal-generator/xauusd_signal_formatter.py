...
class XAUUSDSignalFormatter:
    ...
    def format_telegram_message(cls, signal: XAUUSDSignal, include_indicators: bool = True) -> str:
        ...
        if signal.reason:
            ...
            reason_short = signal.reason.replace("; ", " | ")
        ...
        # Источник и ID
        message_parts.append(f"🔧 Source: {signal.source} | ID: {signal.sid}")
    ...
    def format_redis_payload(cls, signal: XAUUSDSignal) -> Dict[str, Any]:
        ...
        return {
            ...
            "side": signal.side,
            "direction": signal.side,  # ← FIX: notify-worker ожидает 'direction'
            ...
            "ts": str(signal.ts),
            "trail_after_tp1": "true" if signal.trail_after_tp1 else "false",
