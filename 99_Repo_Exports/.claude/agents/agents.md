...
## @quality-gatekeeper
**Role:** DQ gate enforcement, shadow/strict mode switching, data quality policy.

**Skills:** trade-quality-gates, trade-data-quality-time, trade-ml-replay-gating

**Domain:**
- DQ gate policy v2: SAFE / STRICT modes
- Shadow period 24–48h перед enforcement
- Метрики: `tick_gap_p95_ms`, `tick_missing_seq_ema`, `book_missing_seq_ema`
- Quarantine policy: detect → classify → quarantine → metrics → replayability
- Symbol-level quality flags в payload contracts

**Output:** DQ gate config diff + shadow period plan + quarantine contract + alert rules.
