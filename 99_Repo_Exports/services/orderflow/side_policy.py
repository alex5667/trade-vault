...
- ignore_delta (default): keep tick; signed qty downstream becomes 0.0 (fail-open)
...
def is_unknown_side_tick(tick: Dict[str, Any]) -> bool:
    """Returns True when tick has no reliable aggressor side."""
    try:
        side = str(tick.get("side") or "").strip().upper()
        conf = str(tick.get("side_conf") or "").strip().lower()
        if conf == "unknown":
            return True
        if side in ("BUY", "SELL"):
            return False
        if tick.get("is_buyer_maker", None) is None:
            return True
        return False
    except Exception:
        return False
