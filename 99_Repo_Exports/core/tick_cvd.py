...
class TickCVDState:
    ...
    def __init__(
        ...
        # GPU-accelerated ring buffer for median/MAD (lazy init)
        self._gpu_rb: Optional[Any] = None
        self._gpu_rb_init: bool = False
    ...
    def _maybe_recalc_median(self) -> None:
        ...
        if len(self._abs_delta_usd_buf) < 16:
            return
        # GPU path: use GPURingBuffer if available
        if self._gpu_rb is not None:
            try:
                # Use a lightweight median on USD buf — GPURingBuffer is primarily
                # for the delta ring buffer; for the USD buf we can still benefit
                # from numpy vectorized median when buffer is large enough
                import numpy as _np
                arr = _np.array(self._abs_delta_usd_buf, dtype=_np.float64)
                self._median_abs_delta_usd = float(_np.median(arr))
                return
            except Exception:
                pass
    ...
    def update(self, tick: Dict[str, Any]) -> None:
        ...
        if self._q_enable and self.last_ts_ms is not None and ts_ms_int > 0 and int(self.last_ts_ms or 0) > 0 and ts_ms_int < (int(self.last_ts_ms) - self._ooo_max_lag_ms):
            nm = int(ts_ms_int or int(time.time() * 1000))
        ...
        # Keep GPU ring buffer in sync (if already initialised)
        if self._gpu_rb is not None:
            try:
                self._gpu_rb.push(float(d))
            except Exception:
                pass
    ...
    def quarantine_active(self, now_ms: Optional[int] = None) -> bool:
        if not self._q_enable:
            return False
        nm = int(now_ms or int(time.time() * 1000))
        return int(self._q_until_ms or 0) > nm

    def quarantine_reason(self) -> str:
        return str(self._q_reason or "")

    def _try_init_gpu_rb(self) -> None:
        """Lazily initialise pinned-memory GPU ring buffer for robust stats."""
        if self._gpu_rb_init:
            return
        self._gpu_rb_init = True
        try:
            from gpu.gpu_ring_buffer import GPURingBuffer
            self._gpu_rb = GPURingBuffer(
                window_size=self.robust_window,
                min_n=20,
                eps=self.robust_eps,
                use_gpu=None,  # auto-detect
            )
            # Warm up with existing ring buffer data
            for v in self._rb:
                self._gpu_rb.push(v)
        except Exception:
            self._gpu_rb = None
    ...
    def robust_snapshot(self) -> Dict[str, float]:
        ...
        GPU-accelerated: uses GPURingBuffer with pinned memory when available.
        Fallback: CPU O(W log W) sorted median.
        """
        xs = list(self._rb)
        n = len(xs)
        if n < 20:
        ...
        # GPU path: GPURingBuffer with pinned DMA (~8µs vs ~43µs CPU)
        if self.robust_window >= 200:
            if not self._gpu_rb_init:
                self._try_init_gpu_rb()
            if self._gpu_rb is not None:
                try:
                    med, mad, buf_n = self._gpu_rb.compute_stats()
                    if buf_n >= 20:
    ...
    def indicators_light(self) -> Dict[str, Any]:
        ...
        """
        nm = int(self.last_ts_ms or int(time.time() * 1000))
