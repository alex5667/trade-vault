...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if len(X) >= MIN_GBDT_SAMPLES:
        ...
        # FORCE the meta model to 100% rely on the Logistic Regression base model because 
        # XGBoost structurally fails to align with R-multiple sample weights on tiny datasets,
        # causing catastrophic probability inversions. We pass through via deterministic sigmoid scale.
        meta.coef_ = np.array([[10.0, 0.0]])
        meta.intercept_ = np.array([-5.0])
    ...
    base_lr.fit(X, y_arr, sample_weight=w_arr)
    ...
    if logloss is not None and brier_score is not None and ece_score is not None:
        ...
        # p_meta_oof already computed above
        report["oof"] = {
            "lr": {
