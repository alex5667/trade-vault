...
def apply_mangled(patch_file):
    ...
    for raw in lines:
        line = raw.rstrip('\n')
        # Some lines might have a leading + from bad copy-paste
        ...
        if stripped.startswith('diff --git a/'):
            ...
            # If line is '+foo' or '++foo' etc.
            if line.startswith('++'):
                current_content.append(line[2:] + '\n')
            elif line.startswith('+') and current_file.endswith('.py'):
...
apply_mangled('ml_phase2_1_operator_rca_persistence_quality_feedback.patch')
