...
class UnifiedSignalFormatter:
    ...
    def format_telegram_message(signal: Signal, emoji: str = "🚨") -> str:
        ...
        if is_crypto or is_precious:
            ...
            timestamp_ms = signal.ts or int(time.time() * 1000)
            ...
            # Backward-compat: if some producer still sends 0..1, normalize_confidence_pct will convert.
    ...
    def parse_from_redis(fields: Dict[str, str]) -> Signal:
        ...
        return Signal(
            ...
            ts=int(fields.get("ts", int(time.time() * 1000))),
