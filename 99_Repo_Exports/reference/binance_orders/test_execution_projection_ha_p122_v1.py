...
_BASE_MS = int(time.time() * 1000)  # current ms so cursor_age_ms is near 0
...
class FakeRedis:
    ...
    def set(self, key: str, val, ex=None, px=None, nx: bool = False):
        ...
        self.kv[key] = val
        if ex is not None:
            self.kv_px[key] = int(time.time() * 1000) + int(ex) * 1000
        elif px is not None:
            self.kv_px[key] = int(time.time() * 1000) + int(px)
        else:
            self.kv_px.pop(key, None)
        return True
    ...
    def pexpire(self, key: str, ms: int) -> None:
        ...
        if key in self.kv:
            self.kv_px[key] = int(time.time() * 1000) + int(ms)
