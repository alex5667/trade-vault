...
class TestPassThroughMode:
    ...
    @pytest.mark.asyncio
    async def test_virtual_proposal_published_in_passthrough(self):
        svc = _build_service(require_of_confirm=False)
...
class TestSafeguard:
    ...
    @pytest.mark.asyncio
    async def test_ok1_virtual_order_passes(self):
        """Virtual orders with ok=1 should pass normally."""
        svc = _build_service(require_of_confirm=True, match_ms=5000)
        now_ms = int(time.time() * 1000)
