...
def _parse_trade(fields: Dict[str, str]) -> ClosedTradeSnapshot:
    ...
    # пытаемся взять exit_ts_ms явно; если нет — оставляем 0
    ...
    # Попытка восстановить one_r_money, если оно 0 (частая проблема в данных)
    if one_r <= 1e-9 and lot > 0 and entry_px > 0:
        # 1. Проверяем signal_payload
        payload_raw = fields.get("signal_payload")
        if payload_raw:
            try:
                p = json.loads(payload_raw)
                p_sl = float(p.get("sl") or 0.0)
                p_entry = float(p.get("entry") or p.get("entry_price") or entry_px)
                if p_sl > 0:
                    one_r = abs(p_entry - p_sl) * lot
            except Exception:
                pass
        
        # 2. Если все еще 0, пробуем через корневые поля sl/entry
        if one_r <= 1e-9 and sl_px > 0:
             one_r = abs(entry_px - sl_px) * lot
        
        # 3. Крайний случай: если нет SL, но есть ATR в фичах/пейлоаде
        # (в трейлинге 1R обычно = 1 ATR * mult)
        if one_r <= 1e-9:
            atr = 0.0
            payload_raw = fields.get("signal_payload")
            if payload_raw:
            ...
            if atr > 0:
                # Берем стандартный риск 1.0 ATR если не знаем mult
                one_r = atr * 1.0 * lot
...
def main(argv: list[str] | None = None) -> int:
    ...
    for symbol in symbols:
        ...
        if not rec_all and not rec_trailing:
            # Diagnostics: count wins with the same filters as recommend_trailing_size
            sym_trades = [t for t in trades if t.symbol == symbol and t.source == args.source]
            ...
            if len(sym_trades) == 0:
                ...
                # Проверяем, есть ли вообще сделки в stream для этого символа
                all_symbols_in_stream = {}
                for t in trades:
                    sym = canon_symbol(t.symbol)
                    all_symbols_in_stream[sym] = all_symbols_in_stream.get(sym, 0) + 1
                ...
                if all_sources_for_symbol and args.source not in all_sources_for_symbol:
...
def build_trailing_report_markdown_from_env(r: redis.Redis | None = None) -> str:
    ...
    for symbol in symbols:
        ...
        if not rec_all and not rec_trailing:
            # Diagnostics: count wins with the same filters as recommend_trailing_size
            sym_trades = [t for t in trades if t.symbol == symbol and t.source == source]
            ...
            if len(sym_trades) == 0:
                ...
                if all_sources_for_symbol and source not in all_sources_for_symbol:
                    msg += f"\n  💡 Возможно несоответствие source. Найдены: {dict(all_sources_for_symbol)}"
