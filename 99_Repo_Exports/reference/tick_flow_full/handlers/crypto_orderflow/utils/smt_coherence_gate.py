...
class SmtLeaderCoherenceGate:
    ...
    def _maybe_diag(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            pass
        try:
            self.redis.xadd(self.diag_stream, {"data": json.dumps(payload, ensure_ascii=False)}, maxlen=50000)
        except Exception:
            return
    ...
    def evaluate(self, *, ctx: Any, symbol: str, kind: str, direction: str) -> GateDecision:
        ...
        if not self.bundle_id:
            return GateDecision(apply=False, veto=False, reason_code="SMT_DISABLED", notes="SMT_COH_BUNDLE empty")
        ...
        if not isinstance(st, dict) or not st:
            ...
            return GateDecision(apply=False, veto=False, reason_code="SMT_NO_STATE", notes=key)
        ...
        if self.mode == "veto" and news_blocked == 1:
            try:
                ...
                pass
            return GateDecision(apply=True, veto=True, reason_code="VETO_SMT_NEWS_GATE", notes=f"until={news_until_ts_ms}")
        ...
        if decision == "reversal" and pick and symbol.upper() == pick:
            ...
            try:
                ...
                pass
            return GateDecision(apply=True, veto=False, reason_code="SMT_GOLDEN_REVERSAL", notes=f"picked {pick}")
        ...
        # observe: never veto
        if self.mode != "veto":
            return GateDecision(apply=True, veto=False, reason_code="SMT_OBSERVE", notes=block_reason)

        # veto mode: veto ONLY narrow rule + optional kind allowlist
        if blocked == 1:
            if self.veto_kinds is not None and kind_l and (kind_l not in self.veto_kinds):
                return GateDecision(apply=True, veto=False, reason_code="SMT_BLOCKED_BUT_KIND_NOT_APPLICABLE", notes=block_reason)
            return GateDecision(apply=True, veto=True, reason_code="VETO_SMT_COUNTERTREND", notes=block_reason)

        return GateDecision(apply=True, veto=False, reason_code="SMT_OK", notes="")
