...
class SignalGenerator:
    ...
    def __init__(self, symbol: str, config: Any, outbox: Any, cooldown: Any = None,
        ...
        # Если хотите "жёстко требовать режим", можно включать:
        self.regime_require = bool(getattr(config, "regime_require_score", False))
    ...
    def _compute_confidence(self, ctx: OrderflowSignalContext) -> Tuple[float, Dict[str, float]]:
        ...
        c_z = min(abs(z) / 2.0, 1.0)
        c_burst = 1.0 if self._burst_gate_ok(ctx) else 0.3
        w_obi, w_z, w_burst = 0.4, 0.4, 0.2
    ...
    def generate(self, ctx: OrderflowSignalContext, signal_type: str = "bar") -> PublishResult:
        ...
        ts_ms = int(getattr(ctx, "ts", 0) or int(time.time() * 1000))
