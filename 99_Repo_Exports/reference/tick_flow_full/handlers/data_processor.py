...
from typing import Optional, Dict, Any, Tuple, List, Deque, Union, Type
...
try:
    from handlers.cvd_tracker import CVDTracker
except ImportError:
...
class OrderFlowDataProcessor:
    ...
    def _process_tick(self, tick: Tick) -> Tuple[Optional[object], Optional[int]]:
        ...
        # Update CVD tracker
        taker_buy_vol = tick.volume if delta > 0 else 0.0
        taker_sell_vol = tick.volume if delta < 0 else 0.0
        cvd_state = self.cvd_tracker.update(tick.last, taker_buy_vol, taker_sell_vol)
        st.cvd = cvd_state["cvd"]
        ...
        if ts > 0 and price > 0.0:
            ...
            try:
                ...
                now_ms = int(ts_ms if ts_ms > 0 else int(time.time() * 1000))
    ...
    def _process_book(self, book: Dict[str, Any]) -> None:
        """
        book может быть:
          1) уже распарсенным dict: {"snapshot": SimpleL2Snapshot, "ts_ms": int, ...}
          2) сырыми fields из Redis stream (fallback)
        """
        parsed = None
        if isinstance(book, dict) and ("snapshot" in book) and ("ts_ms" in book):
            parsed = book
        else:
            parsed = self.parser._parse_book(book)
        if not parsed:
            return

        snap = parsed["snapshot"]
        ts_ms = int(parsed["ts_ms"] or getattr(snap, "ts_ms", 0) or 0)
        if ts_ms <= 0:
