            ...
            if val is not None:
                try:
                    ...
                    if ts_ms > 1_000_000_000_000:  # milliseconds
                        return (time.time() * 1000 - ts_ms) / 1000.0
...
def run_cleanup(r: redis.Redis) -> int:
    ...
    try:
        ...
        logger.info("🔍 Scanning %d open positions (max_age=%ds, dry_run=%s)", total, MAX_AGE_SEC, DRY_RUN)
        ...
        for pos_id in members:
            ...
            if not exists:
                ...
                if not DRY_RUN:
                    ...
                    removed += 1
                    logger.info("🧹 Removed orphan ref %s (hash missing)", pos_id)
                else:
                    logger.info("🧹 [DRY-RUN] Would remove orphan ref %s (hash missing)", pos_id)
                    removed += 1
                missing_hash += 1
                continue
