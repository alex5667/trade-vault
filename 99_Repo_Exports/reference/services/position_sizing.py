...
def apply_position_sizing_to_ctx(
    ...
    try:
        ...
        # ── 1. Resolve risk_usd ────────────────────────────────────────────
        # Priority: RISK_USD_PER_TRADE > RISK_PERCENT * wallet_balance
        risk_usd = _env_float("RISK_USD_PER_TRADE", 0.0)

        if risk_usd <= 0:
            risk_pct = _env_float("RISK_PERCENT", 0.0)
            if risk_pct > 0:
                # Dynamic balance: Redis snapshot → Binance REST → static env
                deposit_v: float = 0.0
                if BalanceProvider is not None:
                    try:
                        bp = BalanceProvider.from_ctx(ctx)
                        deposit_v = bp.get_wallet_balance()
                    except Exception as _bp_err:
                        if logger:
                            logger.warning("[sizing] BalanceProvider failed: %s", _bp_err)
                # last resort: static env / ctx
                if deposit_v <= 0:
