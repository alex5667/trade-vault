...
class AsyncSignalPublisher:
    ...
    async def xadd_json_internal(
        ...
        *,
        ...
        # 3) xadd — with latency tracking (H4 budget: p99 < 8ms)
        ...
        # Latency audit: record signal emit latency (fail-open)
        try:
            from services.orderflow.metrics import signal_emit_latency_us
            _dt_us = (_time.monotonic_ns() - _t0) / 1_000
            signal_emit_latency_us.labels(symbol=str(symbol), stream=str(sink.name)).observe(_dt_us)
        except Exception:
            pass
