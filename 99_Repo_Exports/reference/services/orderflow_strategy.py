...
class OrderFlowStrategy:
    ...
    async def process_tick(self, runtime: SymbolRuntime, tick: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # ENFORCE: bad data health → hard veto (not virtual)
            try:
                shadow_thr = float(cfg.get("data_health_shadow_only_below", 0.40))
            except Exception:
                shadow_thr = 0.40
            indicators["data_health_shadow_only"] = 1 if float(dh.score) < shadow_thr else 0
            if dh.reasons:
                indicators["data_health_shadow_reason"] = ",".join(list(dh.reasons)[:5])
        except Exception:
            pass

        # data_health ENFORCE gate: drop signal if data quality is below threshold
        if int(indicators.get("data_health_shadow_only", 0)) == 1:
            logger.warning(
                "🛑 [DATA-HEALTH] (%s) VETO: score=%.2f reasons=%s — signal dropped (enforce)",
                runtime.symbol,
                float(indicators.get("data_health", 0.0)),
                indicators.get("data_health_shadow_reason", ""),
            )
            return None
            ...
            if bool(int(os.getenv("ATR_TF_CALIB_ENABLE", "1"))):
                ...
                if now_ts > 0 and (now_ts - last) >= refresh_ms and close_px > 0:
                    ...
                    if atr_bps_by_tf:
                        ...
                        mode = str(os.getenv("ATR_TF_SELECTOR_MODE", "enforce")).lower()  # "shadow"|"enforce"
