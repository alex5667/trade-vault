...
def _apply_blocker_mode(
    ...
    try:
        ...
        blocker["rg_calib_updated_ms"] = int(time.time() * 1000)
...
def run_research_guard_calibration(
    ...
    *,
    ...
    logger.info(
        "RG Calibrator: mode=%s→%s, recommend=%s, PSR=%.3f, DSR=%.3f, PBO=%.3f, "
        "streak=%d/%d, age=%.0fs",
...
def run_once() -> None:
    ...
    if result:
        logger.info(
            "RG Calibrator result: mode=%s, recommend=%s, PSR=%.3f, DSR=%.3f, PBO=%.3f",
