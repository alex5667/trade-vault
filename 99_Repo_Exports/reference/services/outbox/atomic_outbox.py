...
def _event_stream_key() -> str:
    ...
    if str(os.getenv("OUTBOX_EVENT_STREAM_ENABLE", "1")).strip().lower() not in {"1", "true", "yes", "on"}:
        return "__none__"
    raw = str(os.getenv("OUTBOX_EVENT_STREAM_KEY", RS.ORDERS_EXEC)).strip()
...
def _prepare_contract_payload(
    ...
    payload.setdefault(
        "contract_ver",
        str(payload.get("contract_ver") or meta.get("contract_ver") or os.getenv("OUTBOX_CONTRACT_VER", "execution_contract:v1")),
    )
    payload.setdefault(
        "execution_plan_id",
