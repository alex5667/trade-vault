...
class SymbolRuntime:
    ...
    async def ensure_specs_fresh(self, r) -> None:
        ...
        try:
            ...
            # Load per-bucket slippage decomp coeff from Redis (nightly calibrated).
            # Keys: cfg:slippage_decomp_impact_coeff_bps:{sym}:{bucket}
            try:
                _slip_buckets = ("NORMAL", "LOW_LIQ", "HIGH_VOL", "HIGH_VOL_LOW_LIQ")
                _slip_keys = [
                    f"cfg:slippage_decomp_impact_coeff_bps:{self.symbol}:{b}"
                    for b in _slip_buckets
                ]
                try:
                    _slip_vals = await r.mget(*_slip_keys)
                except Exception:
                    _slip_vals = [await r.get(k) for k in _slip_keys]
                _coeff_map = {}
                for _b, _v in zip(_slip_buckets, _slip_vals or []):
                    if _v in (None, "", "na"):
                        continue
                    try:
            ...
            # Optional: global (or per-symbol) enforce bucket allowlists (loaded from Redis).
            # Keys supported:
            #   cfg:slippage_decomp_enforce_buckets
            #   cfg:slippage_decomp_enforce_buckets:{symbol}
            #   cfg:taker_flow_gate_enforce_buckets
            #   cfg:taker_flow_gate_enforce_buckets:{symbol}
            try:
                sym = str(self.symbol)
                keys = [
                ...
                if sd:
                    try:
                        ...
                        pass
                    try:
                        # inject into runtime.config so dict(runtime.config) carries it
