...
def _read_purpose_status(r: Any, cfg: Cfg, purpose: str, now_ms: int) -> dict[str, Any]:
    ...
    # P4.12 + P4.13: compute current binding and check for semantic mismatch
    current_binding = build_drift_binding(r, state_prefix=cfg.state_prefix, purpose=purpose, now_ms=now_ms)
...
def cmd_prepare_override(
    ...
    *,
    ...
    raw = prepare_override_approval(
        ...
        prepared_freshness_s=cfg.approval_prepared_freshness_s,
        ops_stream=cfg.ops_stream,
        # P4.12+P4.13: bind to current drift snapshot at prepare time
        drift_binding=build_drift_binding(r, state_prefix=cfg.state_prefix, purpose=purpose, now_ms=now_ms),
        now_ms=now_ms,
    )
...
def cmd_ack(
    ...
    *,
    ...
    approval_state = None
    # P4.12+P4.13: compute current binding snapshot before validation
    current_binding = build_drift_binding(r, state_prefix=cfg.state_prefix, purpose=purpose, now_ms=now_ms)
