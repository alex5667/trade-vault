...
# Signal Ensemble (weighted voting layer)
try:
    ...
    HAS_ENSEMBLE = False
    SignalEnsemble = None
    _import_warnings.append(f"Ensemble import failed: {e}")
...
class HubConfig:
    ...
    # Ensemble config
    ensemble_mode: str = os.getenv("ENSEMBLE_MODE", "shadow")  # shadow | enforce | disabled
    ensemble_threshold: float = float(os.getenv("ENSEMBLE_THRESHOLD", "0.35"))
    ensemble_consensus_ratio: float = float(os.getenv("ENSEMBLE_CONSENSUS_RATIO", "1.5"))
...
class AggregatedSignalHubV2:
    ...
    def __init__(self, cfg: Optional[HubConfig] = None):
        ...
        try:
            writer_cfg.notify_signal_every_n = max(
                1,
                int(os.getenv("CRYPTO_NOTIFY_SIGNAL_EVERY_N", "1"))
            )
        except ValueError:
        ...
        # Signal Ensemble
        self.ensemble = None
        if HAS_ENSEMBLE and SignalEnsemble and self.cfg.ensemble_mode != "disabled":
            try:
                self.ensemble = SignalEnsemble(
                    redis_client=self.r,
                    symbol=self.cfg.symbol,
                    mode=self.cfg.ensemble_mode,
                    threshold=self.cfg.ensemble_threshold,
                    consensus_ratio=self.cfg.ensemble_consensus_ratio,
                    logger=log,
                )
                log.info(
                    "✅ Ensemble enabled: mode=%s threshold=%.2f consensus_ratio=%.2f",
                    self.cfg.ensemble_mode,
                    self.cfg.ensemble_threshold,
                    self.cfg.ensemble_consensus_ratio,
                )
            except Exception as e:
                log.warning("⚠️  Ensemble init failed: %s", e)
        else:
            log.info("ℹ️  Ensemble disabled (mode=%s, available=%s)", self.cfg.ensemble_mode, HAS_ENSEMBLE)
    ...
    def step(self, snap: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # 4b. Ensemble voting gate
        ensemble_decision = None
        if self.ensemble and HAS_ENSEMBLE:
            try:
                ensemble_signals = {
                    "orderflow": build_orderflow_vote(conf, side_hint, conf_parts),
                    "ta_indicators": build_ta_vote(self.r, self.cfg.symbol),
                    "microstructure": build_microstructure_vote(metrics_pro, metrics_legacy, cl_score),
                    "regime_filter": build_regime_vote(self.r, self.cfg.symbol, side_hint),
                }
                ensemble_decision = self.ensemble.vote(ensemble_signals)

                # In enforce mode: ensemble can block the signal
                if self.cfg.ensemble_mode == "enforce" and ensemble_decision.action == "skip":
                    self.filtered_signal_count += 1
                    self.message_count += 1
                    if self.message_count % 1000 == 0:
                        log.info(
                            "Signal filtered by ensemble: %s (conf=%.2f) [msg #%d]",
                            ensemble_decision.reason,
                            conf,
                            self.message_count,
                        )
                    return None

                # In enforce mode: use ensemble direction if it overrides
                if self.cfg.ensemble_mode == "enforce" and ensemble_decision.action in ("long", "short"):
                    side_hint = ensemble_decision.action.upper()

            except Exception as e:
                # Fail-open: ensemble error does NOT block signal
                log.warning("⚠️  Ensemble vote error (fail-open): %s", e)
        ...
        # Append ensemble info to reason
        if ensemble_decision and ensemble_decision.action != "skip":
            reason += f"; ensemble:{ensemble_decision.action}(s={ensemble_decision.score:.2f})"
        ...
        # 6b. Evaluate expiration TTL based on volatility and ATR
        regime = "ranging"
        try:
            regime_key = f"ta:last:regime:{self.cfg.symbol}"
            val = self.r.get(regime_key)
            if val:
                import json
                r_data = json.loads(val)
                regime = r_data.get("regime", "ranging")
        except Exception:
            pass
        
        signal_ttl = self.compute_signal_ttl(atr, regime)
        ...
        result = self.writer.write_and_push(
            ...
            **({
                "ensemble_sources": [
                    s
                    for s, v in (ensemble_decision.votes if ensemble_decision else {}).items()
                    if isinstance(v, dict) and v.get("direction") in ("long", "short")
                ],
                "ensemble_score": ensemble_decision.score if ensemble_decision else 0.0,
            } if ensemble_decision else {}),
            ttl=signal_ttl
        )
        ...
        if result:
            ...
            # 7.5 Write features to TimescaleDB
            # ✅ Публикуем virtual signals for Ensemble Calibrator (TA Indicators, Microstructure, Regime Filter)
            try:
                import json as json_lib
                signal_stream = f"signals:aggregated:{self.cfg.symbol}"
                
                if ensemble_decision and ensemble_decision.votes:
                    for comp_source, vote in ensemble_decision.votes.items():
                        if isinstance(vote, dict):
                            v_dir = vote.get("direction")
                            if v_dir in ("long", "short"):
                                v_side = "LONG" if v_dir == "long" else "SHORT"
                                
                                # Determine SL/TP for the component's chosen direction
                                if v_side == side:
                                    c_sl = result.sl
                                    c_tps = result.tp_levels
                                else:
                                    stop_dist = abs(result.price - result.sl)
                                    if v_side == "LONG":
                                        c_sl = result.price - stop_dist
                                        c_tps = [result.price + abs(tp - result.price) for tp in result.tp_levels]
                                    else:
                                        c_sl = result.price + stop_dist
                                        c_tps = [result.price - abs(result.price - tp) for tp in result.tp_levels]
                                        
                                v_sid = f"{comp_source}_{result.sid}"
                                v_payload = {
                                    "sid": v_sid,
                                    "symbol": self.cfg.symbol,
                                    "side": v_side,
                                    ...
                                    "source": comp_source,
                                    "atr": atr,
                                    "confidence": vote.get("confidence", 1.0) * 100 if vote.get("confidence", 1.0) <= 1.0 else vote.get("confidence", 1.0),
                                    "ts": get_ny_time_millis(),
                                    "trail_after_tp1": trail_after_tp1,
                                    "trail_profile": trail_profile,
                                    "expires_at": ts_ms + (signal_ttl * 1000),
                                    "is_virtual": True
                                }
                                self.r.xadd(signal_stream, {"data": json_lib.dumps(v_payload)}, maxlen=1000)

                log.debug("📨 Signal and virtual ensemble signals published to %s for tracking", signal_stream)
            except Exception as e:
                log.warning("Failed to publish to signal stream for tracking: %s", e)

            try:
                from services.feature_snapshot_writer import get_feature_snapshot_writer
                fs_writer = get_feature_snapshot_writer()
                if fs_writer:
                    feature_set = {
                        "conf": conf,
                        "conf_parts": conf_parts,
                        "metrics_pro": metrics_pro,
                        ...
                        } if ensemble_decision else None,
                        "atr": atr,
                        "mid": mid,
                        "bid": bid,
                        "ask": ask,
                    ...
                    fs_writer.emit_async(self.cfg.symbol, ts_ms, feature_set)
            except Exception as e:
                log.warning("⚠️ FeatureSnapshotWriter emission failed (fail-open): %s", e)
    ...
    def compute_signal_ttl(self, atr: float, regime: str, base_ttl_sec: int = 300) -> int:
        """Чем выше волатильность, тем короче жизнь сигнала."""
        vol_factor = {
            "momentum": 0.7,     # быстрее — момент короткий
            "ranging": 1.2,      # медленнее — диапазон держится
            "high_vol": 0.4,     # очень быстро — хаос
        }.get(regime, 1.0)
        
        # Approximate ATR normalization, fallback to 1.0
        atr_factor = 1.0
        # If we had historically tracked ATR percentiles in Redis, we could fetch:
        # atr_percentile = self.r.get(...) ...
        
        ttl = int(base_ttl_sec * vol_factor * atr_factor)
        return max(60, min(ttl, 600))  # [1 мин, 10 мин]
                ...
                if self.cfg.tick_stream:
                    ...
                    for msg_id, fields in tick_msgs:
                        try:
                            ...
                            ts_ms = int(data.get("ts", get_ny_time_millis()))
    ...
    def _enrich_crypto_orderflow_signal(self, signal: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        enriched.setdefault("ts", get_ny_time_millis())
