...
from common.normalization import generate_signal_id, normalize_direction, normalize_side_3
...
from core.data_health import apply_book_evidence_policy, compute_data_health
...
# ──────────────────────────────────────────────────────────────────────────────
# Hot-path ENV cache — eliminates os.getenv syscalls from per-tick processing.
# Refresh every 30s via monotonic timer (same pattern as AsyncSignalPublisher).
# ──────────────────────────────────────────────────────────────────────────────
class _StrategyEnvCache:
    ...
    def refresh(self) -> None:
        """Read all hot-path ENV vars. Called at init and every 30s."""
    ...
    def maybe_refresh(self) -> None:
        """Lazy refresh — only re-reads ENV if >30s since last refresh."""
...
class OrderFlowStrategy:
    ...
    def __init__(self, redis: aioredis.Redis, ticks: Any, publisher: AsyncSignalPublisher,
        ...
        # Hot-path ENV cache (P0 latency optimization)
    ...
    def _ensure_proof_state(self, now_ms: int):
        ...
        P0 FIX: increased throttle from 500ms to 5000ms to reduce syscalls.
        Disk I/O (exists/getmtime/open) is synchronous but only runs every 5s.
        """
        if (now_ms - self.conf_cal_proof_last_check_ms) < 5000:
             return
    ...
    async def process_tick(self, runtime: SymbolRuntime, tick: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            t1 = float(runtime.dynamic_cfg.get(DK.ATR_FLOOR_T1_BPS, runtime.config.get("atr_floor_t1_bps", 0.0)))
            t2 = float(runtime.dynamic_cfg.get(DK.ATR_FLOOR_T2_BPS, runtime.config.get("atr_floor_t2_bps", 0.0)))
            ...
            k_max = float(runtime.config.get("tp1_atr_k_max", 3.0)) # safety upper bound
            atr_val_floor = float(getattr(runtime, "last_atr", 0.0) or 0.0)
            ...
            if atr_val_floor > 0 and price > 0:
                ...
                indicators["pred_tp1_bps"] = float(tp1_bps)
        ...
        try:
            ...
            try:
                veto_thr = float(cfg.get("data_health_veto_below", cfg.get("data_health_shadow_only_below", 0.70)))
            except Exception:
                veto_thr = 0.70

            is_unhealthy = 1 if float(dh.score) < veto_thr else 0
            ...
            if is_veto == 1:
                ...
                if dh_mode in ("canary", "canary_enforce", "canary-only"):
                    ...
                    sid_str = str(getattr(runtime, "last_sid", f"{runtime.symbol}_{int(tick_ts)}"))
        ...
        if int(indicators.get("data_health_veto_active", 0)) == 1:
            self.logger.warning(
                "🛑 [DATA-HEALTH] (%s) ENFORCE VETO: score=%.2f reasons=%s — pipeline dropped",
                runtime.symbol,
                float(indicators.get("data_health", 0.0)),
        ...
        try:
            ...
            indicators["max_expected_slippage_bps_eff"] = float(max_expected_slippage_bps)
            ...
            indicators["expected_slippage_model_bps"] = float(est.expected_bps)

            # Decomp
            try:
                ip = float(indicators.get("impact_proxy", 0.0) or 0.0)
                size_usd = float(indicators.get("order_size_usd", cfg.get("slippage_decomp_size_ref_usd", 10000.0)))
                de = expected_slippage_decomp_bps(spread_bps=spr, impact_proxy=ip, cfg=cfg, order_size_usd=size_usd)
                indicators["expected_slippage_spread_bps"] = float(de.spread_bps)
                indicators["expected_slippage_impact_bps"] = float(de.impact_bps)
                indicators["expected_slippage_decomp_bps"] = float(de.total_bps)
        ...
        try:
            ...
            if runtime.l3_stats:
                ...
                indicators["added_bid_rate_ema"] = float(getattr(runtime.l3_stats, "added_bid_rate_ema", 0.0) or 0.0)
                indicators["added_ask_rate_ema"] = float(getattr(runtime.l3_stats, "added_ask_rate_ema", 0.0) or 0.0)
                indicators["added_total_rate_ema"] = float(indicators["added_bid_rate_ema"] + indicators["added_ask_rate_ema"])

                # New: VPIN-like toxicity proxy
            ...
            if ofc:
                ...
                if dec and hasattr(dec, "need") and hasattr(dec, "have"):
                    ...
                    if not is_ok:
                        ...
                        dec.ok = False
                        if "need_bump_veto" not in str(ofc.reason):
    ...
    async def _on_microbar_closed(self, runtime: SymbolRuntime, bar: MicroBar) -> None:
            ...
            if self._env.atr_tf_calib_enable:
                ...
                if now_ts > 0 and close_px > 0:
                    ...
                    for tf in cands:
                        ...
                        fresh = float(1.0 / (1.0 + (max(0, age_ms) / float(max(1, self._env.atr_tf_calib_max_age_ms) / 2))))
                        cons = 1.0
        ...
        # ------------------------------------------------------------------
            ...
            if close_px > 0 and atr_val > 0:
                ...
                runtime.dynamic_cfg[DK.ATR_CALIB_READY] = int(
                    1 if floors.n >= int(runtime.config.get("atr_bps_calib_min_samples", self._env.atr_bps_calib_min_samples)) else 0
                )
                ...
                try:
                    ...
                    if self._env.atr_bps_calib_enable and gap_ms > 0 and (int(bar.end_ts_ms) - last_p) >= gap_ms:
                        if self.calib_svc:
            ...
            if self._env.atr_tf_calib_enable:
                ...
                if now_ts > 0 and (now_ts - last) >= refresh_ms and close_px > 0:
                    ...
                    if atr_bps_by_tf:
                        ...
                        runtime.dynamic_cfg[DK.ATR_TF_READY] = int(1 if choice.src != "static" and choice.n >= self._env.atr_tf_calib_min_samples else 0)
        ...
        try:
            ...
            if (now_ts - int(getattr(runtime, "last_atr_ts_ms", 0) or 0)) >= refresh_ms:
                ...
                try:
                    if self._env.atr_tf_calib_enable and close_px > 0:
