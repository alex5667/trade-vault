...
class AsyncSignalPublisher:
    ...
    async def xadd_json(
        ...
        *,
        ...
        if not res.ok and not no_retry:
            try:
        ...
        *,
        ...
        if sink.name.startswith("orders:queue"):
            try:
                ...
                if not allow:
                    # Signal blocked by InvariantRuntimeEngine
                    errors += 1
                    return AsyncPublishResult(ok=False, raw_written=False, busy_loading=False, errors=errors)
