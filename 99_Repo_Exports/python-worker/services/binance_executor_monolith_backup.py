...
EXEC_ATTEMPT_TOTAL = _metric(Counter,
    "trading_exec_attempt_total",
    "Total execution attempts",
    ["symbol"],
)

EXEC_REJECT_TOTAL = _metric(Counter,
    "exec_reject_rate",
    "Total execution rejections by exchange",
    ["venue", "symbol"],
)
...
try:
    ...
    EXEC_SLIPPAGE_BPS = _metric(Histogram,
        "slippage_bps",
...
class BinanceExecutor:
    ...
    def handle_open(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if EXEC_SLIPPAGE_BPS is not None:
                entry_intent_price = _f(payload.get("entry_price"), 0.0)
                if entry_intent_price > 0 and avg_price > 0:
                    if logical == "LONG":
                        slip_bps = ((avg_price - entry_intent_price) / entry_intent_price) * 10000.0
                    else:
                        slip_bps = ((entry_intent_price - avg_price) / entry_intent_price) * 10000.0
                    EXEC_SLIPPAGE_BPS.labels(symbol=symbol, venue="binance").observe(slip_bps)
        except Exception:
            pass
    ...
    def process_one(self, raw: str) -> None:
        ...
        if EXEC_ATTEMPT_TOTAL is not None:
            EXEC_ATTEMPT_TOTAL.labels(symbol=symbol).inc()
