...
def _build_trade_from_fields(fields: dict[str, str]) -> Trade | None:
    ...
    # 💰 Risk & R-metrics hydration
    # Fix for runtime errors where one_r_money or pnl_if_fixed_exit are missing
    ...
    if p_fixed is not None:
        ...
        # Fallback: if baseline pnl is missing, assume trailing didn't change it (Edge=0)
        # OR if it was a TP hit, gross pnl might be a better approximation of fixed exit
        pnl_fixed = _sf(fields.get("pnl_gross") or pnl_net_val, 0.0)

    # All fields for tag_stats.Trade
    return Trade(
        source=src,
        symbol=sym,
        ...
        one_r_money=one_r,
        giveback=_sf(fields.get("giveback_pnl") or fields.get("giveback") or 0.0),
        missed_profit=_sf(fields.get("missed_profit_pnl") or fields.get("missed_profit") or 0.0),
        mfe_pnl=_sf(fields.get("mfe_pnl") or fields.get("mfe_usd") or 0.0),
        mae_pnl=_sf(fields.get("mae_pnl") or fields.get("mae_usd") or 0.0),
        trailing_started=trailing_started_final,
