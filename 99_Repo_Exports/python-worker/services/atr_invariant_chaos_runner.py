...
def run_once() -> dict[str, Any]:
    drill_code = os.getenv("ATR_INVARIANT_CHAOS_DRILL", "BUY_ORDERING_BROKEN")
    mode = os.getenv("ATR_INVARIANT_CHAOS_MODE", "audit_only")
    ...
    run_id = f"chaos_{datetime.datetime.now(datetime.UTC).strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
