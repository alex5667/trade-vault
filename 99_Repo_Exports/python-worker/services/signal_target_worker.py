...
local send = 1
local incr_ok, cnt = pcall(redis.call, 'INCR', KEYS[4])
if not incr_ok then
  redis.call('DEL', KEYS[2])
  return {-4}
end
...
class SignalTargetWorker:
    ...
    def __init__(self, target: str):
        ...
        self.visibility_ms = int(os.getenv("SIGNAL_TARGET_VISIBILITY_MS", "120000"))
    ...
    def _deliver_notify(self, sid: str, payload: Dict[str, Any]) -> bool:
        ...
        if code == -4:
            from redis.exceptions import TimeoutError
            raise TimeoutError("notify_incr_failed_retry")
        if code in (1, 2):
            self._complete_cleanup(sid)
            return True
        return False
    ...
    def _process_one(self, task: Dict[str, Any]) -> None:
        ...
        if attempt >= self.max_attempts:
            if not self._send_dlq("max_attempts", task):
