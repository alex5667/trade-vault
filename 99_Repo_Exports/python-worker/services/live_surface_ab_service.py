...
def _min_group_n() -> int:
    try:
        return int(os.getenv("LIVE_SURFACE_AB_MIN_GROUP_N", "50") or 50)
    except Exception:
        return 50
...
def _window_days() -> int:
    try:
        return int(os.getenv("LIVE_SURFACE_AB_WINDOW_DAYS", "21") or 21)
    except Exception:
        return 21


def run_once() -> int:
    conn = psycopg2.connect(_dsn(), connect_timeout=5, application_name="live_surface_ab_service")
    r = _redis()
    written = 0
    try:
        with conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                ...
                WITH base AS (
                  SELECT
                    ...
                    coalesce(t.live_surface_applied, false) AS live_surface_applied,
                    count(*) AS n,
            ...
            cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS horizon_live_surface_ab_daily (
                  ...
                  live_surface_applied boolean NOT NULL,
                  n integer NOT NULL,
            ...
            INSERT INTO horizon_live_surface_ab_daily (
              day, source, symbol, scenario, regime, risk_horizon_bucket, live_surface_applied,
            ...
            cur.execute(
                ...
                WITH pair AS (
                  SELECT
                    ...
                    max(CASE WHEN live_surface_applied THEN avg_slippage_bps END) AS slip_canary,
                    max(CASE WHEN NOT live_surface_applied THEN avg_slippage_bps END) AS slip_control
