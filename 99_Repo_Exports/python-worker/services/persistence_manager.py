...
class PersistenceManager:
    ...
    async def _get_pool(self) -> asyncpg.Pool:
        ...
        each creating their own pool (race condition observed as triple
        '🔌 Initializing asyncpg pool' log lines at startup).
        """
        # Fast-path: pool already exists (no lock needed after creation).
        if self._pool is not None:
            return self._pool

        # Lazy-init the lock inside a running event loop.
