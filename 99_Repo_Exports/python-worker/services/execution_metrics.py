...
# ---------------------------------------------------------------------------
# P0-5: Kill-switch arming timestamp (PROTECTION_ARM_TIMEOUT blind-spot guard)
# ---------------------------------------------------------------------------
# Set to time.time() when executor enters FSM_PROTECTION_ARMING.
# Reset to 0 when protection is confirmed (FSM_PROTECTED) or emergency-flattened
# (FSM_EMERGENCY_FLATTENED). A non-zero value older than PROTECTION_ARM_TIMEOUT_MS
# means the position may be naked with no pager alert fired.
# Alert rule: KillSwitchTimeoutExceeded (severity=page) fires when
#   (time() - kill_switch_armed_timestamp) > PROTECTION_ARM_TIMEOUT_MS / 1000 * 1.5
KILL_SWITCH_ARMED_TIMESTAMP = _metric(
    Gauge,
    "kill_switch_armed_timestamp",
    "Unix timestamp (seconds) when executor entered PROTECTION_ARMING for this symbol. "
    "0 = not armed or already protected. Non-zero = protection not yet confirmed.",
    ["symbol"],
)

# ---------------------------------------------------------------------------
# P0-5b: kill_switch_active — binary armed state for alerting
# ---------------------------------------------------------------------------
# Complements kill_switch_armed_timestamp by providing a simple 0/1 gauge that
# alert rules can aggregate with `max by ()` / `sum by (scope)` without parsing
# a Unix timestamp. Set to 1 when a kill switch is armed (symbol-level or
# global/cohort), 0 when cleared. Labels:
#   scope   — "symbol" | "cohort" | "global"
#   reason  — short tag, e.g. "protection_arming_timeout", "risk_breach",
#             "manual", "drift_autosilence"; use "" when unknown.
#
# Alert suggestion (see docs/OBSERVABILITY_ALERTS.md):
#   - KillSwitchActive: max by(scope, reason)(kill_switch_active) == 1 for 1m
#   - KillSwitchActiveLong: same but `for 15m` → page oncall
KILL_SWITCH_ACTIVE = _metric(
    Gauge,
    "kill_switch_active",
    "1 when a kill switch is armed, 0 when cleared. "
    "Pairs with kill_switch_armed_timestamp; intended for alerting rules.",
    ["scope", "reason"],
)
