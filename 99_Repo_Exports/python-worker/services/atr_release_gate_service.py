...
def _check_inv_no_live_without_replay(change: dict[str, Any], replay_status: str) -> str | None:
    ...
    if (change.get("scenario", "")).startswith("live") or (change.get("layer", "")).startswith("live"):
        is_live = True
...
def build_scorecard(change_id: str) -> dict[str, Any]:
    ...
    with get_conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        ...
        scenario = (change.get("scenario") or "")
...
def decide_release(change_id: str) -> dict[str, Any]:
    ...
    try:
        from services.atr_graph_consistency_gate import _ENFORCE, decide_graph_consistency
        # Fetch risk_level
        with get_conn() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
