...
log = setup_logger("tp_hit_trailing_orchestrator")
...
class TpHitTrailingOrchestrator:
    ...
    def _process_tp1_event(self, event: Dict[str, Any], record_stats: bool) -> TrailingResult:
        ...
        if atr_value and atr_value > 0:
            try:
                ...
                payload_val = signal.get("trail_atr_mult_calibrated")
                if payload_val and float(payload_val) > 0:
                    custom_atr_mult = float(payload_val)
                else:
                    raw_val = self.r.hget(f"symbol_specs:{symbol}", "trailing_tp1_offset_atr")
                    if raw_val and float(raw_val) > 0:
                        custom_atr_mult = float(raw_val)
            except Exception as e:
                log.warning("⚠️ Failed to read calibrated trailing offset for %s: %s", symbol, str(e)[:200])

        active_atr_mult = custom_atr_mult if custom_atr_mult else profile.atr_mult

        if profile.mode == "BREAKEVEN":
            trail_distance_price = point_size # just a dummy > 0 to pass checks
        elif atr_value and atr_value > 0:
            trail_distance_price = atr_value * active_atr_mult
            if custom_atr_mult:
                log.info(
                    "🎯 Using dynamically calibrated ATR multiplier %.2f for %s (profile default: %.2f)",
                    active_atr_mult, symbol, profile.atr_mult
                )
        ...
        if atr_value and atr_value > 0:
            ...
            if custom_atr_mult:
                metadata["trail_atr_mult_calibrated"] = custom_atr_mult
        if trail_points:
    ...
    def _persist_signal_sl_update(
        ...
        try:
            ...
            signal.setdefault("trailing_history", []).append({
                ...
                "new_sl": new_sl,
                "reason": "tp_hit_trailing_orchestrator",
    ...
    def _compute_trailing_sl(
        ...
        if profile_mode == "BREAKEVEN" and entry_price and entry_price > 0:
            # Смещаем SL в безубыток с компенсацией небольшого fee (0.1%)
            if side == "SHORT":
                candidate = entry_price * 0.999
                if original_sl is not None:
                    candidate = min(candidate, original_sl)
                candidate = self._round_to_point(side, candidate, point)
            else:
                candidate = entry_price * 1.001
                if original_sl is not None:
