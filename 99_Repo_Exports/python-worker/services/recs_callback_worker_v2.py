            ...
            for msg_id, fields in msgs:
                ...
                try:
                    ...
                    if action == "confirm":
                        ...
                        if not _rich_of_gate_notify(r, bundle_id, "confirm", "applied", bundle):
                            notify(r, f"<b>Applied</b>\nid=<code>{bundle_id}</code>\nops=<code>{n}</code>\nactor=<code>{actor}</code>")
                        r.xack(stream, group, msg_id)
                        continue

                    if action in ("reject", "reject2"):
                        if st in ("APPLIED", "ROLLED_BACK"):
                            notify(r, f"<b>Reject ignored</b>\nid=<code>{bundle_id}</code> status=<code>{st}</code>")
                        else:
                            set_status(r, bundle_id, "REJECTED", ttl)
                            if not _rich_of_gate_notify(r, bundle_id, "reject", "rejected", bundle):
                                notify(r, f"<b>Rejected</b>\nid=<code>{bundle_id}</code>\nactor=<code>{actor}</code>")
                        r.xack(stream, group, msg_id)
                        continue

                    if action == "rollback":
                        n = rollback_ops(r, bundle_id, ttl, actor)
                        if not _rich_of_gate_notify(r, bundle_id, "rollback", "rolled_back", bundle):
