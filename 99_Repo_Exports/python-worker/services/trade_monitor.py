...
class TradeMonitorService:
    ...
    def __init__(
        ...
        *,
        ...
        # Background Housekeep Loop (Replaces per-tick synchronous housekeep)
        self._housekeep_thread_stop = threading.Event()
        self._housekeep_thread = threading.Thread(
            target=self._housekeep_loop, daemon=True, name="TMHousekeep"
        )
    ...
    def _warmup_price_cache(self) -> None:
        ...
        Reads the last entry of stream:tick_{SYMBOL} for each symbol that has an open position.
        This prevents ORPHAN_TIMEOUT_NO_PRICE on positions opened before restart.

        ENV:
          REDIS_TICKS_URL          — URL to redis-ticks shard (default: redis://redis-ticks:6379/0)
          TM_WARMUP_MAX_PRICE_AGE_MS — max age of tick price to accept (default: 600_000 ms = 10 min)
        """
        ...
        for sym in symbols:
            ...
            try:
                ...
                # Parse timestamp from stream ID (epoch_ms-seq)
                try:
                ...
                if age_ms > max_age_ms:
                    logger.debug("[warmup] %s: price too old (%d ms), skipping", sym, age_ms)
                    skipped_old += 1
                    continue

                price = float(
                    fields.get("mid") or fields.get("price") or fields.get("last") or 0.0
                )
                if price > 0:
                    with self._lock:
                        # Only update if we don't already have a fresher price
                ...
                logger.debug("[warmup] %s: error reading tick: %s", sym, e)
                skipped_err += 1
    ...
    def _index_add(self, pos: PositionState) -> None:
        ...
        if self._price_index_enabled:
            try:
                ...
                if tp_price > 0:
                    if sym not in self._tp_index:  # type: ignore
    ...
    def _index_remove(self, pos: PositionState) -> None:  # type: ignore
    ...
    def _collect_candidate_pos_ids(self, symbol: str, mid: float) -> set[str] | None:  # type: ignore
        """
  ...
  # type: ignore
        try:
            ...
            health_snapshot_key = f"orderflow:{symbol}:health_snapshot"  # type: ignore
    ...
    def _get_health_snapshot_for_trade(self, symbol: str) -> dict[str, str]:  # type: ignore
        """
    ...
    def _position_from_hash(self, h: dict[str, str]) -> PositionState | None:
        try:
            ...
            pos = PositionState(
                ...
                symbol=(h.get("symbol") or "UNKNOWN"),
                tf=(h.get("tf") or "tick"),  # type: ignore
                ...
                lot=float(h.get("lot") or 0.0),
                qty=float(h.get("qty") or h.get("lot") or 0.0),  # type: ignore
                quantity=float(h.get("quantity") or h.get("lot") or 0.0),
                remaining_qty=float(h.get("remaining_qty") or h.get("lot") or 0.0),
                sl=float(h.get("sl") or 0.0),
                tp_levels=tp_levels,
    ...
    def _peek_pos_and_symbol_by_sid(self, sid: str) -> tuple[str | None, str | None]:  # type: ignore
        ...
        ВАЖНО: после выхода из self._lock symbol может устареть — ниже всегда делаем re-check  # type: ignore
    ...
    def _io_save_trailing_sync(self, pos: PositionState, ts: int) -> None:
        self.repo.save_trailing_sync(pos, ts)  # type: ignore
    ...
    def _io_save_trailing_move(self, pos: PositionState, previous_sl: float, new_sl: float, ts_ms: int) -> None:
        self.repo.save_trailing_move(pos, previous_sl, new_sl, ts_ms)  # type: ignore
    ...
    def _stamp_closed_trade_meta(self, pos: PositionState, closed: TradeClosed, close_reason_raw: str) -> None:
        ...
        try:
            ...
            closed.trailing_policy_level = str(getattr(pos, "trailing_policy_level", ""))  # type: ignore
        except Exception:  # type: ignore
            pass
  # type: ignore
        # Phase 0.3: copy scalar horizon/ATR fields onto closed trade for analytics.  # type: ignore
        with contextlib.suppress(Exception):
    ...
    def on_signal(self, raw_signal: dict[str, Any]) -> str | None:
        ...
        with self._lock:
            ...
            if len(self._signal_buffer) > 100:
                logger.warning(f"⚠️ [JITTER_BUFFER] Backlog is high: {len(self._signal_buffer)} signals")  # type: ignore
  # type: ignore
            with contextlib.suppress(Exception):
    ...
    def _process_signal_norm(self, sig: SignalNorm) -> str | None:
        ...
        with self._lock:
            ...
            pos.v_gate_reason = str(sig.payload.get("v_gate_reason") or sig.payload.get("validation_reason") or "")
            ...
            try:
                ...
                # giving calibrators (cont_ctx_window, adverse_gate, etc.) a real outcome feedback loop.  # type: ignore
                try:  # type: ignore
        ...
        try:
            ...
            if is_replay:
                ...
                if entry_ts_ms > effective_now_ms:
                    ...
                    if skew > 5:  # Only log significant skews to prevent spam
                        if self.logger:
                            self.logger.warning(f"⚠️ Future entry timestamp detected: {entry_ts_ms} > {effective_now_ms} (skew={skew}ms, ctx=replay). Clamping to market time.")
    ...
    def _housekeep_loop(self) -> None:
        """Background thread loop to run global housekeep periodically."""
        interval_sec = max(1.0, self._orphan_housekeep_interval_ms / 1000.0)
        logger.info(f"Started TMHousekeep thread, interval={interval_sec}s")
        while not getattr(self, "_housekeep_thread_stop", threading.Event()).is_set():
            ...
            try:
                ...
                logger.error(f"Housekeep loop error: {e}")
            finally:
            ...
            # Wait with small chunks to allow quick shutdown
        ...
        with sym_ctx:
            ...
            for pos in pos_list:
                    ...
                    if ev.event_type == "TP_HIT":
                        ...
                        if int(p.get("tp_level", 0)) == self._trail_tp_activate_level and getattr(pos, "sid", ""):
                            ...
                            try:
                                ...
                                if self._trailing_local_fallback and self._is_trailing_after_tp1_enabled(pos, spec):
                                    if pos_atr > 0:
                                        # Application logic
                ...
                if closed:
                    ...
                    hs = {}
                    try:
                        hs = self._get_health_snapshot_for_trade(str(closed.symbol))  # type: ignore
                    except Exception:
                        hs = {}
            ...
            for kind, payload in io_steps:  # type: ignore
                try:
                    if kind == "append_event":
                        ...
                        try:
                            _d = payload if isinstance(payload, dict) else {}
                            if not _d:
                                logger.error(f"save_trailing_sync payload is not a dict: type={type(payload)}")  # type: ignore
                                continue
                        ...
                        try:
                            _d = payload if isinstance(payload, dict) else {}
                            if not _d:
                                logger.error(f"save_closed payload is not a dict: type={type(payload)}")  # type: ignore
                                continue
                        ...
                        try:
                            ...
                            fut.add_done_callback(_log_future_exception)  # type: ignore
                        except Exception as e:
                            logger.warning("Failed to submit trade to analytics DB: %s", e)
            ...
            self.tm_tick_latency_us.labels(symbol=symbol).observe(t_dur_us)  # type: ignore
        ...
        *,
        ...
        with self._symbol_lock_ctx(sym):
            ...
            try:
                ...
                if tp_level >= 1:
                    fsm_from = getattr(self._fsm_map.get(getattr(pos, "id", "")), "status", None)  # type: ignore
  ...
  # type: ignore
    ...
    def _resolve_closed_at(self, closed) -> datetime:  # type: ignore
        ...
        if isinstance(closed_at, (int, float)):
            ts_sec = float(closed_at)  # type: ignore
    ...
    def _update_stats(self, pos: PositionState, closed) -> None:
        try:
            ...
            pos_dict = asdict(pos) if hasattr(pos, "__dataclass_fields__") else dict(getattr(pos, "__dict__", {}) or {})
            closed_dict = asdict(closed) if hasattr(closed, "__dataclass_fields__") else dict(getattr(closed, "__dict__", {}) or {})
            StatsAggregator.update_stats(self.redis, pos_dict, closed_dict)  # final-close event
            ...
            logger.warning("stats update failed: %s", e)  # type: ignore
