...
class LiquidationMapService:
    ...
    def _publish_symbol(self, symbol: str, now_ms: int) -> None:
        ...
        for wname, agg in aggs.items():
            ...
            import orjson
            j_bytes = orjson.dumps(payload)
            j = j_bytes.decode("utf-8")
            key = f"{self.snapshot_key_prefix}:{symbol}:{wname}"
            self.r.set(key, j_bytes, ex=self.snapshot_ttl_sec)

            liqmap_levels_gauge.labels(symbol=symbol, window=wname).set(len(levels))
            liqmap_snapshot_bytes.labels(symbol=symbol, window=wname).set(len(j_bytes))
        ...
        while True:
            ...
            if loop_start - self._last_autoclaim_ms >= self._autoclaim_every_ms:
                try:
                    ...
                    logger.warning("autoclaim skipped: Redis is loading dataset in memory")
                except redis.exceptions.ConnectionError as e:
                    logger.warning("autoclaim skipped: Redis connection error: %s", e)
                except Exception as e:
            ...
            try:
                ...
                logger.warning("read/process skipped: Redis is loading dataset in memory")
                time.sleep(2.0)
            except redis.exceptions.ConnectionError as e:
                logger.warning("read/process skipped: Redis connection error: %s", e)
                time.sleep(1.0)
            except Exception as e:
            ...
            if now_ms - self._last_publish_ms >= self.publish_interval_ms:
                try:
                    ...
                    logger.warning("publish skipped: Redis is loading dataset in memory")
                except redis.exceptions.ConnectionError as e:
                    logger.warning("publish skipped: Redis connection error: %s", e)
                except Exception as e:
