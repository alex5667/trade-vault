...
class BookSnapshot:
    ...
    def from_raw(book: dict) -> BookSnapshot:
        ...
        sort_fallback = os.getenv("BOOK_SORT_FALLBACK", "0").strip().lower() in ("1", "true", "yes", "on")
...
from services.orderflow.configuration import DEFAULT_CONFIG, _safe_int
from services.orderflow.manip_patterns import ManipulationTracker

# Phase E / P4: Manipulation patterns (OTR, quote stuffing, layering)
...
# P5 stream integrity (seq gaps/dups + schema drift)
...
class SymbolRuntime:
    ...
    # Rolling trackers (A3). Updated on microbar close (bar_processor) and
    # exposed to tick_processor through `runtime.dynamic_cfg`.
    ...
    # only covers Shard 1/1B/2 and leaves Shard 3/3B with regime=na.
    _regime_vwap: float = 0.0
    _regime_pv: float = 0.0
    _regime_vol: float = 0.0
    _regime_day_id: int = 0
    _regime_open_day: float = 0.0
    _regime_delta_ema: float = 0.0
    _regime_last_side: int = 0
    ...
    book_seq_last_reason: str = "init"
    book_missing_seq_ema: float = 0.0

    # OFInputs V3 circuit breaker cache (P100)
    of_inputs_v3_disabled_until_ms: int = 0
    of_inputs_v3_disabled_hard_until_ms: int = 0
    of_inputs_v3_disabled_reason: str = ""
    of_inputs_v3_cb_last_refresh_ts_ms: int = 0
    ...
    ofi_norm_stats: RollingRobustZ = field(init=False)
    ofi_norm_z: float = 0.0
    ...
    # P5: book sanity (crossed BBO, NaNs, neg qty). Populated by BookProcessor.
    # Stored as simple fields to keep Redis snapshots small.
    book_sanity_ok: int = 1
    book_sanity_flags: str = ""  # CSV
    ...
    # Set by BookProcessor.process_book(), read by TickProcessor for indicators + of_inputs.
    last_lob_event: dict[str, Any] | None = None
    lob_qi_mean: float = 0.0          # mean queue imbalance L1..L5
    lob_qi_max_abs: float = 0.0       # max abs queue imbalance
    lob_qi_slope: float = 0.0         # imbalance slope over depth levels
    lob_micro_mid_div_bps: float = 0.0  # microprice vs mid (bps)
    lob_micro_shift_bps: float = 0.0    # microprice shift from prev snapshot (bps)
    lob_depth_slope_bid: float = 0.0
    lob_depth_slope_ask: float = 0.0
    lob_depth_slope_imb: float = 0.0  # bid - ask slope
    lob_depth_convexity_bid: float = 0.0
    lob_depth_convexity_ask: float = 0.0
    lob_depth_convexity_imb: float = 0.0  # bid - ask convexity
    lob_dw_obi: float = 0.0           # depth-weighted OBI (near-touch emphasis)
    dw_obi_z: float = 0.0             # robust z-score of dw_obi
    dw_obi_stability_score: float = 0.0  # stability quality [0..1]
    dw_obi_stable_secs: float = 0.0   # continuous stable seconds
    dw_obi_stable: bool = False        # is dw_obi currently stable?
    # Initialized in __post_init__ (RollingRobustZ + OBIStabilityTracker)
    ...
    # P5: Stream integrity telemetry (seq gaps/dups + schema hash).
    # These trackers do not veto by themselves; gates consume their outputs.
    tick_integrity: StreamIntegrityTracker = field(init=False)
    ...
    # Trail Calibrator enforced params (per-symbol, from trail:calib:{sym}:{regime})
    # Loaded in ensure_specs_fresh(), used by signal_pipeline for trail_callback_pct.
    # None = no calibrated params available or mode != enforce.
    trail_calib_params: dict[str, Any] | None = None
    ...
    # -----------------------------------------------------------------------
    # Cross-asset / v12_of runtime attributes (hydrated from go-worker via Redis)
    # Published by go-worker/internal/crossasset.Tracker to:
    #   HSET runtime:crossasset:{SYMBOL}  <field> <value>
    # Fail-open defaults: all 0.0 (safe for model inference).
    # -----------------------------------------------------------------------
    eth_btc_corr_5m: float = 0.0
    perp_spot_basis_bps: float = 0.0
    stable_coin_flow_delta: float = 0.0
    depth_migration_bps_ema: float = 0.0
    _crossasset_last_load_ms: int = 0

    async def maybe_load_crossasset(self, r) -> None:
        """
        Periodically load v12_of cross-asset metrics from Redis Hash
        written by go-worker/internal/crossasset.Tracker.

        Key: runtime:crossasset:{SYMBOL}
        Fields: eth_btc_corr_5m, perp_spot_basis_bps, stable_coin_flow_delta,
                depth_migration_bps_ema

        Cache TTL: 5s  (go-worker updates ~every book event at ~100ms)
        Fail-open: on any error, previous values are preserved (default 0.0).
        """
        try:
            ...
            def _f(k: str) -> float:
                v = raw.get(k) or raw.get(k.encode(), None)
                try:
                    return float(v) if v is not None else 0.0
                except (TypeError, ValueError):
                    return 0.0

            self.eth_btc_corr_5m       = _f("eth_btc_corr_5m")
            self.perp_spot_basis_bps   = _f("perp_spot_basis_bps")
            self.stable_coin_flow_delta = _f("stable_coin_flow_delta")
            self.depth_migration_bps_ema = _f("depth_migration_bps_ema")
            self._crossasset_last_load_ms = now_ms
        except Exception:
            # Fail-open: any error leaves current values unchanged.
            return

    # -----------------------------------------------------------------------
    # Cross-asset / v13_of runtime attributes (ND group)
    # Published by go-worker to HSET runtime:crossasset:v13:{SYMBOL}
    # Fail-open defaults: all 0.0 until go-worker REST polling deployed.
    # -----------------------------------------------------------------------
    btc_dominance_momentum: float = 0.0
    oi_weighted_funding: float = 0.0
    total_market_oi_delta: float = 0.0
    liq_heatmap_distance_bps: float = 0.0
    long_short_ratio: float = 0.0
    _crossasset_v13_last_load_ms: int = 0

    async def maybe_load_crossasset_v13(self, r) -> None:
        """
        Periodically load v13_of cross-asset metrics (ND group) from Redis Hash.
        Key: runtime:crossasset:v13:{SYMBOL}
        Fail-open: all default to 0.0.
        """
        try:
            now_ms = get_ny_time_millis()
            ...
            self.btc_dominance_momentum = _f("btc_dominance_momentum")
            self.oi_weighted_funding    = _f("oi_weighted_funding")
            self.total_market_oi_delta  = _f("total_market_oi_delta")
            self.liq_heatmap_distance_bps = _f("liq_heatmap_distance_bps")
            self.long_short_ratio       = _f("long_short_ratio")
            self._crossasset_v13_last_load_ms = now_ms
        except Exception:
            return
    ...
    last_u: int = 0
    last_id: int = 0
    # P2/F: aggTradeId continuity tracking (Binance futures)
    last_trade_id: int = 0
    ...
    # Phase E / P4: Message rate + manipulation pattern trackers
    # msg_rate: tracks book_update_rate_hz/z, trade_msg_rate_hz/z, cancel_rate_z, otr, otr_z
    # manip:   tracks quote_stuffing_score, layering_score, manip_flags
    msg_rate: MessageRateTracker = field(init=False)
    manip: ManipulationTracker = field(init=False)
    # Cached outputs (read by strategy / indicators)
    ...
    def __post_init__(self) -> None:
        ...
        # v13_of runtime tracker (OHLC vol, liquidity, toxicity, entropy, mean reversion)
        self.v13_tracker = V13RuntimeTracker()
        ...
        tau_ms = int(self.config.get("dq_seq_tau_ms", tau_default) or tau_default)
        self.tick_seq_gap = GapEmaTracker(tau_ms=tau_ms)
        self.book_seq_gap = GapEmaTracker(tau_ms=tau_ms)

        # P5 stream integrity trackers (gaps/dups/schema drift). These are hot-path safe.
        z_window = int(self.config.get("dq_seq_z_window", 120) or 120)
        maxgap_win_ms = int(self.config.get("dq_seq_maxgap_window_ms", 60_000) or 60_000)
        ...
        # --- LOB pressure P91: dw_obi rolling robust-z and stability tracker ---
        # dw_obi_stats: shared RollingRobustZ for depth-weighted OBI z-score.
        # dw_obi_tracker: reuses OBIStabilityTracker with same config as obi_tracker.
        ...
        # Window controls smoothness and false positives. Keep bounded for hot path.
        try:
        ...
        # Both are stateful, single-writer (async hot-path); no locking needed.
        self.msg_rate = MessageRateTracker()
        ...
        try:
            v_fast = float(self.config.get("vol_fast_alpha", float(os.getenv("VOL_FAST_ALPHA", "0.25"))))
            v_slow = float(self.config.get("vol_slow_alpha", float(os.getenv("VOL_SLOW_ALPHA", "0.03"))))
            v_win = int(self.config.get("vol_ratio_z_window", int(os.getenv("VOL_RATIO_Z_WINDOW", "240"))))
            self.vol_regime = VolRegimeTracker(fast_alpha=v_fast, slow_alpha=v_slow, z_window=v_win)
        except Exception:
            self.vol_regime = VolRegimeTracker()

        # Liquidity resilience tracker (depth replenishment after sweeps)
        try:
            tgt = float(self.config.get("res_target_recovery_ratio", float(os.getenv("RES_TARGET_RECOVERY_RATIO", "0.85"))))
            wnd = int(self.config.get("res_max_window_ms", int(os.getenv("RES_MAX_WINDOW_MS", "15000"))))
            self.resilience = BookResilienceTracker(target_recovery_ratio=tgt, max_window_ms=wnd)
        except Exception:
            self.resilience = BookResilienceTracker()

        # Phase C/P2: liquidity resiliency tracker (spread/depth recovery time).
        # Complementary to BookResilienceTracker (depth replenishment after sweeps).
        try:
            a = float(self.config.get("liq_res_alpha", float(os.getenv("LIQ_RES_ALPHA", "0.02"))))
            s_mult = float(self.config.get("liq_res_stress_spread_mult", float(os.getenv("LIQ_RES_STRESS_SPREAD_MULT", "1.75"))))
            d_drop = float(self.config.get("liq_res_stress_depth_drop_frac", float(os.getenv("LIQ_RES_STRESS_DEPTH_DROP_FRAC", "0.50"))))
            r_mult = float(self.config.get("liq_res_recover_spread_mult", float(os.getenv("LIQ_RES_RECOVER_SPREAD_MULT", "1.20"))))
        ...
        # These are updated in BarProcessor and stored into runtime.dynamic_cfg.
        try:
    ...
    def get_atr_tf_selected(self) -> str:
        ...
        """
        tf = str((self.dynamic_cfg or {}).get("atr_tf_selected") or self.config.get("atr_tf") or "5m")
        tf = tf.strip()
        return tf if tf else "5m"
    ...
    def apply_config(self, new_config: dict[str, Any]) -> None:
        ...
        # Manually push ML Gate overrides into dynamic_cfg for strategy.py
        if "sg_calib_mode" in self.config:
            self.dynamic_cfg[DK.SG_CALIB_MODE] = str(self.config["sg_calib_mode"])
        if "adv_calib_mode" in self.config:
            self.dynamic_cfg[DK.ADV_CALIB_MODE] = str(self.config["adv_calib_mode"])

        try:
            tb_size = int(self.config.get("tick_buffer") or 1000)
            tb_size = max(1, tb_size)
        except Exception:
            tb_size = 1000
        self.tick_buffer = deque(prev_ticks, maxlen=tb_size)
    ...
    async def ensure_specs_fresh(self, r) -> None:
        ...
        Optimized: batches GET/MGET calls into a single Redis pipeline to reduce round-trips.
        """
        now = get_ny_time_millis()
        # Cache for 60s
        ...
        try:
            ...
            _slip_buckets = ("NORMAL", "LOW_LIQ", "HIGH_VOL", "HIGH_VOL_LOW_LIQ")
            _slip_keys = [
                f"cfg:slippage_decomp_impact_coeff_bps:{sym}:{b}"
                for b in _slip_buckets
            ]
            _enforce_keys = [
            ...
            # --- Batched pipeline: 1 round-trip for all GET/MGET commands ---
            # HGETALL is sent separately (pipeline still saves 2+ round-trips vs sequential)
            try:
                pipe = r.pipeline(transaction=False)
                pipe.get(f"symbol_specs:{sym}")               # idx 0: specs JSON
                pipe.mget(*_slip_keys)                        # idx 1: slippage coeffs
                pipe.mget(*_enforce_keys)                     # idx 2: enforce buckets
                pipe_results = await pipe.execute()
            except Exception:
                # Fallback: sequential (graceful degradation)
                pipe_results = [
                    await r.get(f"symbol_specs:{sym}"),
                    await r.mget(*_slip_keys),
                    await r.mget(*_enforce_keys),
                ]

            # --- Process specs ---
            raw = pipe_results[0] if pipe_results else None
            if raw:
                try:
            ...
            # ---- Trail Calibrator (HGETALL — separate call, cannot pipeline with GET) ----
            try:
                _trail_raw = await r.hgetall(_trail_key)
                if _trail_raw and (_trail_raw.get("mode", "shadow")) == "enforce":
                    _cb = float(_trail_raw.get("callback_atr_mult", 0) or 0)
                    if _cb > 0:
                        _comp_ms = int(float(_trail_raw.get("computed_at_ms", 0) or 0))
                        _max_stale = 172_800_000  # 48h
                        _age = get_ny_time_millis() - _comp_ms if _comp_ms > 0 else 0
                        if _age <= _max_stale or _comp_ms == 0:
                            self.trail_calib_params = {
                                "callback_atr_mult": _cb,
                                "activate_offset_bps": float(_trail_raw.get("activate_offset_bps", 5.0) or 5.0),
                                "min_profit_lock_r": float(_trail_raw.get("min_profit_lock_r", 0.1) or 0.1),
                                "confidence": float(_trail_raw.get("confidence", 0) or 0),
                                "mode": "enforce",
                                "regime": _regime,
                            }
                        else:
                            self.trail_calib_params = None  # stale
                    else:
                        self.trail_calib_params = None
                else:
                    self.trail_calib_params = None  # shadow mode or missing
            except Exception:
                pass  # fail-open: keep previous value

            # --- Process slippage decomp coefficients (from pipeline idx 1) ---
            try:
                _slip_vals = pipe_results[1] if len(pipe_results) > 1 else None
                _coeff_map = {}
                if _slip_vals:
                    for _b, _v in zip(_slip_buckets, _slip_vals):
                        if _v in (None, "", "na"):
                            continue
            ...
            # --- Process enforce bucket allowlists (from pipeline idx 2) ---
            try:
                vals = pipe_results[2] if len(pipe_results) > 2 else None
                sd = None
                tf = None
                if vals:
                    # take first non-empty in priority order
                    for k, v in zip(_enforce_keys, vals):
                        if v in (None, "", "na"):
                            continue
                        if k.startswith("cfg:slippage_decomp_enforce_buckets"):
                            sd = str(v)
                            break
                    for k, v in zip(_enforce_keys, vals):
                        if v in (None, "", "na"):
                            continue
                        if k.startswith("cfg:taker_flow_gate_enforce_buckets"):
                            tf = str(v)
                            break
                ...
                if tf:
                    with contextlib.suppress(Exception):
                        self.dynamic_cfg[DK.TAKER_FLOW_GATE_ENFORCE_BUCKETS] = tf
                    try:
                        if isinstance(self.config, dict):
                            self.config["taker_flow_gate_enforce_buckets"] = tf
                    except Exception:
                        pass
            except Exception:
                pass
