...
# Strict cardinality control for reason_code to prevent Prometheus blast
ALLOWED_REASON_CODES = {
    "ok_hard", "ok_soft", "dq_fail", "drift_block", "veto", 
    "book_stale", "wide_spread", "liq_geom", "flow_tox_veto", 
    "deriv_ctx_veto", "atr_floor_veto", "book_sanity_veto", "cvr_veto"
}
...
def enrich_schema_fields(row: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
    ...
    if rc in (None, ""):
        ...
        sanitized_rc = why_label(rc)
        # Enforce cardinality
        if sanitized_rc in ALLOWED_REASON_CODES:
            row["reason_code"] = sanitized_rc
        else:
            row["reason_code"] = "other_veto"
