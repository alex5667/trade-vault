...
def main(argv: Optional[Sequence[str]] = None) -> int:  # pragma: no cover
    ...
    if args.wait_until_ready or args.run_executor:
        ...
        if args.run_executor:
            ...
            # Start Prometheus metrics server before entering the executor loop.
            # Mirrors the same pattern as binance_executor.main(); the supervisor
            # bypasses main() so we must start the HTTP server here.
            try:
                from prometheus_client import start_http_server as _start_http
                _metrics_port = int(os.getenv('BINANCE_EXECUTOR_METRICS_PORT', '9876'))
                _start_http(_metrics_port)
                print(f'📊 metrics server starting at port={_metrics_port}')
            except Exception as _e:
                print(f'⚠️  metrics server not started: {_e}')
