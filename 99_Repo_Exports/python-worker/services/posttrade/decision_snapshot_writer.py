...
def _normalize_row(evt: dict[str, Any]) -> dict[str, Any]:
    ...
    # P1: accept explicit EMIT aliases written by SignalPipeline.
    decision_mid = _to_float(evt.get("decision_mid") if evt.get("decision_mid") is not None else evt.get("decision_mid_at_emit"))
    decision_exp_slip = _to_float(
        evt.get("decision_expected_slippage_bps")
        if evt.get("decision_expected_slippage_bps") is not None
    ...
    row: dict[str, Any] = {
        ...
        "decision_expected_slippage_bps": decision_exp_slip,
        "decision_exec_risk_norm": _to_float(evt.get("decision_exec_risk_norm")),
        "decision_price": _to_float(evt.get("decision_price") or evt.get("decision_mid") or evt.get("decision_mid_at_emit")),
...
class DecisionSnapshotStreamWorker:
    ...
    async def run_once(self) -> int:
        ...
        for _, entries in resp:
            for entry_id, fields in entries:
                total += 1
                all_entries.append((_decode_id(entry_id), fields))
        ...
        for _ in range(max(1, self.cfg.pel_max_iters)):
            ...
            try:
                ...
                if callable(xautoclaim):
                    ...
                    rows, ack_bad = await self._process_entries([(_decode_id(i), f) for (i, f) in msgs], allow_db=True)
                    if ack_bad:
                        await self._ack(*ack_bad)
            ...
            try:
                ...
                # XPENDING response can be a dict (redis-py >= 3.0) or tuple.
                ids = []
                for p in pending:
                    if isinstance(p, dict):
                        # With decode_responses=False, keys might be bytes or strings
                        if b"message_id" in p:
                            ids.append(_decode_id(p[b"message_id"]))
                        elif "message_id" in p:
                ...
                rows, ack_bad = await self._process_entries([(_decode_id(i), f) for (i, f) in claimed], allow_db=True)
                if ack_bad:
                    await self._ack(*ack_bad)
                ...
                cursor = _decode_id(ids[-1]) if ids else cursor
