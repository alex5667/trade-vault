...
def normalize_position_closed_event(
    ...
    Returns:
        Tuple: (dict[str, str] готов для redis.xadd(, maxlen=200000), list[str] ошибок).
    """
    d = dict(doc or {})
    errs: List[str] = []
    out: Dict[str, str] = {}

    def _get_fstr(*keys: str) -> str:
        for k in keys:
            v = d.get(k)
    ...
    price_str = _get_fstr("price", "px")
    if not price_str:
        errs.append("missing_price")
    out["price"] = price_str
    out["px"] = _get_fstr("px", "price")
    if not out["px"]:
        out["px"] = price_str

    # 8. pnl
    pnl_str = _get_fstr("pnl")
    if not pnl_str:
    ...
    fee_bps_str = _get_fstr("fee_bps", "fees_bps")
    if not fee_bps_str:
