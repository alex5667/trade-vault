...
class Pg:
    def __init__(self, dsn: str):
        ...
        try:
            ...
            # Create a ThreadedConnectionPool (min 1, max 20 connections)
            self._pool = psycopg2.pool.ThreadedConnectionPool(1, 20, self.dsn)
        except Exception as e:
            logger.error(f"tca_worker: Failed to initialize ThreadedConnectionPool: {e}")
            raise RuntimeError(f"Database pool initialization failed: {e}") from None
    ...
    def _put_conn(self, conn):
        try:
            # Ensure no transactions are left idle
            conn.rollback()
        except Exception:
            pass
...
async def main() -> None:
    ...
    while True:
        try:
            ...
            # Distinguish between connection errors and logic errors if possible
            if "OperationalError" in str(type(e)):
                logger.error(f"tca_worker: Database connection lost or refused. Retrying in 5s... Error: {e}")
                await asyncio.sleep(5.0)
            else:
                logger.exception("tca_worker: Unexpected loop error. Retrying in 2s...")
