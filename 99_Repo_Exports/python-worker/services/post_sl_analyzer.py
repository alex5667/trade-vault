    ...
    def _ensure_group(self, stream: str, group: str):
        ...
        for i in range(max_retries):
            try:
                ...
                wait_time = 2.0
                logger.warning(f"Redis connection error, retrying group {group} creation in {wait_time}s... ({i+1}/{max_retries}): {e}")
                time.sleep(wait_time)
                continue
    ...
    def start(self):
        ...
        while self.running:
            try:
                ...
                logger.warning(f"Redis connection error in main loop: {e}")
                time.sleep(1)
