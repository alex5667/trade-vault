...
class IdempotencyStore:
    ...
    def is_msg_done(self, msg_id: str) -> bool:
        if not msg_id:
            return False
        if self.redis is None:
            return False
        try:
            v = self.redis.get(KeyUtils.outbox_done_key(self.config.msg_done_prefix, msg_id))
            if v in (None, "", b""):
                return False
    ...
    def mark_msg_done(self, msg_id: str) -> None:
        try:
            if self.redis is None:
                return
            self.redis.set(KeyUtils.outbox_done_key(self.config.msg_done_prefix, msg_id), "1", ex=int(self.config.done_ttl_sec), nx=True)
        except Exception:
            return

    def is_outbox_done(self, msg_id: str) -> bool:
        if not msg_id:
            return False
        if self.redis is None:
            return False
        try:
            v = self.redis.get(KeyUtils.outbox_done_key(f"{self.config.done_prefix}:msg", msg_id))
            if v in (None, "", b""):
                v2 = self.redis.get(KeyUtils.done_key(self.config.done_prefix, msg_id))
                if v2 in (None, "", b""):
                    return False
    ...
    def mark_outbox_done(self, msg_id: str) -> None:
        try:
            self.redis.setex(KeyUtils.outbox_done_key(f"{self.config.done_prefix}:msg", msg_id), self.config.done_ttl_sec, "1")
            if os.getenv("SIGNAL_OUTBOX_WRITE_LEGACY_DONE", "1").lower() not in {"0", "false", "no"}:
                with contextlib.suppress(Exception):
                    self.redis.setex(KeyUtils.done_key(self.config.done_prefix, msg_id), self.config.done_ttl_sec, "1")
    ...
    def is_env_done(self, sid: str) -> bool:
        if not sid:
            return False
        if self.redis is None:
            return False
        try:
            v = self.redis.get(KeyUtils.env_done_key(self.config.env_done_prefix, sid))
            if v in (None, "", b""):
                v = self.redis.get(KeyUtils.done_key(self.config.done_prefix, sid))
            if v in (None, "", b""):
                return False
            return True
        except Exception:
    ...
    def marker_exists(self, client: Any, target: str, sid: str) -> bool:
        try:
            return bool(client.get(DeliveryHelpers.delivery_key(self.config.marker_prefix, target, sid)))
        except Exception:
            return False

    def xadd_idempotent_atomic(self, client: Any, *, target: str, sid: str, stream: str,
    ...
    def setex_idempotent_atomic(self, client: Any, *, target: str, sid: str, key: str,
        ...
        res = self.lua_scripts.execute(
            "setex_and_mark",
            keys=[marker, key],
            args=[str(self.config.delivery_marker_ttl_sec), str(int(ttl_sec)), value_json],
            client=client
        )
        if not res:
            return False
    ...
    def xadd_idempotent(self, client: Any, *, target: str, sid: str, stream: str, fields: dict[str, Any], maxlen: int) -> bool:
        fv: list[str] = []
    ...
    def setex_idempotent(self, client: Any, *, target: str, sid: str, key: str, value_json: str, ttl_sec: int) -> bool:
        res = self.lua_scripts.execute(
            "setex_then_mark",
    ...
    def notify_idempotent(self, client: Any, *, sid: str, payload: dict[str, Any]) -> bool:
        fv: list[str] = []
