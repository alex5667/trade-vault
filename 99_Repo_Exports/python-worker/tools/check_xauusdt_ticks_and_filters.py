...
def check_atr_value(r: redis.Redis) -> Dict[str, Any]:
    ...
    try:
        ...
        if atr_str:
            ...
            print(f"  ⚠️ ATR не найден в Redis")
            return {"exists": False}
    except Exception as e:
        print(f"  ❌ Ошибка: {e}")
        return {"exists": False, "error": str(e)}
...
def check_config(r: redis.Redis) -> Dict[str, Any]:
    ...
    try:
        ...
        if config:
            ...
            print(f"  ⚠️ Конфигурация не найдена (используются дефолты)")
            return {"exists": False}
    except Exception as e:
        print(f"  ❌ Ошибка: {e}")
        return {"exists": False, "error": str(e)}
...
def analyze_filters(config: Dict[str, Any], atr_value: Optional[float]) -> None:
    ...
    if atr_gate_audit_only.lower() in ("true", "1", "yes"):
        print(f"    ✅ ATR gate в режиме AUDIT (не блокирует, только логирует)")
    else:
        print(f"    ⚠️ ATR gate в режиме ENFORCE (блокирует сигналы)")
    ...
    if atr_value:
        ...
        if atr_bps_approx < max(atr_bps_min_static, fees_th_typical):
            ...
            print(f"       → ATR gate блокирует сигналы!")
    ...
    if cooldown_sec > 10:
        print(f"    ⚠️ Cooldown довольно большой, может буферизовать сигналы")
    ...
    if disable_confidence:
        print(f"    ✅ Confidence filter отключен")
    else:
...
def main():
    ...
    if ticks_result.get("length", 0) == 0:
        ...
        print(f"  2. Проверьте логи go-worker или tick-ingest сервиса")
    ...
    if not atr_gate_audit and atr_value:
        ...
        if atr_bps_approx < 15.0:  # Типичный порог
            ...
            print(f"     Решение: установить atr_gate_audit_only=1 для XAUUSDT")
            print(f"     Команда: redis-cli HSET {CONFIG_KEY} atr_gate_audit_only 1")
    
    if not config_result.get("exists"):
        print(f"  4. Рекомендуется настроить конфигурацию для XAUUSDT")
