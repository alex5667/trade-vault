    ...
    if not os.path.exists(args.dataset_parquet):
        ...
        try:
            ...
            if missing_features and "indicators" in df.columns:
                print(f"DEBUG: Unpacking {len(missing_features)} features from nested 'indicators' dictionary...")
                try:
                    # Dynamically load the correct builder from core
                    import importlib
                    schema_mod_name = schema_name.replace("feat", "features")
                    schema_mod = importlib.import_module(f"core.{schema_mod_name}")
                    builder_func = getattr(schema_mod, f"build_{schema_mod_name}", None)
                    
                    if builder_func:
                        from tools.train_meta_model_lr_v4 import build_row_features
                        
                        # Apply row features builder to reconstruct features (like lag1_pnl_15m)
                        # We only evaluate missingness on the last 100 rows to speed things up
                        sample_df = df.tail(100)
                        def _build_feat(row: pd.Series) -> pd.Series:
                            row_dict = dict(row)
                            if "indicators" in row_dict and isinstance(row_dict["indicators"], dict):
                                row_dict.update(row_dict["indicators"])
                            return pd.Series(build_row_features(row_dict, builder_func))
                            
                        df_ind = sample_df.apply(_build_feat, axis=1)
                        # Re-evaluate missingness on this sample
                        sample_joined = sample_df.join(df_ind, rsuffix="_ind")
                        
                        valid_features = [f for f in features if f in sample_joined.columns]
                        missing_features = [f for f in features if f not in sample_joined.columns]
                        print(f"INFO: After unpacking and building features, found {len(valid_features)}/{len(features)} features.")
                        
                        # We update df to sample_joined so that missingness checks calculate over it
                        df = sample_joined
                    else:
                        print(f"WARNING: Could not find builder for {schema_name}")
