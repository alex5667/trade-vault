...
def check_redis_config(redis_url: str, champion_key: str) -> tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
    ...
    try:
        ...
        if not cfg or not isinstance(cfg, dict):
            print(f"❌ Конфигурация не является валидным JSON объектом")
            return False, None, None
        ...
        if not model_path:
            print(f"⚠️  Поле 'model_path' отсутствует или пустое")
        else:
...
def check_model_file(model_path: str) -> tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if not readable:
            print(f"   ⚠️  Файл не доступен для чтения!")
            return False, model_path
...
def check_joblib() -> tuple[bool, Optional[str]]:
...
def try_load_model(model_path: str) -> tuple[bool, Optional[Any], Optional[str]]:
    ...
    try:
        ...
        if has_predict_util and has_predict_unc:
            print(f"✅ Модель соответствует формату UtilMHModelV1")
        else:
            print(f"⚠️  Модель не имеет требуемых методов для util_mh")
...
def check_logs_for_errors(redis_url: str, metrics_stream: str = "metrics:ml_confirm", limit: int = 100) -> None:
    ...
    try:
        ...
        if no_model_count > 0:
            ...
            print(f"   Это подтверждает проблему загрузки модели")
