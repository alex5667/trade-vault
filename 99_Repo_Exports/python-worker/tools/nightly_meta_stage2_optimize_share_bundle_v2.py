...
Nightly tool for Stage2 share optimization per cell (SYMBOL|bucket) - v2 with multi-objective optimization.

"Ещё выше-6": multi-objective per-cell optimizer + отдельные grid trend/range + лимит шага + turnover-proxy

Flow:
  1. Reads meta:unfreeze:cells (Stage1 progress)
  2. Waits META_STAGE1_EVAL_DELAY_HOURS
  3. For each eligible cell (stage==1, delay passed):
     - Exports trades history (META_OPT_EXPORT_HOURS)
     - Optimizes share per cell using multi-objective function:
       * meanR (opportunity mean return)
       * tail (executed tail rate penalty)
       * p05 (5th percentile penalty)
       * turnover proxy (exec_rate drop penalty)
       * step regularization (penalty for large changes)
     - Uses separate grids for trend vs range buckets
     - Applies step limits (max_up_step, max_down_step)
     - Enforces turnover constraints (min_exec_rate, max_exec_rate_drop)
...
Usage:
  python -m tools.nightly_meta_stage2_optimize_share_bundle_v2
...
logger = setup_logger("NightlyMetaStage2OptimizeShareV2")
...
def pick_best_share_multiobjective(
    ...
    *,
    ...
    """
    Constraints:
      share in [cur_share-max_down_step, cur_share+max_up_step]
      exec_tail <= tail_exec_cap
      exec_rate >= min_exec_rate
      exec_rate_drop <= max_exec_rate_drop (relative to current share sim)

    Objective:
      obj = opp_meanR
            - lam_tail * exec_tail
            - lam_p05 * max(0, -opp_p05)
            - lam_turn * max(0, (exec_rate_ref - exec_rate))   # penalize turnover drop
    ...
    # Reference exec_rate at current share (turnover proxy)
    ref = simulate_share(rows, share=cur_share, salt=salt)
    exec_rate_ref = float(ref["exec_rate"])

    best_s = cur_share
    best_rep = {"ref": ref, "best": None, "fallback": "no_change"}
    best_obj = -1e18
    ...
    for s in grid:
        ...
        rep = simulate_share(rows, share=s, salt=salt)
        used = int(rep["used"])
        if used < 200:
            continue
        ...
        obj = (
            ...
            - lam_turn * drop
            - lam_step * abs(s - cur_share)
        )
        rep["objective"] = obj
        rep["turnover_drop"] = drop
        rep["exec_rate_ref"] = exec_rate_ref

        if obj > best_obj:
            best_obj = obj
            best_s = s
            best_rep = {"ref": ref, "best": rep}

    if best_rep.get("best") is None:
        # keep current, but attach ref only
        return cur_share, best_rep
...
def main() -> None:
    ...
    for ck, raw in unf_map.items():
        ...
        if bucket not in ("trend", "range"):
            continue
        candidates.append((sym, bucket, cell, rec))
    ...
    run_dir = f"{out_dir}/meta_opt_stage2_v2_{ts}"
    ...
    if total > 0 and (missing_key / total) > 0.30:
        notify(r, f"<b>Stage2 optimize v2 skipped</b>\nreason=<code>missing_meta_enforce_key</code>\nmissing={missing_key}/{total}")
        logger.warning("Stage2 optimize v2 skipped: missing_meta_enforce_key %d/%d", missing_key, total)
        return
    if total > 0 and (missing_veto / total) > 0.30:
        notify(r, f"<b>Stage2 optimize v2 skipped</b>\nreason=<code>missing_meta_veto</code>\nmissing={missing_veto}/{total}")
        logger.warning("Stage2 optimize v2 skipped: missing_meta_veto %d/%d", missing_veto, total)
        return
    ...
    for sym, bucket, cell, rec in candidates[:max_cells]:
        # rows for this cell
        cell_rows = [x for x in rows if x["symbol"] == sym and x["bucket"] == bucket]
        if len(cell_rows) < 400:
            logger.info("Cell %s has insufficient trades: %d < 400", cell, len(cell_rows))
            continue
        ...
        # If no change -> skip this cell
        if abs(best_s - cur_share) < 1e-9:
            logger.info("Cell %s: no change (best_share=%.2f == cur_share=%.2f)", cell, best_s, cur_share)
            continue

        # Apply
        ops.append({"op": "HSET", "key": hk, "field": "meta_model_enable", "value": "1"})
        ops.append({"op": "HSET", "key": hk, "field": "meta_model_mode", "value": "ENFORCE"})
        ops.append({"op": "HSET", "key": hk, "field": share_field, "value": f"{best_s:.2f}"})
    ...
    bundle = {
        ...
        "created_ms": now_ms(),
        "ttl_sec": ttl,
        "who": "nightly_meta_stage2_optimize_share_bundle_v2",
        "ops": ops,
        "meta": {
    ...
    notify(
        r,
        "<b>Stage2 UNFREEZE proposal (per-cell optimized share v2)</b>\n"
        f"id=<code>{bundle_id}</code>\n"
        f"cells=<code>{picked_cells}</code>\n"
        f"step_cap=<code>+{max_up_step:.2f}</code> turnover_drop_max=<code>{max_exec_rate_drop:.2f}</code>\n"
        f"trend_grid=<code>{grid_trend}</code>\nrange_grid=<code>{grid_range}</code>",
        buttons=buttons,
    )
