...
Each cell is SYM|trend or SYM|range, and outcome stats are computed per bucket.

Key features:
- Outcome-gate per bucket: trend and range are evaluated separately (short=2h, long=24h)
- REMOVE (RESTORE) is forbidden if long-outcome is bad in range (even if trend is good)
  → means we restore trend, but range stays clamped/relaxed, clamp not fully removed
- Relax caps per bucket: trend≤0.30, range≤0.10 (configurable)
- AUTO mode by default, but can switch to PROPOSE (Approve/Reject) and separately forbid REMOVE runtime
- Per-cell state tracking: CLAMPED/RELAXED/RESTORED with remaining cells set
- Selective RESTORE: only cells where long-outcome OK for their bucket are restored

Stage A (RELAX): when all windows healthy + short-outcome OK (2h) for bucket and streak >= RELAX_N cycles
  → partially restore caps (trend≤0.30, range≤0.10) not higher than pre-clamp for eligible cells only.

Stage B (RESTORE): when all windows healthy + long-outcome OK (24h) for bucket and streak >= REMOVE_N cycles
...
Usage:
  python -m tools.of_gate_hardstop_cap_unclamp_v5
...
def _bucket_thr(env_key: str, default: float) -> float:
...
def _apply_restores_direct(
    ...
    *,
    ...
    bundle = {"id": bundle_id, "created_ms": ts, "ttl_sec": ttl_sec, "who": who, "ops": ops_out, "meta": {"kind": "meta_unclamp_v5_step"}}
...
def main() -> None:
    ...
    # Pending lifecycle (PROPOSE): if bundle applied/rejected -> update state then exit
    ...
    if pending_raw:
        ...
        if isinstance(pend, dict) and pend.get("bundle_id"):
            ...
            if st == "APPLIED":
                ...
                # clear clamp fully if no remaining cells
                if r.scard(remaining_cells_key) == 0:
    ...
    st_short = read_outcome_stats_sym_bucket(
        r, stream=trades_stream, since_ms=now_ms() - int(out_short_h * 3600_000), symbols=syms, max_scan=out_max_scan
    )
    st_long = read_outcome_stats_sym_bucket(
        r, stream=trades_stream, since_ms=now_ms() - int(out_long_h * 3600_000), symbols=syms, max_scan=out_max_scan
    )

    # thresholds per bucket (short -> RELAX eligibility)
    s_min_n_tr = int(_bucket_thr("META_OUT_S_MIN_N_TREND", 20))
    s_mean_tr = float(_bucket_thr("META_OUT_S_MEAN_MIN_TREND", -0.03))
    s_tail_tr = float(_bucket_thr("META_OUT_S_TAIL_MAX_TREND", 0.35))

    s_min_n_rg = int(_bucket_thr("META_OUT_S_MIN_N_RANGE", 20))
    s_mean_rg = float(_bucket_thr("META_OUT_S_MEAN_MIN_RANGE", -0.03))
    s_tail_rg = float(_bucket_thr("META_OUT_S_TAIL_MAX_RANGE", 0.35))

    # thresholds per bucket (long -> RESTORE eligibility)
    l_min_n_tr = int(_bucket_thr("META_OUT_L_MIN_N_TREND", 80))
    l_mean_tr = float(_bucket_thr("META_OUT_L_MEAN_MIN_TREND", -0.02))
    l_tail_tr = float(_bucket_thr("META_OUT_L_TAIL_MAX_TREND", 0.30))

    l_min_n_rg = int(_bucket_thr("META_OUT_L_MIN_N_RANGE", 80))
    l_mean_rg = float(_bucket_thr("META_OUT_L_MEAN_MIN_RANGE", -0.02))
    l_tail_rg = float(_bucket_thr("META_OUT_L_TAIL_MAX_RANGE", 0.30))
    ...
    for sym in syms:
        ...
        for bucket in ("trend", "range"):
            ...
            if bucket == "trend":
                ok_s, _ = outcome_ok(ss, min_n=s_min_n_tr, mean_min=s_mean_tr, tail_max=s_tail_tr)
                ok_l, _ = outcome_ok(ll, min_n=l_min_n_tr, mean_min=l_mean_tr, tail_max=l_tail_tr)
            else:
                ok_s, _ = outcome_ok(ss, min_n=s_min_n_rg, mean_min=s_mean_rg, tail_max=s_tail_rg)
                ok_l, _ = outcome_ok(ll, min_n=l_min_n_rg, mean_min=l_mean_rg, tail_max=l_tail_rg)
    ...
    # REMOVE/RESTORE: require health_ok + allow_remove + at least 1 restore cell
    ...
    # relax caps per bucket (requested)
    ...
    # Decision:
    # - If stage CLAMPED: RELAX eligible cells when relax_streak >= relax_n
    # - If stage RELAXED: RESTORE eligible cells when remove_streak >= remove_n
    ...
    if stage == "CLAMPED" and relax_streak >= relax_n and relax_cells:
        action = "RELAX"
        cells_to_act = sorted(list(set(relax_cells)))
    elif stage == "RELAXED" and remove_streak >= remove_n and allow_remove and restore_cells:
        action = "RESTORE"
        cells_to_act = sorted(list(set(restore_cells)))
    else:
        return
    ...
    if action == "RELAX":
        ...
        who = "of_gate_hardstop_cap_unclamp_v5_restore"
        meta = {
            "kind": "meta_unclamp_v5_restore",
            "clamp_id": clamp_bundle_id,
            "cells": cells_to_act,
            "health": {"30m": h30, "2h": h120, "12h": h720, "bad": health_bad},
            "outcome": out_dbg,
    ...
    if mode == "AUTO":
        ...
        _notify(
            r,
            "<b>Unclamp AUTO applied (v5 bucket-outcome)</b>\n"
            ...
            f"caps=<code>{{'trend':{cap_relax_trend},'range':{cap_relax_range}}}</code>\n"
            f"out_windows=<code>{{'short_h':{out_short_h},'long_h':{out_long_h}}}</code>",
            buttons=buttons,
        )
        return
    ...
    _notify(
        r,
        "<b>Unclamp PROPOSAL (v5 bucket-outcome)</b>\n"
        f"action=<code>{action}</code> id=<code>{bid}</code>\n"
        f"cells=<code>{cells_to_act}</code>\n"
