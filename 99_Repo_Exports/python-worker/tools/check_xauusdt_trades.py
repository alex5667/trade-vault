...
def read_recent_entries(r: redis.Redis, stream_name: str, count: int = 100, filter_symbol: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        for oid in open_ids:
            try:
                ...
                source = str(order_data.get("source") or "").strip()
...
def main():
    ...
    if signals_result.get("found", 0) > 0 and open_result.get("xauusdt_open", 0) == 0:
        ...
        if "avg_confidence" in signals_result:
            ...
            if conf < 0.70:
                print(f"      ⚠️  Confidence слишком низкая! Нужно >= 70%")
