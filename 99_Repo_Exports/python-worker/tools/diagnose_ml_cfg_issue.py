...
def main():
    ...
    try:
        ...
        if cfg.get('kind', '').lower().startswith('util_mh'):
            ...
            if not has_util or not has_unc:
                print(f"   ❌ Model missing required methods for util_mh")
                return 1
    except ImportError:
        print(f"   ⚠️  joblib not available")
    except Exception as e:
    ...
    try:
        ...
        if gate._cfg:
            print(f"   ✅ Config loaded via gate")
            print(f"      Source: {gate._cfg_source}")
            print(f"      Kind: {gate._cfg.get('kind')}")
        else:
            print(f"   ❌ Config NOT loaded via gate")
            print(f"      Source: {gate._cfg_source}")
        ...
        if gate._model:
            print(f"   ✅ Model loaded via gate")
        else:
            print(f"   ⚠️  Model NOT loaded via gate")
        ...
        if dec.status == "ERR_NO_CFG":
            print(f"   ❌ Still getting ERR_NO_CFG!")
            return 1
        else:
            print(f"   ✅ No ERR_NO_CFG error")
