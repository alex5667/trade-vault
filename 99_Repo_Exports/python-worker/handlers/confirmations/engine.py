...
def _finalize_decision(*, veto: bool, veto_reason: str, veto_reason_code: str, soft_code: str = "") -> tuple[str, int, str, int]:
    ...
    du16 = reason_u16(dc)  # тот же реестр, что и veto
...
class ConfirmationsEngine:
    ...
    def validate(
        ...
        # --- absorption ---
        if k == "absorption":
            # внедрение: строгие 2-из-N подтверждений для absorption теперь в AbsorptionValidator
            require_2ofn = False
            if ff is not None:
                require_2ofn = bool(getattr(ff, "absorption_require_2ofn_confirmations", False))
            ...
            if hasattr(r, "flags"):
                if isinstance(r.flags, dict):
                    parts.update(r.flags)
                elif isinstance(r.flags, list):
                    flags.extend(r.flags)
            if hasattr(r, "parts") and isinstance(r.parts, dict):
        ...
        conf = safe_float(soft_conf, 0.0) or 0.0
