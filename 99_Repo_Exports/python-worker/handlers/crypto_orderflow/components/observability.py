...
def emit_veto_metric_dual(*, emit_veto_metric, kind: str, ctx: Any, reason_code: str) -> None:
    ...
    if (os.getenv("EDGE_DUAL_EMIT_LEGACY_THIN_COST", "0") or "0").strip().lower() in {"1", "true", "yes", "on"}:
        try:
...
class CryptoObservability:
    ...
    def maybe_log_candidate(self, *, ctx: Any, cand: Any, parts: dict[str, Any], now_ms: int | None = None) -> None:
        """
    ...
    def emit_veto_metric(self, *, kind: str, ctx: Any, reason_code: str) -> None:
        ...
        try:
            ...
            m.inc("signals_veto_total", 1, tags={"reason": rc, "gate": (kind or "")})
        except Exception:
            return
