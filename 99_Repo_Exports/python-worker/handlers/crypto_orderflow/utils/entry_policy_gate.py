...
class EntryPolicyGate:
    ...
    def __init__(self) -> None:
        ...
        self.adverse_cross_soft_bps = _safe_float(os.getenv("ENTRY_ADVERSE_CROSS_SOFT_BPS", "0.5"), 0.5)
        self.adverse_cross_hard_bps = _safe_float(os.getenv("ENTRY_ADVERSE_CROSS_HARD_BPS", "1.5"), 1.5)
    ...
    def evaluate(self, *, ctx: Any, symbol: str, kind: str) -> GateDecision:
        ...
        btc_soft_hit = (
            (btc_stale_ms > 0 and btc_stale_ms >= self.book_stale_soft_ms)
            or (btc_cross_bps > 0 and btc_cross_bps >= self.adverse_cross_soft_bps)
        )
        if btc_soft_hit:
            soft_flags.append(
