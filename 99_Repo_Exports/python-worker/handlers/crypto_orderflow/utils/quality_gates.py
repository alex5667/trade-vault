...
try:
    ...
    emit_event_time_metrics = None  # type: ignore

try:
    from services.orderflow.book_trade_consistency_gate import BookTradeConsistencyGate
except Exception:  # pragma: no cover
    BookTradeConsistencyGate = None  # type: ignore

# Lazily initialized singleton for BookTradeConsistencyGate (fail-open if unavailable).
_book_trade_gate_instance: Any = None


def _get_book_trade_gate() -> Any:
...
class RegimeSessionLiquidityGate:
    ...
    def evaluate(self, *, ctx: Any, symbol: str, kind: str, side: str) -> QualityGateDecision:
        ...
        # 5-minute ATR calculation
        atr_5m_val = _safe_float(_get_metric(ctx, ("atr_5m", "atr"), allow_of=True))
        price_val = _safe_float(_get_metric(ctx, ("last_price", "price", "mid", "close"), allow_of=True))
        atr_5m_bps = None
        if atr_5m_val is not None and price_val is not None and price_val > 0:
            atr_5m_bps = (atr_5m_val / price_val) * 10000.0
        ...
        # 5-minute atr_bps bounds
        atr_5m_bps_min = _pick_float("QUALITY_5M_ATR_BPS_MIN", symbol=symbol, kind=kind, regime=regime_s, default=0.0)
...
class SignalConsistencyGate:
    ...
    def evaluate(self, *, ctx: Any, symbol: str, kind: str, side: str) -> QualityGateDecision:
        ...
        sym_z = _cached_getenv(f"{base}_DELTA_Z_THRESHOLD")
        sym_obi = _cached_getenv(f"{base}_OBI_THRESHOLD")
