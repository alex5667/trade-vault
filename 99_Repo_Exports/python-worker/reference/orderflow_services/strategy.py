...
from domain.evidence_keys import MetaKeys, MLKeys
...
OF_GATE_METRICS_QUARANTINE_STREAM = os.getenv("OF_GATE_METRICS_QUARANTINE_STREAM", "quarantined:metrics:of_gate")
...
class OrderFlowStrategy:
    def __init__(self, redis: aioredis.Redis, ticks: aioredis.Redis, publisher: AsyncSignalPublisher,
                 ...
                 notify_client: aioredis.Redis | None = None, notify_stream: str = "notify:telegram"):
        self.redis = redis
        self.ticks = ticks
    ...
    def _liqmap_snapshot_key(self, *, symbol: str, window: str) -> str:
        return f"{self.liqmap_snapshot_key_prefix}:{symbol.upper()}:{str(window)}"
    ...
    async def _maybe_add_liqmap_features(self, *, runtime, indicators: dict, mid_px: float, now_ms: int) -> None:
        ...
        # Per-symbol cache state
        cache = getattr(runtime, 'liqmap_snapshot_cache', None)
        if cache is None or not isinstance(cache, dict):
            cache = {}
            runtime.liqmap_snapshot_cache = cache

        primary_window = None
        for w in self.liqmap_feature_windows:
            if not primary_window:
                primary_window = w
            key = self._liqmap_snapshot_key(symbol=runtime.symbol, window=w)

            # cache entry: { 'fetch_ms': int, 'raw': bytes|str }
            ent = cache.get(w) if isinstance(cache, dict) else None
            fetch_ms = int(ent.get('fetch_ms', 0) or 0) if isinstance(ent, dict) else 0
            raw = ent.get('raw') if isinstance(ent, dict) else None

            if (now_ms - fetch_ms) >= int(self.liqmap_feature_cache_ms) or raw is None:
                try:
                    raw = await asyncio.wait_for(self.redis.get(key), timeout=float(self.liqmap_feature_redis_timeout_s))
                    cache[w] = {'fetch_ms': int(now_ms), 'raw': raw}
                except Exception:
                    # Keep old cache on timeout / redis error
                    raw = raw
            ...
            # Promote directional anchors from the primary window to generic keys for downstream logic
    ...
    async def process_tick(self, runtime: SymbolRuntime, tick: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            if price > 0:
                ...
                if sym:
                    ...
                    self._mbatch.put("set", f"cfg:last_px:{sym}", str(price), ex=ttl)
                    self._mbatch.put("set", f"cfg:last_px_ts_ms:{sym}", str(int(tick_ts)), ex=ttl)
        except Exception:
            pass
        ...
        try:
            ...
            try:
                ...
                if mid > 0:
                    ...
                    try:
                        liq_min_slope = float(os.getenv("LIQ_MIN_BOOK_SLOPE", "0") or 0)
                        liq_max_dws = float(os.getenv("LIQ_MAX_DWS_BPS", "0") or 0)
                        liq_max_rec = float(os.getenv("LIQ_MAX_RECOVERY_TIME_MS", "0") or 0)
                        flags = []
                        smin = min(float(slope_bid), float(slope_ask))
                        if liq_min_slope > 0 and smin > 0 and smin < liq_min_slope:
                            flags.append("slope_low")
                        if liq_max_dws > 0 and float(dws_bps) > liq_max_dws:
                            flags.append("dws_high")
                        rec_ms = float(indicators.get("liq_recovery_time_ms", 0) or 0)
                        if liq_max_rec > 0 and rec_ms > liq_max_rec:
                ...
                try:
                    ...
                    try:
                        import os as _os
                        manip_mode = str(_os.getenv("MANIP_MODE", "auto") or "auto").strip().lower()
                        gate_profile = str(_os.getenv("GATE_PROFILE", "default") or "default").strip().lower()
                        # In auto mode, strict → tighten; hard → tighten (veto is in pipeline)
                        should_tighten = (
                            (manip_mode == "tighten") or
                            (manip_mode == "auto" and gate_profile in ("strict", "hard"))
                        )
                        if should_tighten and indicators["manip_flags"]:
                            tighten_mult = float(_os.getenv("MANIP_TIGHTEN_ADD_MULT", "1.0") or 1.0)
                            tighten_cap = float(_os.getenv("MANIP_TIGHTEN_ADD_CAP_BPS", "6.0") or 6.0)
                            # Composite score drives tighten add
                            manip_score = max(
                                float(indicators.get("quote_stuffing_score", 0.0) or 0.0),
                                float(indicators.get("layering_score", 0.0) or 0.0),
                            )
                            if manip_score > 0.0:
                                add_bps = float(min(tighten_cap, manip_score * tighten_mult * 3.0))  # 3 bps/score unit
            ...
            if os.getenv("ATR_SANITY_ENABLE", "1") == "1":
                ...
                try:
                    if int(res.bad) == 1:
                        ...
                        self._mbatch.put("expire", "cfg:atr_bad:symbols", int(os.getenv("ATR_BAD_SYMBOLS_SET_TTL_SEC", "86400")))
                        self._mbatch.put("hincrby", f"metrics:atr_bad_total:{runtime.symbol}", reason, 1)
                        self._mbatch.put("expire", f"metrics:atr_bad_total:{runtime.symbol}", int(os.getenv("METRICS_COUNTER_TTL_SEC", "604800")))
                    ...
                    if int(getattr(res, "jump_event", 0) or 0) == 1:
                        ...
                        self._mbatch.put("expire", "cfg:atr_jump:symbols", int(os.getenv("ATR_JUMP_SYMBOLS_SET_TTL_SEC", "86400")))
                        self._mbatch.put("incr", f"metrics:atr_jump_total:{runtime.symbol}")
                        self._mbatch.put("expire", f"metrics:atr_jump_total:{runtime.symbol}", int(os.getenv("METRICS_COUNTER_TTL_SEC", "604800")))
                except Exception:
                    pass
            else:
        ...
        try:
            if int(indicators.get("cvd_quarantine_active", 0) or 0) == 1:
                ...
                self._mbatch.put("expire", "cfg:cvd_quarantine:symbols", int(os.getenv("CVD_QUAR_SYMBOLS_SET_TTL_SEC", "86400")))
                self._mbatch.put("incr", f"metrics:cvd_quarantine_activations_total:{runtime.symbol}")
                self._mbatch.put("expire", f"metrics:cvd_quarantine_activations_total:{runtime.symbol}", int(os.getenv("METRICS_COUNTER_TTL_SEC", "604800")))
        except Exception:
            pass
            ...
            if ofc:
                ...
                try:
                    if isinstance(ev, dict):
                        _cov_bucket = (ev.get(MetaKeys.ENFORCE_COV_BUCKET) or "")
                        if _cov_bucket:
                            indicators[MetaKeys.ENFORCE_COV_BUCKET] = _cov_bucket
                ...
                try:
                    if OF_GATE_METRICS_ENABLE:
                        ...
                        if rate <= 0:
                            ...
                            ml = ev.get(MLKeys.DECISION, {}) if isinstance(ev.get(MLKeys.DECISION), dict) else {}
                            ...
                            payload = {
                                ...
                                "meta_mode": (ev.get(MetaKeys.MODE, "") or ""),
                                "meta_enable": str(int(ev.get(MetaKeys.ENABLE, 0) or 0)),
                                "meta_reason": (ev.get(MetaKeys.REASON, "") or "")[:80],
                                "meta_schema_name": (ev.get(MetaKeys.SCHEMA_NAME, "") or ""),
                                "meta_schema_version": str(int(ev.get(MetaKeys.SCHEMA_VERSION, 0) or 0)),
                                "meta_model_schema_name": (ev.get(MetaKeys.MODEL_SCHEMA_NAME, "") or ""),
                                "meta_model_schema_version": str(int(ev.get(MetaKeys.MODEL_SCHEMA_VERSION, 0) or 0)),
                                "meta_feature_coverage": str(float(ev.get(MetaKeys.FEATURE_COVERAGE, 1.0) or 1.0)),
                                "meta_feature_missing_rate": str(float(ev.get(MetaKeys.FEATURE_MISSING_RATE, 0.0) or 0.0)),
                                "meta_model_feature_total": str(int(ev.get(MetaKeys.MODEL_FEATURE_TOTAL, 0) or 0)),
                                "meta_model_feature_missing": str(int(ev.get(MetaKeys.MODEL_FEATURE_MISSING, 0) or 0)),
                                "meta_enforce_cov_bucket": (ev.get(MetaKeys.ENFORCE_COV_BUCKET, "") or ""),
                                "meta_enforce_bucket_type": (ev.get("meta_enforce_bucket_type", "") or ""),
...
#                             )
                ...
                if isinstance(oe, dict):
                    ...
                    if ots > 0 and 0 <= (now_ms - ots) <= ttl:
                        ...
                        if int(indicators["ofi_stable"] or 0) == 1:
                            if (oe.get("direction", "") or "").upper() == str(direction).upper():
        ...
        payload = {
            ...
            # P4 latency contract: marks feature/gate-computation completion in this process.
            # SignalPipeline will overwrite ts_emit_ms at the true publish boundary.
            "ts_feature_ms": int(tick_ts),
...
#                 "cvd": float(bar.cvd_close)
    ...
    def _update_liquidity_regime(self, runtime: SymbolRuntime, tick_ts: int, indicators: dict):
        ...
        try:
            ...
            indicators.update({
                "liq_score": float(liq.liq_score),
                "liq_regime": str(liq.liq_regime),
                "liq_depth_usd_min_5": float(liq.depth_usd_min_5),
                "liq_spread_bps": float(liq.spread_bps),
                "liq_book_rate_hz": float(liq.book_rate_ema_hz),
                "liq_book_stale_ms": int(liq.book_stale_ms)
            })
            if liq.why:
                indicators["liq_why"] = str(liq.why)
        except Exception as exc:
