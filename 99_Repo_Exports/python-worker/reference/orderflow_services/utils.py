...
import json
# Fast JSON: orjson is 2-10x faster than stdlib + releases GIL during parse.
# Falls back to stdlib json if not installed (e.g. in legacy test envs).
try:
    import orjson as _orjson  # type: ignore[import]
    def _orjson_loads(s: Any) -> Any:
        """orjson.loads — accepts bytes, bytearray, memoryview, str."""
        if isinstance(s, str):
            s = s.encode("utf-8")  # orjson prefers bytes, str also works in >=3.10
        return _orjson.loads(s)
except ImportError:
    _orjson = None  # type: ignore[assignment]
    def _orjson_loads(s: Any) -> Any:  # type: ignore[misc]
...
def _parse_tick_payload(
    ...
    try:
        ...
        if isinstance(payload, str):
            merged = _orjson_loads(payload)   # fast path: orjson or json fallback
    ...
    if "data" in merged:
        try:
            nested = _orjson_loads(merged["data"]) if isinstance(merged["data"], str) else merged["data"]
...
def _parse_book_payload(payload: Dict[str, Any], symbol: str) -> Dict[str, Any]:
    if "data" in payload:
        try:
            nested = _orjson_loads(payload["data"])
