...
from domain.evidence_keys import CtxKeys, MetaKeys
...
class TickProcessor:
    ...
    async def _inject_liqmap_features(
        ...
        *,
        ...
        try:
            ...
            # NOTE: loop per-window; features are namespaced by liqmap_{w}_* keys.
            for w in windows:
                wnd = str(w)
                ck = (sym, wnd)
                ...
                if cached and refresh_ms > 0 and now_ms < int(next_ts):
                ...
                try:
                    ...
                    raw = await self.redis.get(f"{prefix}:{sym}:{wnd}")
                except Exception:
                    try:
                        silent_errors_total.labels(kind="liqmap", symbol=sym, where="redis_get").inc()
                    except Exception:
                        pass
                    raw = None
                ...
                if raw is None:
                    ...
                    if isinstance(cached, dict):
                        try:
                            ...
                            if good_ms > 0 and stale_ms > 0 and (now_ms - good_ms) <= int(stale_ms):
    ...
    async def _inject_liqmap_features(
        ...
        *,
        ...
        for w in windows:
            ...
            self._liqmap_next_refresh_ts_ms[cache_key] = now_ms + max(250, int(refresh_ms))
            ...
            if not raw:
                ...
                cached = self._liqmap_cache.get(cache_key) or {}
                good_ms = int(cached.get("good_ms", 0) or 0)
                if good_ms and (now_ms - good_ms) <= int(failopen_ms):
                    feats = cached.get("feats")
                    if isinstance(feats, dict) and feats:
                        indicators.update(feats)
                    else:
            ...
            try:
                ...
                age_ms = (now_ms - int(snap_ts_ms)) if snap_ts_ms > 0 else -2
                try:
    ...
    async def process_tick(self, runtime: SymbolRuntime, tick: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            try:
                _bkt = (_rdind.get("exec_regime_bucket", "NORMAL") or "NORMAL")
            except Exception:
                _bkt = "NORMAL"
        ...
        try:
            ...
            if q > 0:
                ...
                runtime._a5_trade_qty_ema = float(new_ema)
                runtime._a5_trade_qty_ts_ms = int(new_ts)
                if bad_time:
                    runtime._a5_bad_time_total = int(getattr(runtime, "_a5_bad_time_total", 0) or 0) + 1
            ...
            if bs is not None:
                ...
                if book_ts > 0 and book_ts != prev_book_ts:
                    ...
                    runtime._a5_depth_total10_last = float(depth10)
                    ...
                    runtime._a5_depth_total10_ema = float(new_ema)
                    runtime._a5_depth_ts_ms = int(new_ts)
                    runtime._a5_depth_book_ts_ms = int(book_ts)
                    if bad_time:
                        runtime._a5_bad_time_total = int(getattr(runtime, "_a5_bad_time_total", 0) or 0) + 1
        except Exception:
                    ...
                    if cache_ts != book_ts:
                        ...
                        try:
                            ...
                            if prev_book_ts_ms is None:
                                ...
                                runtime._ms_v4_prev_book_ts_ms = int(book_ts)
                                runtime._ms_v4_prev_depth_imbalance_10 = float(cur_imb10)
                            else:
                                if bad_dt:
                                    ...
                                    runtime._ms_v4_book_deriv_bad_time_total = int(getattr(runtime, "_ms_v4_book_deriv_bad_time_total", 0) or 0) + 1
                                else:
                            ...
                            try:
                                ...
                                pass
                        runtime._ms_v4_cache_book_ts = book_ts
                        runtime._ms_v4_prev_raw = bs.raw
                        runtime._ms_v4_cache = ms
        ...
        try:
            ...
            if ev_ofi:
                ...
                if isinstance(ev_ofi, dict):
                    ...
                    dir0 = (ev_ofi.get("direction", "") or "").upper()
        ...
        try:
            ...
            if not vrl:
                try:
                    ...
                    vrl = "na"
            indicators["vol_regime_label"] = (vrl or "na")
        except Exception:
        ...
        try:
            b = compute_exec_regime_bucket(
                liq_regime_label=(indicators.get("liq_regime_label", "na") or "na"),
                vol_regime_label=(indicators.get("vol_regime_label", "na") or "na"),
            )
            _bucket = str(b.bucket)
            indicators["exec_regime_bucket"] = _bucket
        except Exception:
            indicators.setdefault("exec_regime_bucket", "NORMAL")
            _bucket = (indicators.get("exec_regime_bucket", "NORMAL"))
        ...
        try:
            ...
            try:
                ...
                trade_impact_proxy.labels(
                    sym=str(runtime.symbol), bucket=(indicators.get("exec_regime_bucket", "NORMAL"))
                ).observe(float(_ip))
            ...
            try:
                ...
                _bk   = (indicators.get("exec_regime_bucket", "NORMAL"))
                _rsn  = (indicators.get("taker_flow_gate_reason", "") or "")[:40]
        ...
        try:
            ...
            _per_bucket_map = {}
            try:
                _per_bucket_map = dict(runtime.dynamic_cfg.get("slippage_decomp_impact_coeff_bps") or {})
            except Exception:
                pass
            _bucket_now = (indicators.get("exec_regime_bucket", "NORMAL"))
            ...
            if do_enforce:
                buck = (indicators.get("exec_regime_bucket", "NORMAL") or "NORMAL").strip().upper()
                raw = (cfg.get("slippage_decomp_enforce_buckets", "HIGH_VOL_LOW_LIQ") or "HIGH_VOL_LOW_LIQ")
        ...
        # ---------------------------------------------------------------
        # v11_of Group A: Regime / Structural Context
        # Source: bar_processor → runtime attrs (fail-open 0.0/0.5)
        # ---------------------------------------------------------------
        try:
            indicators["hurst_exp_50"] = float(getattr(runtime, "hurst_exp_50", 0.5) or 0.5)
        except Exception:
            indicators.setdefault("hurst_exp_50", 0.5)
        try:
            indicators["vol_regime_code"] = float(getattr(runtime, "vol_regime_code", 1) or 1)
        except Exception:
            indicators.setdefault("vol_regime_code", 1.0)
        try:
            indicators["tick_autocorr_lag1"] = float(
                getattr(runtime, "tick_sign_autocorr_lag1", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("tick_autocorr_lag1", 0.0)
        try:
            indicators["kyle_lambda"] = float(getattr(runtime, "kyle_lambda", 0.0) or 0.0)
        except Exception:
            indicators.setdefault("kyle_lambda", 0.0)
        try:
            # Roll's spread estimator: 2 * sqrt(max(-Cov(ΔP_t, ΔP_{t-1}), 0))
            _scov = float(getattr(runtime, "spread_return_cov", 0.0) or 0.0)
            indicators["roll_spread_est"] = 2.0 * (_scov ** 0.5) if _scov > 0 else 0.0
        except Exception:
            indicators.setdefault("roll_spread_est", 0.0)

        # ---------------------------------------------------------------
        # v11_of Group B: Trade History / Session Context
        # Source: trade ring-buffer → runtime rolling stats (fail-open 0.0)
        # ---------------------------------------------------------------
        try:
            indicators["kelly_fraction_roll"] = float(
                getattr(runtime, "kelly_fraction_roll", 0.0) or 0.0
            )
            indicators["profit_factor_roll20"] = float(
                getattr(runtime, "profit_factor_roll20", 0.0) or 0.0
            )
            indicators["expectancy_bps"] = float(
                getattr(runtime, "expectancy_bps_roll", 0.0) or 0.0
            )
            indicators["recovery_factor_roll"] = float(
                getattr(runtime, "recovery_factor_roll", 0.0) or 0.0
            )
            indicators["trade_freq_per_hr"] = float(
                getattr(runtime, "trade_freq_per_hr", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("kelly_fraction_roll", 0.0)
            indicators.setdefault("profit_factor_roll20", 0.0)
            indicators.setdefault("expectancy_bps", 0.0)
            indicators.setdefault("recovery_factor_roll", 0.0)
            indicators.setdefault("trade_freq_per_hr", 0.0)

        # ---------------------------------------------------------------
        # v11_of Group C: Cross-Asset / Correlation
        # Fail-open 0.0 until go-worker external feed is deployed.
        # ---------------------------------------------------------------
        try:
            indicators["market_breadth_score"] = float(
                getattr(runtime, "market_breadth_score", 0.0) or 0.0
            )
            indicators["crypto_fear_greed"] = float(
                getattr(runtime, "crypto_fear_greed_proxy", 0.0) or 0.0
            )
            indicators["alt_season_index"] = float(
                getattr(runtime, "alt_season_index", 0.0) or 0.0
            )
            indicators["cross_asset_vol_ratio"] = float(
                getattr(runtime, "cross_asset_vol_ratio", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("market_breadth_score", 0.0)
            indicators.setdefault("crypto_fear_greed", 0.0)
            indicators.setdefault("alt_season_index", 0.0)
            indicators.setdefault("cross_asset_vol_ratio", 0.0)

        # ---------------------------------------------------------------
        # v11_of Group D: Order Flow Microstructure Extensions
        # ---------------------------------------------------------------
        try:
            indicators["trade_size_skew"] = float(
                getattr(runtime, "trade_size_skew", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("trade_size_skew", 0.0)
        try:
            indicators["sweep_velocity_bps_s"] = float(
                getattr(runtime, "sweep_velocity_bps_s", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("sweep_velocity_bps_s", 0.0)
        try:
            indicators["book_refresh_rate_hz"] = float(
                getattr(runtime, "book_update_rate_hz", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("book_refresh_rate_hz", 0.0)
        try:
            _c_cancel = float(getattr(runtime, "book_cancel_count", 0.0) or 0.0)
            _c_fill   = float(getattr(runtime, "book_fill_count",   0.0) or 0.0)
            _c_denom  = _c_cancel + _c_fill
            indicators["cancel_to_fill_ratio"] = _c_cancel / _c_denom if _c_denom > 0 else 0.0
        except Exception:
            indicators.setdefault("cancel_to_fill_ratio", 0.0)
        try:
            indicators["depth_pull_ratio"] = float(
                getattr(runtime, "depth_pull_ratio", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("depth_pull_ratio", 0.0)

        # ---------------------------------------------------------------
        # v11_of Group E: Signal Self-Awareness
        # ---------------------------------------------------------------
        try:
            _conf_now = float(indicators.get("confidence", 0.0) or 0.0)
            _conf_ma  = float(getattr(runtime, "conf_ma_24h", 0.0) or 0.0)
            indicators["conf_ma_ratio"] = (
                _conf_now / _conf_ma if _conf_ma > 1e-9 else 1.0
            )
        except Exception:
            indicators.setdefault("conf_ma_ratio", 1.0)
        try:
            _cluster_n = int(getattr(runtime, "signal_cluster_60s_count", 0) or 0)
            indicators["signal_cluster_flag"] = 1.0 if _cluster_n >= 3 else 0.0
        except Exception:
            indicators.setdefault("signal_cluster_flag", 0.0)
        try:
            _gate_hard = float(getattr(runtime, "gate_hard_veto_count", 0.0) or 0.0)
            _gate_tot  = float(getattr(runtime, "gate_total_count", 1.0) or 1.0)
            indicators["gate_hardness_score"] = _gate_hard / max(_gate_tot, 1.0)
        except Exception:
            indicators.setdefault("gate_hardness_score", 0.0)
        try:
            indicators["model_calibration_err"] = float(
                getattr(runtime, "model_calibration_err_roll50", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("model_calibration_err", 0.0)

        # ---------------------------------------------------------------
        # v11_of Group F: Derived / Interaction Features (pure products)
        # Always available if source keys are present; no external deps.
        # ---------------------------------------------------------------
        try:
            indicators["kyle_x_vpin"] = (
                float(indicators.get("kyle_lambda", 0.0) or 0.0)
                * float(indicators.get("vpin_rolling", 0.0) or 0.0)
            )
            indicators["momentum_x_vol_ratio"] = (
                float(indicators.get("momentum_10s", 0.0) or 0.0)
                * float(indicators.get("vol_ratio", 0.0) or 0.0)
            )
            indicators["pressure_x_obi"] = (
                float(indicators.get("pressure", 0.0) or 0.0)
                * float(indicators.get("obi", 0.0) or 0.0)
            )
            indicators["liq_score_x_spread"] = (
                float(indicators.get("liq_score", 0.0) or 0.0)
                * float(indicators.get("spread_bps", 0.0) or 0.0)
            )
            indicators["confidence_x_of_score"] = (
                float(indicators.get("confidence", 0.0) or 0.0)
                * float(indicators.get("of_score_final", 0.0) or 0.0)
            )
        except Exception:
            indicators.setdefault("kyle_x_vpin", 0.0)
            indicators.setdefault("momentum_x_vol_ratio", 0.0)
            indicators.setdefault("pressure_x_obi", 0.0)
            indicators.setdefault("liq_score_x_spread", 0.0)
            indicators.setdefault("confidence_x_of_score", 0.0)

        # ---------------------------------------------------------------
        # v12_of Group MA: Microstructure / Trade-by-trade
        # Source: runtime trade ring-buffer counters (fail-open 0.0)
        # ---------------------------------------------------------------
        try:
            _tr_cnt = float(getattr(runtime, "trade_count_window", 0.0) or 0.0)
            _tr_sec = float(getattr(runtime, "trade_window_sec", 1.0) or 1.0)
            indicators["trade_arrival_rate_hz"] = _tr_cnt / max(_tr_sec, 1e-3)
        except Exception:
            indicators.setdefault("trade_arrival_rate_hz", 0.0)
        try:
            _lg_cnt = float(getattr(runtime, "large_trade_count_window", 0.0) or 0.0)
            _tr_cnt = float(getattr(runtime, "trade_count_window", 1.0) or 1.0)
            indicators["large_trade_ratio"] = _lg_cnt / max(_tr_cnt, 1.0)
        except Exception:
            indicators.setdefault("large_trade_ratio", 0.0)
        try:
            indicators["tick_direction_run"] = float(
                getattr(runtime, "tick_direction_run_max", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("tick_direction_run", 0.0)
        try:
            indicators["trade_size_entropy"] = float(
                getattr(runtime, "trade_size_entropy", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("trade_size_entropy", 0.0)

        # ---------------------------------------------------------------
        # v12_of Group MB: Order Book Dynamics (velocity, not snapshot)
        # Source: runtime short-window book counters + last_lob_event
        # ---------------------------------------------------------------
        try:
            _qs_cancel = float(getattr(runtime, "cancel_count_50ms", 0.0) or 0.0)
            _qs_quote  = float(getattr(runtime, "quote_count_50ms", 1.0) or 1.0)
            indicators["quote_stuffing_score"] = _qs_cancel / max(_qs_quote, 1.0)
        except Exception:
            indicators.setdefault("quote_stuffing_score", 0.0)
        try:
            indicators["depth_migration_bps"] = float(
                getattr(runtime, "depth_migration_bps_ema", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("depth_migration_bps", 0.0)
        try:
            # level2_wap_divergence: WAP(5-level L2) - mid_price in bps
            _lob_ev2 = getattr(runtime, "last_lob_event", None)
            _mid_px2 = float(getattr(runtime, "last_book_mid", 0.0) or 0.0) or float(price)
            if _lob_ev2 and isinstance(_lob_ev2, dict) and _mid_px2 > 0:
                _wap_l2 = float(_lob_ev2.get("wap_5lvl", 0.0) or 0.0)
                if _wap_l2 > 0 and _mid_px2 > 0:
                    indicators["level2_wap_divergence"] = (_wap_l2 - _mid_px2) / _mid_px2 * 10_000.0
                else:
                    indicators["level2_wap_divergence"] = 0.0
            else:
                indicators.setdefault("level2_wap_divergence", 0.0)
        except Exception:
            indicators.setdefault("level2_wap_divergence", 0.0)
        try:
            # bid_ask_queue_imbalance at best level: (q_bid - q_ask) / (q_bid + q_ask + eps)
            _lob_ev3 = getattr(runtime, "last_lob_event", None)
            if _lob_ev3 and isinstance(_lob_ev3, dict):
                _q_bid = float(_lob_ev3.get("best_bid_qty", 0.0) or 0.0)
                _q_ask = float(_lob_ev3.get("best_ask_qty", 0.0) or 0.0)
                _q_sum = _q_bid + _q_ask
                indicators["bid_ask_queue_imbalance"] = (
                    (_q_bid - _q_ask) / _q_sum if _q_sum > 1e-9 else 0.0
                )
            else:
                indicators.setdefault("bid_ask_queue_imbalance", 0.0)
        except Exception:
            indicators.setdefault("bid_ask_queue_imbalance", 0.0)

        # ---------------------------------------------------------------
        # v12_of Group MC: Temporal / Seasonality (deterministic from now_ts)
        # ---------------------------------------------------------------
        try:
            # minutes_to_funding: time until next perpetual 8h funding settlement
            # next_funding_ts_ms: runtime attr set by go-worker (fail-open: compute from UTC)
            _next_fund_ts = float(getattr(runtime, "next_funding_ts_ms", 0.0) or 0.0)
            if _next_fund_ts > 0:
                _mins_to_fund = (_next_fund_ts - float(now_ts)) / 60_000.0
            else:
                # Fallback: deterministic from UTC — funding at 00:00/08:00/16:00 UTC
                _now_sec_of_day = (int(now_ts) // 1000) % 86400
                _fund_interval = 8 * 3600  # 8h in seconds
                _next_fund_sec = (_now_sec_of_day // _fund_interval + 1) * _fund_interval
                _mins_to_fund = (_next_fund_sec - _now_sec_of_day) / 60.0
            indicators["minutes_to_funding"] = float(max(0.0, min(_mins_to_fund, 480.0)))
        except Exception:
            indicators.setdefault("minutes_to_funding", 0.0)
        try:
            # session_overlap_flag: 1.0 during NY∩London (12:00-13:00 UTC)
            # or Asia∩London (07:00-08:00 UTC) — high-volume overlap regimes
            _utc_hour = (int(now_ts) // 1000 // 3600) % 24
            _utc_min  = (int(now_ts) // 1000 // 60) % 60
            _utc_hm   = _utc_hour * 60 + _utc_min  # minutes since UTC midnight
            _overlap  = (
                (420 <= _utc_hm <= 479)   # 07:00–07:59 UTC: Asia∩London
                or (720 <= _utc_hm <= 779)  # 12:00–12:59 UTC: London∩NY
            )
            indicators["session_overlap_flag"] = 1.0 if _overlap else 0.0
        except Exception:
            indicators.setdefault("session_overlap_flag", 0.0)
        try:
            # time_since_last_liq_ms: recency of last liquidation event
            _last_liq_ts = float(getattr(runtime, "last_liquidation_event_ts_ms", 0.0) or 0.0)
            indicators["time_since_last_liq_ms"] = (
                float(now_ts) - _last_liq_ts if _last_liq_ts > 0 else 0.0
            )
        except Exception:
            indicators.setdefault("time_since_last_liq_ms", 0.0)

        # ---------------------------------------------------------------
        # v12_of Group MD: Cross-Asset / Macro extensions
        # Fail-open 0.0 until go-worker external feed is deployed.
        # ---------------------------------------------------------------
        try:
            indicators["eth_btc_corr_5m"] = float(
                getattr(runtime, "eth_btc_corr_5m", 0.0) or 0.0
            )
            indicators["perp_spot_basis_bps"] = float(
                getattr(runtime, "perp_spot_basis_bps", 0.0) or 0.0
            )
            indicators["stable_coin_flow_delta"] = float(
                getattr(runtime, "stable_coin_flow_delta", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("eth_btc_corr_5m", 0.0)
            indicators.setdefault("perp_spot_basis_bps", 0.0)
            indicators.setdefault("stable_coin_flow_delta", 0.0)

        # ---------------------------------------------------------------
        # v12_of Group ME: Self-Referential / Meta-Signal
        # ---------------------------------------------------------------
        try:
            indicators["signal_frequency_1h"] = float(
                getattr(runtime, "signal_count_1h", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("signal_frequency_1h", 0.0)
        try:
            indicators["last_trade_outcome_raw"] = float(
                getattr(runtime, "last_trade_pnl_bps", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("last_trade_outcome_raw", 0.0)
        try:
            _cal_ts = float(getattr(runtime, "last_calibration_ts_ms", 0.0) or 0.0)
            indicators["calibration_age_ms"] = (
                float(now_ts) - _cal_ts if _cal_ts > 0 else 0.0
            )
        except Exception:
            indicators.setdefault("calibration_age_ms", 0.0)

        # ---------------------------------------------------------------
        # v12_of Group MX: Derived / Interaction Features (from existing indicators)
        # All computable inline from v10/v11 keys already populated above.
        # ---------------------------------------------------------------
        try:
            indicators["spread_percentile_rank_1d"] = float(
                getattr(runtime, "spread_bps_rank_1d", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("spread_percentile_rank_1d", 0.0)
        try:
            # cvd_divergence_from_price: 1.0 when CVD slope and short-term momentum disagree
            _cvd_slope_sign = 1.0 if float(indicators.get("cvd_slope", 0.0) or 0.0) > 0 else -1.0
            _mom_sign       = 1.0 if float(indicators.get("momentum_10s", 0.0) or 0.0) > 0 else -1.0
            indicators["cvd_divergence_from_price"] = (
                1.0 if _cvd_slope_sign != _mom_sign else 0.0
            )
        except Exception:
            indicators.setdefault("cvd_divergence_from_price", 0.0)
        try:
            indicators["order_imbalance_momentum"] = float(
                getattr(runtime, "ofi_delta_ema", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("order_imbalance_momentum", 0.0)
        try:
            indicators["atr_percentile_rank_30d"] = float(
                getattr(runtime, "atr_bps_rank_30d", 0.0) or 0.0
            )
        except Exception:
            indicators.setdefault("atr_percentile_rank_30d", 0.0)
...
# A3: Rolling trackers (VWAP diff, momentum, realized vol)
        ...
        try:
            ...
            if lp and isinstance(lp, dict):
                ...
                indicators["lob_depth_slope_imb"] = float(lp.get("depth_slope_imb", 0.0) or 0.0)
        ...
        try:
            ...
            if int(indicators.get("dq_veto", 0) or 0) == 1:
                _bucket = sanitize_dq_bucket((indicators.get("dq_reason_bucket", "other") or "other"))
        ...
        try:
            _sym = str(runtime.symbol)
            _bk  = (indicators.get("exec_regime_bucket", "NORMAL") or "NORMAL")
        ...
        try:
            ...
            indicators.setdefault("obi", 0.0)
            indicators.setdefault("obi_z", 0.0)
                                    ...
                                    if isinstance(res2, dict) and int(res2.get("tripped") or 0) == 1:
                                        ...
                                        try:
                                            ...
                                            if until_ms > 0:
                                                ...
                                                if hard_until_ms > 0:
                                                    runtime.of_inputs_v3_disabled_hard_until_ms = int(hard_until_ms)
                                                runtime.of_inputs_v3_disabled_reason = str(downgrade_reason)
                                                try:
                            ...
                            if str(downgrade_reason) != "circuit_disabled":
                                try:
                                    ...
                                    if not isinstance(last_map, dict):
                                        last_map = {}
                                        runtime._of_inputs_quarantine_last = last_map
    ...
    async def _emit_payload(self, runtime: SymbolRuntime, payload: dict[str, Any], tick_ts: int) -> dict[str, Any]:
        ...
        try:
            ...
            if rd is not None:
                ...
                bkt = (_rdind2.get("exec_regime_bucket", "NORMAL") or "NORMAL")
    ...
    def _emit_gate_metrics(self, runtime, ofc, indicators, ev, tick_ts):
        ...
        """
        try:
            enable = (os.getenv("OF_GATE_METRICS_ENABLE", "1") or "1").strip()
            ...
            dq_enable = (os.getenv("OF_GATE_DQ_QUARANTINE_ENABLE", "0") or "0").strip() in ("1", "true", "True", "yes", "YES")
            dq_stream = os.getenv("OF_GATE_DQ_QUARANTINE_STREAM", "quarantine:metrics:of_gate")
            ...
            meta_cov = float(ev.get(MetaKeys.FEATURE_COVERAGE, 0.0) or 0.0)
            meta_enforce = (ev.get("meta_enforce_mode", "") or "")
            meta_rec = (ev.get("meta_recommended", "") or "")
            ...
            try:
                if ok == 1:
                    ...
                    drift = (ev.get("drift_state") or "").lower()
                    if dq and dq not in ("ok", "na"):
                        reason_top1 = "dq_fail"
                    elif drift in ("block", "fail", "veto"):
                        reason_top1 = "drift_block"
                    elif int(ev.get(MetaKeys.VETO, 0) or 0) == 1 or str((ev.get("ml") or {}).get("state", "") or "").lower() in ("veto", "block", "fail"):
            ...
            fields = {
                ...
                "exec_regime_bucket": (indicators.get("exec_regime_bucket", "NORMAL") or "NORMAL"),
                "vol_regime_label": (indicators.get("vol_regime_label", "na") or "na"),
                "liq_regime_label": (indicators.get("liq_regime_label", "na") or "na"),
    ...
    async def _emit_decision_record(
        ...
        try:
            ...
            rec = DecisionRecordV1(
                ...
                meta_enforce_cov_bucket=(f.get(MetaKeys.ENFORCE_COV_BUCKET, "unknown")),
                meta_enforce_applied=bool(f.get(MetaKeys.ENFORCE_APPLIED, False)),
                policy_raw_mode=(f.get("policy_raw_mode", "")),
                policy_effective_mode=(f.get("policy_effective_mode", "")),
                policy_hysteresis_debug=(f.get("policy_hysteresis_debug", "")),
                policy_changed=bool(f.get("policy_changed", False)),
                ctx_enabled=bool(f.get("ctx_enabled", False)),
                ctx_mode=(f.get(CtxKeys.MODE, "off")),
                ctx_key=(f.get(CtxKeys.KEY, "")),
                ctx_bundle_ver=(f.get(CtxKeys.BUNDLE_VER, "")),
                ctx_exec_model_ver=(f.get(CtxKeys.EXEC_MODEL_VER, "")),
                ctx_rule_model_ver=(f.get(CtxKeys.RULE_MODEL_VER, "")),
                ctx_p_rule_raw=f.get(CtxKeys.P_RULE_RAW, None),
                ctx_p_rule_cal=f.get(CtxKeys.P_RULE_CAL, None),
                ctx_cost_p50_bps=f.get(CtxKeys.COST_P50_BPS, None),
                ctx_cost_p90_bps=f.get(CtxKeys.COST_P90_BPS, None),
                ctx_exec_risk_ref_bps=f.get(CtxKeys.EXEC_RISK_REF_BPS, None),
                ctx_edge_net_p50_bps=f.get(CtxKeys.EDGE_NET_P50_BPS, None),
                ctx_edge_net_p90_bps=f.get(CtxKeys.EDGE_NET_P90_BPS, None),
                ctx_reason=(f.get(CtxKeys.REASON, "")),
                ctx_fallback_level=(f.get(CtxKeys.FALLBACK_LEVEL, "")),
                ctx_shadow_disagree=bool(f.get(CtxKeys.SHADOW_DISAGREE, False)),
                ctx_infer_latency_us=f.get(CtxKeys.INFER_LATENCY_US, None),
                payload_summary={
                    ...
                    "ctx_reason": (f.get(CtxKeys.REASON, "")),
                    "ctx_edge_net_p50_bps": f.get(CtxKeys.EDGE_NET_P50_BPS, None),
                    "ctx_shadow_disagree": int(bool(f.get(CtxKeys.SHADOW_DISAGREE, False))),
                },
            )
