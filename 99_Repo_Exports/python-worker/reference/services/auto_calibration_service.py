...
class AutoCalibrationService:
    ...
    def _calibrate_symbol(self, conn, cfg: SymbolConfig) -> None:
        ...
        # лог для контроля
        for s in all_stats:
            from tools.trailing_tp1_calibration import score_offset  # локальный импорт, чтобы не тянуть лишнее наверх
            sc = score_offset(s)
            logger.info(
...
def get_auto_calibration_service() -> AutoCalibrationService:
    ...
    if _auto_calibration_service is None:
        ...
        _auto_calibration_service = AutoCalibrationService(dsn=dsn, redis_url=redis_url, symbols=symbols)
...
def init_auto_calibration(trades_threshold: int, enabled_symbols: List[str], source: str) -> None:
    ...
    with _auto_calibration_lock:
        ...
        _auto_calibration_service = AutoCalibrationService(dsn=dsn, redis_url=redis_url, symbols=selected)
        logger.info(
            "Auto calibration initialized: " +
            f"threshold={thr}, source={src or 'ANY'}, symbols={[c.symbol for c in selected]}"
        )
