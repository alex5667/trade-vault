...
class BarProcessor:
    ...
    async def process_bar(self, runtime: SymbolRuntime, bar: MicroBar):
        ...
        try:
            ...
            # 3. Dynamic Regime Update
            await self._update_regime(runtime)
            ...
            try:
                ...
                try:
                    if getattr(runtime, "vol_regime", None) is not None:
                        ...
                        runtime.dynamic_cfg["vol_fast_bps"]     = float(vol_snap.get("vol_fast_bps", 0.0))
    ...
    async def _update_regime(self, runtime: SymbolRuntime):
        try:
    ...
    def _compute_trend_bias(self, runtime: SymbolRuntime, bar: MicroBar) -> Tuple[str, str, float]:
        ...
        # 3) Regime-based fallback (medium)
        bias_regime_enable = bool(_safe_int(cfg.get("bias_regime_enable", os.getenv("BIAS_REGIME_ENABLE", "0")), 0))
        ...
        # 4) RSI-based fallback (weak/medium)
        bias_rsi_enable = bool(_safe_int(cfg.get("bias_rsi_enable", os.getenv("BIAS_RSI_ENABLE", "0")), 0))
        if bias_rsi_enable:
             ...
             if bool(int(os.getenv("ATR_TF_CALIB_ENABLE", "1"))):
                 ...
                 if now_ts > 0 and (now_ts - last) >= refresh_ms and close_px > 0:
                     ...
                     if atr_bps_by_tf:
                         ...
                         mode = str(os.getenv("ATR_TF_SELECTOR_MODE", "shadow")).lower()
                            ...
                            if direction in ("LONG", "SHORT"):
                                signal_payload = {
                                    ...
                                    "indicators": {
                                        ...
                                        "adx": float(runtime.dynamic_cfg.get("adx14", 0.0)),
