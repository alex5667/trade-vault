...
async def _write_outputs(
    ...
    *,
    ...
    sid, symbol, close_ts_ms, r_mult = extract_close_info(close_ev)
...
async def process_close_event(
    ...
    *,
    ...
    sid, symbol, close_ts_ms, _ = extract_close_info(close_ev)
    if not symbol:
        symbol = "unknown"
    ...
    label = compute_label_and_brier(decision=decision, r_mult=extract_close_info(close_ev)[3])
            ...
            for _stream_name, msgs in res:
                for msg_id, raw_fields in msgs:
                    try:
                        ...
                        sid, symbol, _, _ = extract_close_info(ev)
                        if not symbol:
                            symbol = "unknown"
