...
OUTBOX_META_PREFIX = os.getenv("OUTBOX_META_PREFIX", "signal:meta:")
...
def build_outbox_envelope(
    *,
    ...
    print(f"DEBUG_BUILD_OUTBOX_ENV: sid={sid_s} keys={list(env_safe.keys())} targets={'targets' in env_safe}")
    return env_safe
...
def build_envelope(
    *,
    ...
    """
    env: Dict[str, Any] = {}
    # Tradeable payload fields at top-level
    ...
    try:
        m_obj.setdefault("state_source_stream", os.getenv("OUTBOX_EVENT_STREAM_KEY", "orders:exec"))
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            redis.xadd(stream_name, {"sid": str(event.get("sid", "")), "data": payload})
        return True
    except Exception:
        return False
