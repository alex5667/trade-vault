...
5) quarantine:metrics:of_gate -> of_gate_metrics_quarantine table (P3: "грязь" отдельно)
...
P3 ENV flags:
  ...
  OF_GATE_QUARANTINE_ARCHIVE_ENABLED=1     — enable quarantine:metrics:of_gate archiver
...
class PgWriter:
    ...
    #   - of_gate_metrics_quarantine: quarantine:metrics:of_gate (DQ-flagged rows)
    #
    ...
    def ensure_of_gate_metrics_rollups_timescale(self) -> None:
        ...
        BEGIN
          ...
          BEGIN
            PERFORM add_continuous_aggregate_policy('of_gate_ok_rate_5m', start_offset => INTERVAL '7 days', end_offset => INTERVAL '5 minutes', schedule_interval => INTERVAL '5 minutes');
          EXCEPTION WHEN undefined_function THEN
            NULL;
          END;
          BEGIN
            PERFORM add_continuous_aggregate_policy('of_gate_ok_rate_1h', start_offset => INTERVAL '180 days', end_offset => INTERVAL '1 hour', schedule_interval => INTERVAL '1 hour');
          EXCEPTION WHEN undefined_function THEN
            NULL;
          END;
    ...
    def insert_of_gate_metrics(self, rows: List[Tuple[Any, ...]]) -> int:
        ...
        INSERT INTO of_gate_metrics (
          stream_id, ts_ms, ts,
          symbol, schema_version, scenario_v4,
          ok, ok_soft, reason_code, missing_legs,
          payload_json
        ) VALUES %s
        ON CONFLICT (stream_id, ts) DO NOTHING
        """
    ...
    def insert_of_gate_quarantine(self, rows: List[Tuple[Any, ...]]) -> int:
        ...
        INSERT INTO of_gate_metrics_quarantine (
          stream_id, ts_ms, ts,
          src_stream, src_stream_id, dq_code, err,
          payload_json
        ) VALUES %s
        ON CONFLICT (stream_id, ts) DO NOTHING
        """
...
class StreamArchiver:
    ...
    def __init__(self, r: aioredis.Redis, pg: PgWriter):
        ...
        self.events_stream = env("TRADE_EVENTS_STREAM", "events:trades")
        ...
        self.of_gate_metrics_stream = env("OF_GATE_METRICS_STREAM", "metrics:of_gate")
        self.of_gate_quarantine_stream = env("OF_GATE_QUARANTINE_STREAM", "quarantine:metrics:of_gate")

        # P3: OF-gate streams (per-event archival)
        self.of_gate_stream = env("OF_GATE_METRICS_STREAM", "metrics:of_gate")
        self.of_gate_q_stream = env("OF_GATE_QUARANTINE_STREAM", "quarantine:metrics:of_gate")
    ...
    def of_gate_row(self, stream_id: str, payload: Dict[str, Any]) -> Tuple[Any, ...]:
        ...
        schema_version = safe_int(rule.get("schema_version") or payload.get("schema_version")) or 1
        scenario_v4 = str(rule.get("scenario_v4") or payload.get("scenario_v4") or payload.get("scenario") or "unknown")
        ok = safe_int(rule.get("ok") or payload.get("ok")) or 0
        ok_soft = safe_int(rule.get("ok_soft") or payload.get("ok_soft")) or 0
        reason_code = str(rule.get("reason_code") or payload.get("reason_code") or payload.get("reason") or "na")

        missing_legs = rule.get("missing_legs")
        if missing_legs is None:
            missing_legs = payload.get("missing_legs")
        missing_legs_json = json.dumps(missing_legs, ensure_ascii=False) if missing_legs is not None else None

        return (
            stream_id, ts_ms, ts,
            symbol, int(schema_version), scenario_v4,
            int(ok), int(ok_soft), reason_code,
            missing_legs_json,
            json.dumps(payload, ensure_ascii=False),
        )
    ...
    def of_gate_quarantine_row(self, stream_id: str, payload: Dict[str, Any]) -> Tuple[Any, ...]:
        ...
        dq_code = payload.get("why") or payload.get("dq_code") or payload.get("reason")
        err = payload.get("err") or payload.get("error")

        inner = payload.get("payload")
        if isinstance(inner, (bytes, bytearray)):
            inner = inner.decode("utf-8", "ignore")
        if isinstance(inner, str):
            try:
                inner_obj = json.loads(inner)
            except Exception:
                inner_obj = {"_raw": inner[:4000]}
        ...
        return (
            stream_id, ts_ms, ts,
            str(src_stream) if src_stream is not None else None,
            str(src_stream_id) if src_stream_id is not None else None,
            str(dq_code) if dq_code is not None else None,
            str(err)[:500] if err is not None else None,
    ...
    async def consume_entry_audit(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                await self.dlq(self.entry_dlq, self.entry_stream, ack_ids[0], f"pg_batch_error:{e}", {"batch_size": len(rows)})
    ...
    async def consume_position_events(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                await self.dlq(self.events_dlq, self.events_stream, ack_ids[0], f"pg_batch_error:{e}", {"batch_size": len(rows)})
                await asyncio.sleep(1.0)
    ...
    async def consume_confidence_scores(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                await self.dlq(self.conf_scores_dlq, self.conf_scores_stream, ack_ids[0], f"pg_batch_error:{e}", {"batch_size": len(rows)})
                await asyncio.sleep(1.0)

    async def consume_of_gate_metrics(self) -> None:
        await self.ensure_group(self.of_gate_metrics_stream, self.of_gate_cg)
        loop = asyncio.get_running_loop()
        ...
        while True:
            ...
            for mid, fields in msgs:
                try:
                    ...
                    ack_ids.append(mid)
                    last_ts_ms = max(last_ts_ms, ts_ms_from_stream_id(mid))
                except Exception as e:
                    await self.dlq(self.of_gate_dlq, self.of_gate_metrics_stream, mid, f"parse_error:{e}", {"fields": fields})
                    await self.r.xack(self.of_gate_metrics_stream, self.of_gate_cg, mid)

            if not rows:
                continue

            try:
                inserted = await loop.run_in_executor(None, self.pg.insert_of_gate_metrics, rows)
                await self.r.xack(self.of_gate_metrics_stream, self.of_gate_cg, *ack_ids)
                await self._status_bump(self.of_gate_status_hash, inserted_delta=inserted, last_stream_ts_ms=last_ts_ms)
            except Exception as e:
                await self.dlq(self.of_gate_dlq, self.of_gate_metrics_stream, ack_ids[0], f"pg_batch_error:{e}", {"batch_size": len(rows)})
                await self._status_bump(self.of_gate_status_hash, inserted_delta=0, last_stream_ts_ms=last_ts_ms, err=str(e))
                await asyncio.sleep(1.0)

    async def consume_of_gate_quarantine(self) -> None:
        await self.ensure_group(self.of_gate_quarantine_stream, self.of_gate_quarantine_cg)
        loop = asyncio.get_running_loop()

        while True:
            pending = await self._claim_pending(
                self.of_gate_quarantine_stream,
                self.of_gate_quarantine_cg,
                self.of_gate_quarantine_consumer,
                self.of_gate_quarantine_min_idle,
                self.of_gate_quarantine_batch,
            )
            if pending:
                msgs = pending
            else:
                resp = await self._read_new(
                    ...
                    self.of_gate_quarantine_consumer,
                    self.of_gate_quarantine_batch,
                    self.of_gate_quarantine_block_ms,
                )
                if not resp:
                    continue
                _, msgs = resp[0]
            ...
            for mid, fields in msgs:
                try:
                    payload = parse_stream_payload(fields)
                    rows.append(self.of_gate_quarantine_row(mid, payload))
                    ack_ids.append(mid)
                    last_ts_ms = max(last_ts_ms, ts_ms_from_stream_id(mid))
                except Exception as e:
                    await self.dlq(self.of_gate_quarantine_dlq, self.of_gate_quarantine_stream, mid, f"parse_error:{e}", {"fields": fields})
                    await self.r.xack(self.of_gate_quarantine_stream, self.of_gate_quarantine_cg, mid)

            if not rows:
                continue

            try:
                inserted = await loop.run_in_executor(None, self.pg.insert_of_gate_quarantine, rows)
                await self.r.xack(self.of_gate_quarantine_stream, self.of_gate_quarantine_cg, *ack_ids)
                await self._status_bump(self.of_gate_quarantine_status_hash, inserted_delta=inserted, last_stream_ts_ms=last_ts_ms)
            except Exception as e:
                await self.dlq(self.of_gate_quarantine_dlq, self.of_gate_quarantine_stream, ack_ids[0], f"pg_batch_error:{e}", {"batch_size": len(rows)})
                await self._status_bump(self.of_gate_quarantine_status_hash, inserted_delta=0, last_stream_ts_ms=last_ts_ms, err=str(e))
                await asyncio.sleep(1.0)
    ...
    async def consume_of_gate_metrics(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                parse_errors += 1
                await self.dlq(self.of_gate_dlq, self.of_gate_stream, ack_ids[0], f"pg_batch_error:{e}", {"batch_size": len(rows)})
    ...
    async def consume_of_gate_quarantine(self) -> None:
        """Consume quarantine:metrics:of_gate stream and archive rows to of_gate_metrics_quarantine.
        ...
        while True:
            ...
            try:
                ...
                parse_errors += 1
                await self.dlq(self.of_gate_q_dlq, self.of_gate_q_stream, ack_ids[0], f"pg_batch_error:{e}", {"batch_size": len(rows)})
                # P78: bump error metrics on DB failure
                await self._bump_archiver_metrics(
                    self.of_gate_q_archiver_metrics_key,
                    ack_ids[-1] if ack_ids else last_seen_mid,
