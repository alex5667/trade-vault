...
def _notify_stream(text: str, severity: str = "crit", sid: str = None, source: str = "of_timers_worker") -> None:
    ...
    stream = (
        ...
        or ("notify:telegram:page" if severity == "page" else "notify:telegram:crit" if severity == "crit" else "notify:telegram")
    )
...
def run_decision_coverage_kpi() -> bool:
    """Run Decision Coverage KPI (P63)."""
    return run_tool("tools.decision_coverage_kpi_worker_v1", ["--once"], timeout=900)
...
# Hourly:11 World-practice trackers smoke-check (bucket/vol/res/fill)
            ...
            # P63: Decision Coverage KPI (hourly at :17)
            if minute >= 17 and minute < 18:
                last = last_run.get("decision_coverage", 0)
                if now.timestamp() - last > 3500:
                   run_decision_coverage_kpi()
                   last_run["decision_coverage"] = now.timestamp()
