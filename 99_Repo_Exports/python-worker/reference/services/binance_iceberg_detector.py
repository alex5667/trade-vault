...
ORDERS_QUEUE = os.getenv("ORDERS_QUEUE_BINANCE") or os.getenv("ORDERS_QUEUE") or "orders:queue:binance"
RAW_SIGNAL_STREAM = os.getenv("ICEBERG_RAW_STREAM", "signals:crypto:raw")
NOTIFY_STREAM = os.getenv("ICEBERG_NOTIFY_STREAM", "notify:telegram")
...
class BinanceIcebergDetector:
    ...
    def _publish_signal(self, direction: str, price: float, state: BestLevelState, level_info: Dict[str, Any]) -> None:
        ...
        if use_outbox or shadow_outbox:
            try:
                from services.outbox_envelope import build_outbox_envelope
                ...
                env = build_outbox_envelope(sid=sid, targets=targets, meta=meta)
                # ------------------------------------------------------------
