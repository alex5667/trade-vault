...
class EdgeGateReporter:
    def __init__(self, config: EdgeGateReportConfig, telegram: TelegramClient | None = None):
        self.config = config
    ...
    def _fetch_stats(self) -> list[dict]:
        ...
        with psycopg2.connect(self.config.db_dsn) as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
