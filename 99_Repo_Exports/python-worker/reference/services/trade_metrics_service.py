...
class TradeMetricsService:
    ...
    def accumulate_trade(self, m: Dict[str, Any], t: Dict[str, Any]) -> bool:
        ...
        if tp3 > 0 and pnl <= eps:
            bucket = "TP3"
            # pnl_gross - fees: если fees отрицательный (норма), это pnl_gross + |fees|
            # если fees положительный (аномалия Redis), это pnl_gross - fees (тоже корректно)
            adjusted = pnl_gross - fees
            if adjusted <= eps:
                # Fallback: используем abs(fees), чтобы не получить отрицательный pnl
                # при аномальных fees > 0 (раньше было max(fees, eps) → возвращало fees как pnl)
                adjusted = max(abs(fees), eps)
        ...
        # Increment R-count if we have ANY risk measure (even the floor)
        # to ensure expectancy is calculated correctly across all trades.
        m["cnt_r"] += 1
        m["sum_r"] += r
        m["sum_r2"] += r * r
        if r > eps:
        ...
        try:
            ...
            # 4. OF Confirm Stats (Detailed Breakdown)
            ...
            if of_confirm and isinstance(of_confirm, dict):
                ...
                if ml_dec and isinstance(ml_dec, dict):
                    ...
                    p_min = _sf(ml_dec.get("p_min", 0.55))
                    
                    # We only care if rule_ok was 1 (passed heuristic), 
                    # because ML gate logic in ENFORCE only flips decisions when rule_ok==1.
                    # However, for Shadow analytics, we can track all signals where ML was checked.
                    # Usually, ML is only checked if other gates passed, but build() calls it always.
                    ...
                    if p_edge < 0.3:
                        ...
                        p_edge_bucket = "0.3-0.4"
                    elif p_edge < 0.5:
                        p_edge_bucket = "0.4-0.5"
                    elif p_edge < 0.6:
                        p_edge_bucket = "0.5-0.6"
                    elif p_edge < 0.7:
                        p_edge_bucket = "0.6-0.7"
                    else:
                        p_edge_bucket = "0.7-1.0"
