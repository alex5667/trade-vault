...
class ConfidenceScorer:
    ...
    def score(self, *args, kind: str = None, side: str = None, ctx: Any = None, **kwargs) -> Tuple[float, Dict[str, float]]:
        ...
        if raw_mode in ("trend", "trending", "momentum"):
            regime = "trend"
        elif raw_mode in ("range", "meanrev", "chop", "sideways"):
            regime = "range"
        else:
            regime = "mixed"
        ...
        elif kind == "weak_progress":
            thr = self.main_z_thr
            s_z = self._z_score(z_abs, thr)
            # weak progress is a structural signal: extreme effort vs small result
            # therefore delta_z itself is highly reliable evidence
            s_mode = 1.0 if regime != "trend" else 0.75
            obi_avg = _f_any(ctx, "obi_avg", "obi", default=0.0)
            obi_sustained = _b(ctx, "obi_sustained", False)
            s_obi = self._obi_score(obi_avg, obi_sustained, dir_sign, self.obi_thr)
