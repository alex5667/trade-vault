...
def test_zscore_basic():
    ...
    if result:
        ...
        print(f"❌ No spike detected (expected z≈3.0)")
        return False
...
def test_zscore_bias():
    ...
    if result:
        print(f"✅ Spike detected: delta={result['delta']:.2f}, z={result['z']:.2f}")
        print(f"   Note: Including current value in stats reduces Z-score")
        print(f"   True Z (excluding current): (50-1)/~0 = very large")
        print(f"   Actual Z (including current): ≈9.96")
    else:
...
def test_zscore_market_scenario():
    ...
    print(f"   Spike delta: 5.0")
    print(f"   Expected Z: (5.0 - {mean:.3f}) / {std:.3f} = {(5.0 - mean) / std:.2f}")
    
    if result:
        print(f"✅ Spike detected: delta={result['delta']:.2f}, z={result['z']:.2f}")
    else:
        print(f"⚠️  No spike detected (z < 2.5)")
        print(f"   This may indicate Z-scores are too low in current market")
...
def test_zscore_low_variance():
    ...
    print(f"   Flat market: all deltas = 1.0, then 1.1")
    
    if result:
        print(f"✅ Spike detected: delta={result['delta']:.2f}, z={result['z']:.2f}")
        print(f"   Note: Very low variance can cause false positives")
    else:
        print(f"⚠️  No spike (std=0 check prevented division by zero)")
