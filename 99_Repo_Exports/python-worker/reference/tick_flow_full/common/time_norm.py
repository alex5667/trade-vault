...
def normalize_epoch_ms(timestamp: int | float | str | datetime) -> int:
    ...
    if isinstance(timestamp, int):
        # If it's already an integer, assume it's already in ms
        # But check if it looks like seconds (too small)
        if timestamp < 1_000_000_000:  # Before year 2001
            return timestamp * 1000
        return timestamp

    if isinstance(timestamp, float):
        # If it's a float, check if it's seconds or milliseconds
        if timestamp < 1_000_000_000:  # Before year 2001 in seconds
