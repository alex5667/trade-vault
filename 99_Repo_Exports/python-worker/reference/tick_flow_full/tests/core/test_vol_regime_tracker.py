...
def test_dynamic_cfg_propagation_includes_regime_label():
    ...
    for i, px in enumerate(prices):
        ...
        dynamic_cfg["vol_fast_bps"]     = float(snap["vol_fast_bps"])
        dynamic_cfg["vol_slow_bps"]     = float(snap["vol_slow_bps"])
        dynamic_cfg["vol_ratio"]        = float(snap["vol_ratio"])
        dynamic_cfg["vol_ratio_z"]      = float(snap["vol_ratio_z"])
        dynamic_cfg["vol_regime_label"] = str(snap["vol_regime_label"])

    # All keys present, all numeric values finite
    final_snap = tracker.snapshot()
    assert dynamic_cfg["vol_fast_bps"] == final_snap["vol_fast_bps"]
    assert dynamic_cfg["vol_slow_bps"] == final_snap["vol_slow_bps"]
    assert dynamic_cfg["vol_ratio"]    == final_snap["vol_ratio"]
    assert dynamic_cfg["vol_ratio_z"]  == final_snap["vol_ratio_z"]
    assert dynamic_cfg["vol_regime_label"] in ("shock", "normal", "calm", "na")
    for k in ("vol_fast_bps", "vol_slow_bps", "vol_ratio", "vol_ratio_z"):
