...
class ModeValidator(Validator):
    ...
    def validate(self, ctx: Any, cand: Candidate, q: QualityState) -> None:
        ...
        # breakout: в meanrev требуем сильнее (минимальная защита)
        ...
        # extreme: в meanrev требуем чуть сильнее (как было раньше)
        if cand.kind == "extreme" and mode == "meanrev":
            thr = float(getattr(ctx, "_extreme_thr", 0.0) or 0.0)
            if thr > 0 and z_abs < (thr * 1.15):
                q.veto_with("extreme_in_meanrev_requires_stronger_z")
...
class L3OptionalValidator(Validator):
    ...
    def validate(self, ctx: Any, cand: Candidate, q: QualityState) -> None:
        ...
        if cand.kind == "extreme" and os.getenv("EXTREME_USE_L3_FILTERS", "false").lower() != "true":
            return
        if cand.kind == "absorption" and os.getenv("ABSORPTION_USE_L3_FILTERS", "false").lower() != "true":
            return
