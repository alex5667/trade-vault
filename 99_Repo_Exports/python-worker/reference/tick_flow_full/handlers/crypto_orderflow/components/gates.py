...
class CryptoSignalGates:
    ...
    def check_regime_gate(self, ctx: Any, kind: str) -> Tuple[bool, str]:
        ...
        strict = (os.getenv("REGIME_GATE_STRICT", "0") or "0").strip().lower() in {"1","true","yes","on"}
        if strict and regime:
            def _csv(name: str) -> set[str]:
                v = (os.getenv(name, "") or "").strip().lower()
                return {x.strip() for x in v.split(",") if x.strip()}

            breakout_block = _csv("REGIME_GATE_BREAKOUT_BLOCK")
            extreme_block = _csv("REGIME_GATE_EXTREME_BLOCK")

            if k == "breakout" and any(x in regime for x in breakout_block):
                return (False, "VETO_REGIME_BREAKOUT_BLOCK")
            if k == "extreme" and any(x in regime for x in extreme_block):
