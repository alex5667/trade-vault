...
class BreakoutValidator(KindValidator):
    ...
    def adjust(self, *, kind: str, ctx: Any, side: int, level_price: float | None) -> Adjustment:
        ...
        if mp_shift >= self.fake_shift_bps and taker < self.min_taker:
            flags.append("bo_fake_breakout_penalty")
...
class AbsorptionValidator(KindValidator):
    ...
    def adjust(self, *, kind: str, ctx: Any, side: int, level_price: float | None) -> Adjustment:
        ...
        if not g1:
            flags.append("ab_missing_wall_or_refill")
        if not g2:
            flags.append("ab_missing_mp_contra_or_proxy")
        if taker < self.min_taker:
            flags.append("ab_low_taker_rate")
...
class OBISpikeValidator(KindValidator):
    ...
    def adjust(self, *, kind: str, ctx: Any, side: int, level_price: float | None) -> Adjustment:
        ...
        if c2t >= self.c2t_bad:
            flags.append("obi_high_cancel_to_trade_penalty")
            mult *= self.c2t_penalty

        if c2t >= self.c2t_veto and (not obi_sust):
            flags.append("obi_spoof_veto")
            veto = True
