...
Covers:
  - VolRegimeTracker     — fast/slow realized-vol ratio + robust z-score
  - BookResilienceTracker — sweep -> crater -> replenishment detection
  - fill_prob_proxy       — fill probability monotonicity
  - TickProcessor         — missing qty / volume must never crash the hot path

All tests use the real project API but are written defensively:
helpers _as_float / _call_update / _call_snapshot tolerate minor signature
...
# conftest.py already adds tick_flow_full and python-worker root; this block
# is a belt-and-suspenders fallback that also handles direct `pytest` calls
# from a different CWD.
# ---------------------------------------------------------------------------
_TF_ROOT = Path(__file__).resolve().parents[2]   # tick_flow_full/
_PW_ROOT = Path(__file__).resolve().parents[3]   # python-worker/
for _p in (str(_TF_ROOT), str(_PW_ROOT)):
