...
# ---------------------------------------------------------------------------
# Decision logs (shadow/AB evaluation)
# ---------------------------------------------------------------------------

confidence_cal_decision_log_total = _get_or_create(
    "confidence_cal_decision_log_total",
    "counter",
    "Total confidence-cal decision log events written",
    ["symbol", "stage", "served_arm", "mode"],
)

confidence_cal_decision_log_error_total = _get_or_create(
    "confidence_cal_decision_log_error_total",
    "counter",
    "Total errors while writing confidence-cal decision logs",
    ["symbol", "stage", "err"],
)

confidence_cal_decision_log_sampled_out_total = _get_or_create(
    "confidence_cal_decision_log_sampled_out_total",
    "counter",
    "Total confidence-cal decision log events skipped due to sampling",
    ["symbol", "stage"],
)
...
def inc_decision_log(symbol: str, stage: str, served_arm: str, mode: str) -> None:
    try:
        s = symbol or "unknown"
        st = stage or "unknown"
        a = served_arm or "unknown"
        m = mode or "unknown"
        ctr = _safe_labels(confidence_cal_decision_log_total, symbol=s, stage=st, served_arm=a, mode=m)
        if ctr is not None:
            ctr.inc()
    except Exception:
        pass
...
def inc_decision_log_error(symbol: str, stage: str, err: str) -> None:
    try:
        s = symbol or "unknown"
        st = stage or "unknown"
        e = err or "unknown"
        ctr = _safe_labels(confidence_cal_decision_log_error_total, symbol=s, stage=st, err=e)
        if ctr is not None:
            ctr.inc()
    except Exception:
        pass
...
def inc_decision_log_sampled_out(symbol: str, stage: str) -> None:
    try:
        s = symbol or "unknown"
        st = stage or "unknown"
        ctr = _safe_labels(confidence_cal_decision_log_sampled_out_total, symbol=s, stage=st)
        if ctr is not None:
            ctr.inc()
    except Exception:
        pass
