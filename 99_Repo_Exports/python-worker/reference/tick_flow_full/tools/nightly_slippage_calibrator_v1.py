...
async def run() -> bool:
    ...
    # Capture calibration run timestamp once (used for all per-bucket ts keys)
    now_ms = _now_ms()
    ...
    for (sym, bucket), ratios in sorted(groups.items()):
        ...
        # Companion timestamp key for staleness monitoring by exporter/alerts
        ...
        # Per-bucket last-fit diagnostics (best-effort, readable via redis-cli hgetall)
        try:
    ...
    # and used by SlippageCalibStale / SlippageCalibNoUpdates alert rules.
    try:
    ...
    logger.info("done: updated=%d / total_groups=%d", updated, len(groups))
    return True
