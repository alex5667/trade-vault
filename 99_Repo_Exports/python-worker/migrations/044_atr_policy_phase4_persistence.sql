-- Migration 044: ATR Policy Phase 4 Persistence
-- This makes PostgreSQL the source of truth for runtime policy snapshots and decisions.
...
CREATE INDEX IF NOT EXISTS idx_atr_policy_proposals_lookup
  ON atr_policy_proposals (symbol, scenario, regime, risk_horizon_bucket, updated_at_ms DESC);
...
CREATE INDEX IF NOT EXISTS idx_atr_policy_snapshots_lookup
  ON atr_policy_snapshots (source, symbol, scenario, regime, risk_horizon_bucket, snapshot_kind, created_at DESC);

CREATE TABLE IF NOT EXISTS atr_policy_recovery_events (
  id bigserial PRIMARY KEY,
  event_type text NOT NULL,              -- boot_restore | drift_fix | rollback_last_good | kill_switch_restore
  source text NOT NULL,
  symbol text NOT NULL,
  scenario text NOT NULL,
  regime text NOT NULL,
