...
class TestOutcomeGate:
    ...
    def test_read_outcome_stats_empty(self):
        ...
        stats = read_outcome_stats(r, stream="events:trades", since_ms=0, symbols=[], max_scan=1000)
        assert stats["n"] == 0.0
    ...
    def test_read_outcome_stats_with_trades(self):
        ...
        stats = read_outcome_stats(r, stream="events:trades", since_ms=ts - 10000, symbols=["BTCUSDT"], max_scan=1000)
        assert stats["n"] == 3.0
...
class TestMainTripleWindow:
    ...
    def test_main_triple_window_all_unhealthy(self):
        ...
        with patch.dict(os.environ, {
            ...
            with patch("of_gate_hardstop_cap_unclamp_v3.redis.Redis.from_url", return_value=r):
                main()
