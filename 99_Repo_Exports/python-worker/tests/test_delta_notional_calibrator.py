...
def test_dn_calib_hour_of_week():
    ...
    ts_mon_10 = int(datetime(2024, 1, 1, 10, 0, tzinfo=timezone.utc).timestamp() * 1000)
    # Tuesday 15:00 UTC (weekday 1, hour 15 -> how=39)
    ts_tue_15 = int(datetime(2024, 1, 2, 15, 0, tzinfo=timezone.utc).timestamp() * 1000)
...
def test_hour_of_week_utc():
    ...
    ts_sun_23 = int(datetime(2024, 1, 7, 23, 0, tzinfo=timezone.utc).timestamp() * 1000)
...
def test_session_utc():
    ...
    ts_asia = int(datetime(2024, 1, 1, 3, 0, tzinfo=timezone.utc).timestamp() * 1000)
    assert session_utc(ts_asia) == "asia"

    # EU session: 07:00-12:59
    ts_eu = int(datetime(2024, 1, 1, 9, 0, tzinfo=timezone.utc).timestamp() * 1000)
    assert session_utc(ts_eu) == "eu"

    # US session: 13:00-19:59
    ts_us = int(datetime(2024, 1, 1, 15, 0, tzinfo=timezone.utc).timestamp() * 1000)
    assert session_utc(ts_us) == "us"

    # Late session: 20:00-23:59
    ts_late = int(datetime(2024, 1, 1, 21, 0, tzinfo=timezone.utc).timestamp() * 1000)
