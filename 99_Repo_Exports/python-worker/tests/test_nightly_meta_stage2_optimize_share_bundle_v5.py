...
# -*- coding: utf-8 -*-
"""
test_nightly_meta_stage2_optimize_share_bundle_v5.py

Unit tests for nightly_meta_stage2_optimize_share_bundle_v5.py:
...
- Per-bucket global budgets (trend/range separate)
- Hard-stop detection and degrade-only mode
- Two-circuit selector (per-symbol then per-bucket global)
"""
...
from tools.nightly_meta_stage2_optimize_share_bundle_v5 import (
    now_ms,
    sign,
    _f,
    _i,
...
def test_build_options_degrade_only():
    """Test build_options with degrade_only=True (no increases)."""
    rows = []
    for i in range(500):
        key = f"key{i}"
        veto = 1 if (i % 10) < 3 else 0
    ...
    opts = build_options(
        ...
        max_up_step=max_up_step,
        max_down_step=max_down_step,
        degrade_only=True,  # No increases
        min_exec_rate=0.30,
        max_exec_rate_drop=0.20,
        tail_exec_cap=0.18,
        lam_tail=0.50,
    ...
    # All options should be <= cur_share
    for o in opts:
        assert o["share"] <= cur_share + 1e-9
...
def test_hard_stop():
    """Test hard_stop function."""
    # No hard-stop (healthy)
    st_healthy = {
        "lat_p99_us": 5000.0,  # < 12000
        "exec_p90": 0.80,  # < 0.92
        "soft_rate": 0.40,  # < 0.60
        "ok_rate": 0.25,  # > 0.10
    }
    ...
    with patch.dict(os.environ, {
        ...
        triggered, reasons = hard_stop(st_healthy)
        assert triggered is False
        assert len(reasons) == 0
    
    # Hard-stop triggered (latency)
    st_lat = {
        "lat_p99_us": 15000.0,  # > 12000
        "exec_p90": 0.80,
        "soft_rate": 0.40,
        "ok_rate": 0.25,
    }
    ...
    with patch.dict(os.environ, {
        ...
        triggered, reasons = hard_stop(st_lat)
        assert triggered is True
        assert len(reasons) > 0
        assert any("lat_p99_us" in r for r in reasons)
    
    # Hard-stop triggered (exec)
    st_exec = {
        "lat_p99_us": 5000.0,
        "exec_p90": 0.95,  # > 0.92
        "soft_rate": 0.40,
    ...
    with patch.dict(os.environ, {
        ...
        triggered, reasons = hard_stop(st_exec)
        assert triggered is True
        assert any("exec_p90" in r for r in reasons)
    
    # Hard-stop triggered (multiple)
    st_multi = {
        "lat_p99_us": 15000.0,  # > 12000
        "exec_p90": 0.95,  # > 0.92
        "soft_rate": 0.70,  # > 0.60
    ...
    with patch.dict(os.environ, {
        ...
        triggered, reasons = hard_stop(st_multi)
        assert triggered is True
...
def test_totals():
    ...
    td, rd, sd, so = totals(chosen)
    
    assert abs(td - (0.15 + 0.12)) < 1e-9  # trend_drop total
    assert abs(rd - (0.10 + 0.08)) < 1e-9  # range_drop total
    assert abs(sd - (0.25 + 0.20)) < 1e-9  # sum_drop total
    assert abs(so - (0.20 + 0.18)) < 1e-9  # sum_obj total


def test_select_under_bucket_budgets():
    """Test select_under_bucket_budgets function (v5 per-bucket budgets)."""
    # Create symbol plans with multiple combos
    plans = {
        "BTCUSDT": [
            {"trend_drop": 0.20, "range_drop": 0.15, "sum_drop": 0.35, "sum_obj": 0.25, "trend": {"share": 0.35}, "range": {"share": 0.25}},
            {"trend_drop": 0.15, "range_drop": 0.10, "sum_drop": 0.25, "sum_obj": 0.20, "trend": {"share": 0.25}, "range": {"share": 0.20}},
            {"trend_drop": 0.10, "range_drop": 0.05, "sum_drop": 0.15, "sum_obj": 0.15, "trend": {"share": 0.15}, "range": {"share": 0.15}},
        ],
        "ETHUSDT": [
            {"trend_drop": 0.18, "range_drop": 0.12, "sum_drop": 0.30, "sum_obj": 0.22, "trend": {"share": 0.30}, "range": {"share": 0.20}},
            {"trend_drop": 0.12, "range_drop": 0.08, "sum_drop": 0.20, "sum_obj": 0.18, "trend": {"share": 0.20}, "range": {"share": 0.15}},
            {"trend_drop": 0.05, "range_drop": 0.05, "sum_drop": 0.10, "sum_obj": 0.12, "trend": {"share": 0.10}, "range": {"share": 0.10}},
        ],
    }
    
    budget_trend = 0.25  # Max trend_drop total
    budget_range = 0.20  # Max range_drop total
    budget_total = None  # Optional
    ...
    # Check budgets are respected
    td, rd, sd, _ = totals(chosen)
    assert td <= budget_trend + 1e-12
    assert rd <= budget_range + 1e-12


def test_select_under_bucket_budgets_with_total():
    """Test select_under_bucket_budgets with total budget."""
    plans = {
        "BTCUSDT": [
            {"trend_drop": 0.20, "range_drop": 0.15, "sum_drop": 0.35, "sum_obj": 0.25, "trend": {"share": 0.35}, "range": {"share": 0.25}},
            {"trend_drop": 0.10, "range_drop": 0.05, "sum_drop": 0.15, "sum_obj": 0.15, "trend": {"share": 0.15}, "range": {"share": 0.15}},
    ...
    budget_trend = 1.00  # Not constraining
    budget_range = 1.00  # Not constraining
    budget_total = 0.20  # Total drop should be <= 0.20
    ...
    td, rd, sd, _ = totals(chosen)
    assert sd <= budget_total + 1e-12


def test_select_under_bucket_budgets_within_budget():
    """Test select_under_bucket_budgets when already within budget."""
    plans = {
        "BTCUSDT": [
            {"trend_drop": 0.10, "range_drop": 0.05, "sum_drop": 0.15, "sum_obj": 0.20, "trend": {"share": 0.25}, "range": {"share": 0.20}},
        ],
        "ETHUSDT": [
            {"trend_drop": 0.08, "range_drop": 0.05, "sum_drop": 0.13, "sum_obj": 0.18, "trend": {"share": 0.20}, "range": {"share": 0.15}},
        ],
    }
    
    budget_trend = 0.30  # Total trend_drop = 0.18 < 0.30
    budget_range = 0.20  # Total range_drop = 0.10 < 0.20
