...
def test_engine_selects_v2_schema(engine):
    ...
    with patch.object(engine, "_load_meta_model_slot", return_value=mock_model), \
        ...
        assert evidence["meta_schema_name"] == META_FEAT_V2_NAME
        assert evidence["meta_schema_version"] == META_FEAT_V2_VERSION
...
def test_engine_fallback_v1_schema(engine):
    ...
    with patch.object(engine, "_load_meta_model_slot", return_value=mock_model):
        ...
        assert res is not None
        evidence = res.evidence
        assert evidence["meta_schema_name"] == "meta_feat_v1"
        assert evidence["meta_schema_version"] == 1
