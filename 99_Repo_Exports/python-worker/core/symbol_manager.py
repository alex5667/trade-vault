...
Format: {"action": "add|remove|set", "symbols": ["XAUUSD", "BTCUSD", ...]}
"""
...
def publish_set_symbols(redis_client: redis.Redis, symbols: List[str], stream: str = "config:symbols") -> None:
    ...
    Examples:
        >>> publish_set_symbols(redis_client, ["XAUUSD", "BTCUSD", "ETHUSD"])
        # Все handlers кроме XAUUSD, BTCUSD, ETHUSD будут остановлены
        # XAUUSD, BTCUSD, ETHUSD будут запущены (если еще не запущены)
    """
    command = {
        "action": "set",
        "symbols": symbols,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  python symbol_manager.py set XAUUSD BTCUSD ETHUSD")
