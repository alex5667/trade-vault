...
def get_default_delta_tiers(symbol: str) -> dict:
    ...
    Tiers are set lower than peak spike values to catch mid-frequency structural deltas,
    not just extreme outliers. Runtime calibrator refines them from live data.

    Tiers:
    - Tier 0 (Trend): permissive — catches frequent structural deltas (~p50 spike)
    - Tier 1 (Range/Mixed): standard  (~p75 spike)
    - Tier 2 (Thin/News): strict     (~p90 spike)

    ENV overrides (per-symbol tuning without restart):
      DELTA_TIER_BTC_T0 / DELTA_TIER_BTC_T1 / DELTA_TIER_BTC_T2
      DELTA_TIER_SOL_T0 / DELTA_TIER_SOL_T1 / DELTA_TIER_SOL_T2
    """
    s = normalize_symbol(symbol)

    def _env_tier(key: str, default: float) -> float:
        try:
            raw = os.getenv(key)
    ...
    # 1. BTCUSDT
    if "BTC" in s:
        t0 = _env_tier("DELTA_TIER_BTC_T0", 50_000.0)
        t1 = _env_tier("DELTA_TIER_BTC_T1", 100_000.0)
        t2 = _env_tier("DELTA_TIER_BTC_T2", 250_000.0)
    ...
    # 3. SOLUSDT
        ...
        t0 = _env_tier("DELTA_TIER_SOL_T0", 20_000.0)
        t1 = _env_tier("DELTA_TIER_SOL_T1", 40_000.0)
        t2 = _env_tier("DELTA_TIER_SOL_T2", 60_000.0)
    ...
    # 6. DOGE - Meme tier (lower tiers to match typical $1k-5k delta_usd)
    elif "DOGE" in s:
    ...
    # 12. WIF - Meme (lower tiers)
        ...
        t0, t1, t2 = 1_000.0, 2_000.0, 5_000.0

    # 13. Memes (1000* or PEPE/SHIB/FLOKI/BONK) - lower tiers to match typical $500-3k delta_usd
    elif "1000" in s or any(x in s for x in ["PEPE", "SHIB", "FLOKI", "BONK"]):
        t0, t1, t2 = 1_000.0, 3_000.0, 8_000.0
    
    # 14. Default / Fallback - Conservative
    else:
        t0, t1, t2 = 10_000.0, 20_000.0, 45_000.0
...
def get_default_cancel_spike_settings(symbol: str) -> dict:
    ...
    # NATGASUSDT (Commodities perp) - bootstrap conservative monitor mode
    elif s.startswith("NATGAS"):
        base.update({
            "cancel_spike_mode": "monitor",
            "cancel_spike_min_taker_rate": 0.0,
...
class OrderFlowConfig:
    ...
    min_confirmations: int = 1             # Минимальное количество жестких подтверждений (помимо дельты)
    fp_imb_counts_for_min_confirmations: bool = False # Считать ли fp_imb как жесткое подтверждение
    # === Staleness & TTL ===
    sweep_valid_ms: int = 120_000
    reclaim_signal_valid_ms: int = 120_000
    reclaim_hold_bars: int = 2
    obi_event_ttl_ms: int = 30_000
    obi_stable_min_secs: float = 1.0
    book_stale_ms: int = 15_000
    ...
    w_ice: float = 0.15
    w_abs: float = 0.05
    of_score_min: float = 0.60

    # === Publishing ===
    publish_of_confirm: bool = False
    of_confirm_stream: str = RS.OF_CONFIRM
    ...
    strong_need_range: int = 2             # Required legs for range scenarios
    ...
    stop_atr_mult: float = 1.0             # Множитель ATR для SL (was 0.6, raised to reduce noise stop-outs)
    ...
    def from_env(cls, symbol: str, base: Optional["OrderFlowConfig"] = None) -> "OrderFlowConfig":
        ...
        def_score_range = 0.40 if is_meme else 0.40
        ...
        cfg = cls(
            ...
            min_confirmations=_env_one(f"{prefix}_MIN_CONFIRMATIONS", int, base_cfg.min_confirmations),
            fp_imb_counts_for_min_confirmations=_env_one(f"{prefix}_FP_IMB_COUNTS_FOR_MIN_CONFIRMATIONS", _to_bool, base_cfg.fp_imb_counts_for_min_confirmations),
            ...
            book_stale_ms=_env_one(f"{prefix}_BOOK_STALE_MS", int, base_cfg.book_stale_ms),
...
ETHUSD_CONFIG = OrderFlowConfig(
    ...
    dist_atr_threshold=0.4,
    min_signal_interval_sec=30,
    # === Risk calibration (2026-04-18) ===
    # Avg SL 1.18 ATR vs TP 0.58 ATR was caused by:
    #  - no explicit stop_atr_mult → inherited global STOP_ATR_MULT=1.8 from ENV
    #  - tp_rr default="1,2,3" → TP1=1R which is below commission break-even
    # Fix: tighter stop (0.7 ATR), TP starts at 1.5R min to cover ~0.08% roundtrip fee
    stop_mode="ATR",
    stop_atr_mult=0.7,
    tp_mode="RR",
    tp_rr="1.5,2.5,3.5",
...
NATGASUSDT_CONFIG = OrderFlowConfig(
    ...
    delta_z_threshold=3.3,
    delta_abs_min_usd=1_500.0,
    weak_progress_atr=0.14,
    weak_progress_range_atr=0.45,
    weak_progress_body_atr=0.30,
    obi_threshold=0.38,
    obi_min_duration=2.0,
    iceberg_refresh_count=4,
    iceberg_min_duration=1.2,
    ...
    metadata={
        "asset_class": "tradfi_perp",
        "base_currency": "NATGAS",
        "quote_currency": "USDT",
        "underlying_unit": "mmBtu",
        "venue_hint": "binance_usds_m",
    }
)

NATGASUSDT_SPECS = SymbolSpecs(
    symbol="NATGASUSDT",
    contract_size=1.0,
    pip_value=0.001,
    lot_step=0.01,
...
_exchange_info_synced: bool = False

def _sync_exchange_info_once() -> None:
    global _exchange_info_synced
    if _exchange_info_synced:
        return
    ...
    with _exchange_info_lock:
        ...
        try:
            ...
            req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 (TradeScanner)'})
            with urllib.request.urlopen(req, timeout=3.0) as response:
                if response.status == 200:
                    ...
                    for sym_data in symbols:
                        ...
                        for f in sym_data.get("filters", []):
                            if f.get("filterType") == "PRICE_FILTER":
                                ...
                                if "." in tick_size:
                                    p_dec = len(tick_size.rstrip("0").split(".")[1])
                                else:
                                    p_dec = 0
                            elif f.get("filterType") == "LOT_SIZE":
                                min_lot = float(f.get("minQty", "1.0"))
                                step_size = f.get("stepSize", "1.0")
                                if "." in step_size:
                                    v_dec = len(step_size.rstrip("0").split(".")[1])
                                else:
                                    v_dec = 0
                        
                        if sym in INSTRUMENT_SPECS:
                            spec = INSTRUMENT_SPECS[sym]
                            INSTRUMENT_SPECS[sym] = replace(
                                spec,
                            ...
                            INSTRUMENT_SPECS[sym] = SymbolSpecs(
                                symbol=sym,
                    ...
                    logger.debug(f"Synced {updated_count} symbol specs from Binance exchangeInfo")
                    _exchange_info_synced = True
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"Failed to fetch exchangeInfo: {e}")
            _exchange_info_synced = True
