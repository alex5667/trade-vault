...
class SnapshotBuilder:
    ...
    def _key(self, tpl: str, symbol: str) -> str:
        """Форматирует ключ Redis, подставляя symbol.

        Поддерживает два формата placeholder:
          - ``{symbol}``  (canonical, preferred)
          - ``{SYMBOL}``  (legacy / deprecated)

        .. deprecated::
            Использование ``{SYMBOL}`` (uppercase) — deprecated.
            Мигрируйте все шаблоны на ``{symbol}`` (lowercase) и удалите
            UPPERCASE-branch после того, как все callers будут переведены.
            Трекается в TODO: [MIGRATION] snapshot_builder._key legacy removal.
        """
        if "{SYMBOL}" in tpl:
            warnings.warn(
                f"Template '{tpl}' uses deprecated '{{SYMBOL}}' placeholder. "
                "Migrate to '{{symbol}}' (lowercase). "
                "See TODO: [MIGRATION] snapshot_builder._key legacy removal.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def _get_last_tick(self, symbol: str) -> Optional[Tick]:
        ...
        for i in range(max_retries):
            try:
                ...
                return None
            except (redis.exceptions.BusyLoadingError, redis.exceptions.ResponseError) as e:
                if "LOADING" in str(e):
                    if i < max_retries - 1:
                        time.sleep(2.0)
                        continue
                self.log.warning(f"Error getting last tick: {e}")
                return None
            except Exception as e:
                self.log.warning(f"Unexpected error getting last tick: {e}")
                return None
        return None
