...
# ---------------------------------------------------------------------------
# GPU backend detection — LAZY xgboost import to avoid CUDA init deadlock
# in containers with nvidia runtime. xgboost is imported only at runtime
# inside _fit_gbdt() when GPU training is actually needed.
# ---------------------------------------------------------------------------

# Respect GPU_ENABLED env var (set GPU_ENABLED=0 for forced CPU-only)
def _parse_bool_env(val: str) -> bool:
    """Parse env var that may be '1', '0', 'true', 'false', etc."""
    return str(val).strip().lower() in ("1", "true", "yes", "on")

_GPU_ENABLED_ENV = _parse_bool_env(os.getenv("GPU_ENABLED", "1") or "1")


def _check_gpu_xgb() -> bool:
    """Lazy GPU+XGBoost availability check (called once at training time).

    Defers heavy imports (xgboost, torch, cupy) to avoid module-level
    CUDA init deadlocks in nvidia container runtime environments.
    """
    if not _GPU_ENABLED_ENV:
        return False
    try:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--p_min", type=float, default=0.52)
    ...
    ap.add_argument("--weight_by_rmult", type=int, default=1, help="If 1, applies sample weights relative to abs(r_mult)")
    ...
    # ------------------------------------------------------------------
    # _fit_gbdt: XGBoost device=cuda when GPU available; sklearn fallback
    # ------------------------------------------------------------------
    _gpu_checked = [None]  # mutable cache for lazy GPU check

    def _fit_gbdt(X_tr: np.ndarray, y_tr: np.ndarray, w_tr: Optional[np.ndarray] = None) -> Any:
        import xgboost as xgb
        try:
            model = xgb.XGBClassifier(
                n_estimators=int(args.gbdt_max_iter),
    ...
    for i, r in enumerate(rows):
        ...
        y.append(int(yy))
        w.append(float(np.clip(abs(_f(r.get("r_mult"), 0.1) or 0.1), 0.1, 10.0)))
    ...
    w_arr = np.asarray(w, dtype=np.float32) if int(args.weight_by_rmult) == 1 else None
    ...
    for tr_idx, va_idx in splitter.split(ts):
        ...
        lr.fit(X_tr, y_tr, **({"lr__sample_weight": w_tr} if w_tr is not None else {}))
        gbdt = _fit_gbdt(X_tr, y_tr, w_tr=w_tr)
    ...
    y_z = y_arr[mask]
    w_z = w_arr[mask] if w_arr is not None else None

    MIN_GBDT_SAMPLES = int(os.environ.get("EDGE_STACK_GBDT_MIN_SAMPLES", "5000"))
    
    meta = LogisticRegression(random_state=42)
    meta.classes_ = np.array([0, 1], dtype=int)

    from scipy.optimize import minimize

    def _meta_loss(w):
        # w[0]=LR_coef, w[1]=GBDT_coef, w[2]=intercept
        p = 1.0 / (1.0 + np.exp(-(np.dot(Z, w[:2]) + w[2])))
        p = np.clip(p, 1e-15, 1 - 1e-15)
        if w_z is not None:
            return np.average(- (y_z * np.log(p) + (1 - y_z) * np.log(1 - p)), weights=w_z)
        return -np.mean(y_z * np.log(p) + (1 - y_z) * np.log(1 - p))

    if len(X) >= MIN_GBDT_SAMPLES:

        # Train meta-stacker with bound constraints (NNLS) to strictly prevent negative model correlation
        res = minimize(_meta_loss, [1.0, 1.0, 0.0], bounds=[(0.0, 20.0), (0.0, 20.0), (-20.0, 20.0)])
        meta.coef_ = np.array([res.x[:2]])
        meta.intercept_ = np.array([res.x[2]])
    else:
        # Train meta-stacker with bound constraints (NNLS) to strictly prevent negative model correlation
        # And force GBDT coefficient to 0.0, avoiding XGBoost tiny-dataset instabilities, while 
        # optimizing LR coefficient and intercept so probability mapping remains natively calibrated.
        res = minimize(_meta_loss, [1.0, 0.0, 0.0], bounds=[(0.0, 20.0), (0.0, 0.0), (-20.0, 20.0)])
        meta.coef_ = np.array([res.x[:2]])
        meta.intercept_ = np.array([res.x[2]])

    # Define a helper predict_proba function for the training script's internal calc
    p_meta_oof = 1.0 / (1.0 + np.exp(-(np.dot(Z, meta.coef_[0]) + meta.intercept_[0])))
    ...
    base_gbdt = _fit_gbdt(X, y_arr, w_tr=w_arr)
    base_lr.fit(X, y_arr, **({"lr__sample_weight": w_arr} if w_arr is not None else {}))
    ...
    if int(args.calibrate) == 1 and fit_platt_logit is not None:
        # p_meta_oof already computed above using custom math
    ...
    if logloss is not None and brier_score is not None and ece_score is not None:
        ...
        # Apply calibrator to evaluate true metrics
        p_eval_meta = p_meta_oof
        if calibrator_dict is not None and int(args.calibrate) == 1:
            from services.ml_calibration import sigmoid, clip_prob, logit
            # Apply exactly what PlattLogitCalibrator does
            a_cal = float(calibrator_dict.get("a", 1.0))
            b_cal = float(calibrator_dict.get("b", 0.0))
            p_eval_meta = np.array([sigmoid(a_cal * logit(clip_prob(float(x))) + b_cal) for x in p_meta_oof])
        ...
        report["oof"] = {
            ...
            "meta": {
                "logloss": float(logloss([float(x) for x in p_eval_meta], [int(x) for x in y_z])),
                "brier": float(brier_score([float(x) for x in p_eval_meta], [int(x) for x in y_z])),
                "precision_top5pct": float(_precision_at_top_k(p_eval_meta, y_z, 0.05)),
            },
        }
        ece, bins = ece_score([float(x) for x in p_eval_meta], [int(x) for x in y_z])
