...
def test_edge_stack_spec_v4_of():
    ...
    assert len(spec.feature_cols_hash) == 16, "feature_cols_hash должен быть 16 символов"
...
def test_v10_of_schema_smoke():
    """v10_of must have >=165 numeric keys, schema resolves, hashes stable."""
    reg = _import_registry()
    info = reg.get_schema_info("v10_of")
    assert info.ver == "v10_of"
    assert isinstance(info.feature_names, list)
    # v10_of: 165 numeric + direction/bucket/hour/dow/session one-hots
    assert len(info.feature_names) > 200, (
        f"v10_of only has {len(info.feature_names)} feature_names, expected > 200"
    )
    ...
    # colon-safe
    bad = [c for c in info.column_names if ":" in c]
    assert not bad, f"v10_of column_names contain ':': {bad[:5]}"
    # direction + bucket
    assert any(x.startswith("dir") for x in info.feature_names)
    assert any(x.startswith("bucket") for x in info.feature_names)


def test_v10_of_edge_stack_feature_cols():
    """v10_of EdgeStackFeatureSpec has >= 220 cols, correct f_* keys."""
    reg = _import_registry()
    spec = reg.get_edge_stack_feature_spec("v10_of")
    assert spec.ver == "v10_of"
    assert len(spec.feature_cols) >= 200, (
        f"v10_of only has {len(spec.feature_cols)} feature_cols, expected >= 200"
    )
    ...
    # spot-check new v10 keys
    for key in ("f_vpin_rolling", "f_rsi_price", "f_spread_bps",
                "f_microbar_range_bps", "f_mae_r", "f_btc_corr_5m",
                "f_book_slope_bid", "f_price_to_ema_bps", "f_momentum_10s"):
        assert key in spec.feature_cols, f"v10_of edge-spec missing expected key: {key}"


def test_v10_of_alias_resolves():
    """'v10' must resolve to 'v10_of' with identical schema_hash."""
    reg = _import_registry()
    full = reg.get_schema_info("v10_of")
    alias = reg.get_schema_info("v10")
    assert alias.ver == "v10_of"
    assert alias.schema_hash == full.schema_hash


def test_v10_of_hash_stable():
    """v10_of schema_hash and feature_cols_hash are stable across repeated calls."""
    reg = _import_registry()
    h1 = reg.get_schema_info("v10_of").schema_hash
    h2 = reg.get_schema_info("v10_of").schema_hash
    assert h1 == h2
    c1 = reg.get_edge_stack_feature_spec("v10_of").feature_cols_hash
    c2 = reg.get_edge_stack_feature_spec("v10_of").feature_cols_hash
    assert c1 == c2
