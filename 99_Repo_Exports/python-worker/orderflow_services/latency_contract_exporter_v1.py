...
latency_contract_stage_p95_ms = Gauge(
    "latency_contract_stage_p95_ms",
    "P95 of observed latency-contract stage durations in ms (sliding window)",
    ["service", "stage", "symbol"],
)

latency_contract_stage_p99_ms = Gauge(
    "latency_contract_stage_p99_ms",
    "P99 of observed latency-contract stage durations in ms (sliding window)",
    ["service", "stage", "symbol"],
)
...
_SLIDING_WINDOWS = collections.defaultdict(lambda: collections.deque(maxlen=500))
...
def _observe_histograms_if_fresh(obs_key: str, service: str, stage: str, symbol: str, row: Dict[str, Any]) -> None:
    ...
    win = _SLIDING_WINDOWS[obs_key]
    win.append(dur_ms)
    p95 = np.percentile(win, 95)
    p99 = np.percentile(win, 99)
    latency_contract_stage_p95_ms.labels(service=service, stage=stage, symbol=symbol).set(float(p95))
    latency_contract_stage_p99_ms.labels(service=service, stage=stage, symbol=symbol).set(float(p99))
...
async def _scrape_once(redis_client: Any, prefix: str, stale_s: int, budgets: Dict[str, int]) -> None:
    ...
    for service, stage in required_stage_owners():
        for symbol in symbols:
            keys.append(f"{prefix}:{service}:{stage}:{symbol}")
