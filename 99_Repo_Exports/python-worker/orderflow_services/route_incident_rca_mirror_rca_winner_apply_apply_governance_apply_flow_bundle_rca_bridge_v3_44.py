...
APP_NAME = "route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_bundle_rca_bridge_v3_44"
...
DECISIONS_STREAM = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_DECISIONS_STREAM",
    "stream:ml:route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge_decisions",
)
AUDIT_STREAM = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_AUDIT_STREAM",
    "stream:ml:route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge_audit",
)
LAST_HASH = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_LAST_HASH",
    "metrics:ml:route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge:last",
)
...
VERTEX_HEALTH_HASH = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_VERTEX_HEALTH_HASH",
    "metrics:ml:vertex_health:last",
)
GROUP = os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_GROUP", APP_NAME)
CONSUMER = os.getenv("HOSTNAME", APP_NAME)
PORT = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_PORT", "9974"))
MAXLEN = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_MAXLEN", "20000"))

DEFAULT_MODE = os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_MODE", "AUTO").upper()
DEFAULT_REQUIRE_VERTEX_DEGRADED_FOR_LOCAL = int(
    os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_REQUIRE_VERTEX_DEGRADED_FOR_LOCAL", "1")
)
DEFAULT_MAX_BUNDLE_BYTES = int(
    os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_MAX_BUNDLE_BYTES", "196608")
)
DEFAULT_MAX_PROMPT_CHARS = int(
    os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_MAX_PROMPT_CHARS", "12000")
)
...
RUNS = _counter(
    "ml_route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge_runs_total",
    "Winner-apply apply governance apply-flow RCA bridge runs",
    ("status", "decision"),
)
LAT = _hist(
    "ml_route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge_latency_seconds",
    "Winner-apply apply governance apply-flow RCA bridge latency seconds",
)
UP = _gauge(
    "ml_route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge_up",
    "Winner-apply apply governance apply-flow RCA bridge up",
)
LAST_RUN_TS = _gauge(
    "ml_route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge_last_run_ts_seconds",
    "Winner-apply apply governance apply-flow RCA bridge last run timestamp",
)
ROUTED = _counter(
    "ml_route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca_bridge_routed_total",
    "Winner-apply apply governance apply-flow RCA bridge routed total",
    ("route", "severity"),
)
...
def build_vertex_prompt(bundle: Dict[str, Any]) -> str:
    return (
        ...
        "retry exhaustion, SLO and MTTR pressure, escalation severity, and bounded next actions."
    )
...
def build_local_prompt(bundle: Dict[str, Any]) -> str:
    return (
        ...
        "Perform bounded RCA summarization for this route_incident_rca mirror RCA winner-apply apply governance apply-flow incident bundle. "
        "Focus on apply-controller intent, live policy mismatch, verification outcomes, rollback causes, "
        "retry exhaustion, SLO and MTTR pressure, escalation severity, and bounded next checks."
    )
...
def build_vertex_request(bundle: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "task_family": "route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca",
        "task_type": "route_incident_rca_mirror_rca_winner_apply_apply_governance_apply_flow_rca",
            ...
            for msg_id, payload in messages:
                ...
                try:
                    ...
                    if decision["decision"] == "ROUTE_VERTEX":
                        ...
                        await r.xadd(VERTEX_OUTPUT_STREAM, build_vertex_request(bundle), maxlen=MAXLEN, approximate=True)
                    elif decision["decision"] == "ROUTE_LOCAL":
                        destination_stream = LOCAL_OUTPUT_STREAM
                        await r.xadd(LOCAL_OUTPUT_STREAM, build_local_request(bundle), maxlen=MAXLEN, approximate=True)
                    ...
                    await r.xadd(
                        AUDIT_STREAM,
                        {"event_type": "ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_GOVERNANCE_APPLY_FLOW_RCA_BRIDGE_DECIDED", **out},
                        maxlen=MAXLEN,
                        approximate=True,
                    )
                    await r.hset(
