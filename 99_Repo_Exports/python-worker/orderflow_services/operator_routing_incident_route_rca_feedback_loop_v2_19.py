...
PORT = int(os.getenv("ML_OPERATOR_ROUTING_INCIDENT_ROUTE_RCA_FEEDBACK_PORT", "9904"))

IN_STREAM = "stream:ml:operator_routing_incident_rca_route_rca_feedback"
OUT_STREAM = "stream:ml:operator_routing_incident_rca_route_rca_feedback_summary"

GROUP = "operator_routing_incident_route_rca_feedback_v2_19"
...
RUNS = _counter("ml_operator_routing_incident_route_rca_feedback_runs_total", "Runs", ("status",))
LAT = _hist("ml_operator_routing_incident_route_rca_feedback_latency_seconds", "Latency")
LAST_RUN = _gauge("ml_operator_routing_incident_route_rca_feedback_last_run_ts_seconds", "Last timestamp")
        ...
        for stream_name, records in messages:
            for msg_id, payload in records:
                try:
                    ...
                    usefulness_str = row.get("usefulness", "MIXED")
                    score = map_useful.get(usefulness_str, 0.5)

                    rep = {
                        "incident_id": inc_id,
                        "usefulness_score": score,
                        "base_usefulness": usefulness_str,
                        "ts_ms": now_ms()
                    }

                    await r.xadd(OUT_STREAM, rep, maxlen=MAXLEN, approximate=True)
