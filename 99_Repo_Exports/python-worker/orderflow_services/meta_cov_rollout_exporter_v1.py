...
class Exporter:
    ...
    def _read_recent(self) -> List[Dict[str, Any]]:
        ...
        batch_size = min(2000, self.max_scan)
        total_read = 0
        last_id = "+"
        try:
            while total_read < self.max_scan:
                fetch = min(batch_size, self.max_scan - total_read)
                batch = self.r.xrevrange(self.stream, max=last_id, min=str(since_ms), count=fetch)
                if not batch:
                    break
                ...
                # Advance cursor: last entry id - 1 (exclusive)
                raw_last = batch[-1][0]
                last_id_str = raw_last.decode() if isinstance(raw_last, bytes) else str(raw_last)
                # Decrement sequence to exclude current last entry
                ts_part, seq_part = last_id_str.split("-", 1)
                new_seq = int(seq_part) - 1
                if new_seq < 0:
                    new_seq = 0
                    new_ts = int(ts_part) - 1
                    if new_ts < since_ms:
                        break
                    last_id = f"{new_ts}-18446744073709551615"
                else:
                    last_id = f"{ts_part}-{new_seq}"
                time.sleep(0.002)  # yield 2ms between batches
        except Exception:
            pass
        return rows
    ...
    def step(self) -> None:
        ...
        GAUGE_SQ_N.set(float(_i(cfg2.get("signal_quality_n_24h"), 0)))
        ...
        # P70: per-policy-mode gauges (ok/warn/block/unknown)
        for _mode in ("ok", "warn", "block", "unknown"):
            GAUGE_SQ_EXPECTANCY_R_24H_BY_POLICY_MODE.labels(mode=_mode).set(_f(cfg2.get(f"signal_quality_expectancy_r_24h_policy_{_mode}"), 0.0))
            GAUGE_SQ_PRECISION_TOP5P_24H_BY_POLICY_MODE.labels(mode=_mode).set(_f(cfg2.get(f"signal_quality_precision_top5p_24h_policy_{_mode}"), 0.0))
            GAUGE_SQ_ECE_24H_BY_POLICY_MODE.labels(mode=_mode).set(_f(cfg2.get(f"signal_quality_ece_24h_policy_{_mode}"), 0.0))
            GAUGE_SQ_N_24H_BY_POLICY_MODE.labels(mode=_mode).set(float(_i(cfg2.get(f"signal_quality_n_24h_policy_{_mode}"), 0)))
        ...
        if pe_ts > 0:
            GAUGE_POLICY_EFF_STALENESS_SEC.set((_now - pe_ts) / 1000.0)
        else:
            GAUGE_POLICY_EFF_STALENESS_SEC.set(0.0)

        GAUGE_POLICY_EFF_BASELINE_OK_PRESENT.set(float(_i(cfg2.get("policy_effectiveness_baseline_ok_present"), 0)))

        for _mode in ("ok", "warn", "block", "unknown"):
            GAUGE_POLICY_EFF_SHARE_24H.labels(mode=_mode).set(_f(cfg2.get(f"policy_effectiveness_share_24h_{_mode}"), 0.0))
            GAUGE_POLICY_EFF_EXPECTANCY_R_DELTA_24H.labels(mode=_mode).set(_f(cfg2.get(f"policy_effectiveness_expectancy_r_delta_24h_{_mode}"), 0.0))
            GAUGE_POLICY_EFF_PRECISION_TOP5P_DELTA_24H.labels(mode=_mode).set(_f(cfg2.get(f"policy_effectiveness_precision_top5p_delta_24h_{_mode}"), 0.0))
            GAUGE_POLICY_EFF_ECE_DELTA_24H.labels(mode=_mode).set(_f(cfg2.get(f"policy_effectiveness_ece_delta_24h_{_mode}"), 0.0))

        # P72: Policy regime effectiveness report (within-cell deltas)
        pre_ts = _i(cfg2.get("policy_regime_effectiveness_last_ts_ms"), 0)
        GAUGE_POLICY_REGIME_EFF_LAST_TS_MS.set(float(pre_ts))
        if pre_ts > 0:
            GAUGE_POLICY_REGIME_EFF_STALENESS_SEC.set((_now - pre_ts) / 1000.0)
        else:
            GAUGE_POLICY_REGIME_EFF_STALENESS_SEC.set(0.0)

        GAUGE_POLICY_REGIME_EFF_CELLS_TOTAL.set(float(_i(cfg2.get("policy_regime_effectiveness_cells_total"), 0)))
        GAUGE_POLICY_REGIME_EFF_CELLS_OK_BASELINE.set(float(_i(cfg2.get("policy_regime_effectiveness_cells_ok_baseline"), 0)))

        GAUGE_POLICY_REGIME_EFF_WORST_WARN_EXPECTANCY_DELTA.set(_f(cfg2.get("policy_regime_effectiveness_worst_warn_expectancy_r_delta"), 0.0))
        GAUGE_POLICY_REGIME_EFF_WORST_WARN_PRECISION_DELTA.set(_f(cfg2.get("policy_regime_effectiveness_worst_warn_precision_top5p_delta"), 0.0))
        GAUGE_POLICY_REGIME_EFF_WORST_WARN_ECE_DELTA.set(_f(cfg2.get("policy_regime_effectiveness_worst_warn_ece_delta"), 0.0))

        GAUGE_POLICY_REGIME_EFF_WORST_BLOCK_EXPECTANCY_DELTA.set(_f(cfg2.get("policy_regime_effectiveness_worst_block_expectancy_r_delta"), 0.0))
        GAUGE_POLICY_REGIME_EFF_WORST_BLOCK_PRECISION_DELTA.set(_f(cfg2.get("policy_regime_effectiveness_worst_block_precision_top5p_delta"), 0.0))
        GAUGE_POLICY_REGIME_EFF_WORST_BLOCK_ECE_DELTA.set(_f(cfg2.get("policy_regime_effectiveness_worst_block_ece_delta"), 0.0))


        # P74: Policy calibration suggester (advisory)
        pcs_ts = _i(cfg2.get("policy_calibration_suggest_last_ts_ms"), 0)
        GAUGE_POLICY_CAL_SUG_LAST_TS_MS.set(float(pcs_ts))
        GAUGE_POLICY_CAL_SUG_STALENESS_SEC.set(_f(cfg2.get("policy_calibration_suggest_staleness_sec"), 0.0))
        GAUGE_POLICY_CAL_SUG_INPUTS_STALE.set(float(_i(cfg2.get("policy_calibration_suggest_inputs_stale"), 0)))
        GAUGE_POLICY_CAL_SUG_OK_BASELINE_PRESENT.set(float(_i(cfg2.get("policy_calibration_suggest_ok_baseline_present"), 0)))

        GAUGE_POLICY_CAL_SUG_WARN_ACTION_CODE.set(float(_i(cfg2.get("policy_calibration_suggest_warn_action_code"), 0)))
        GAUGE_POLICY_CAL_SUG_WARN_SEVERITY.set(_f(cfg2.get("policy_calibration_suggest_warn_severity"), 0.0))
        GAUGE_POLICY_CAL_SUG_WARN_SHARE_24H.set(_f(cfg2.get("policy_calibration_suggest_warn_share_24h"), 0.0))

        GAUGE_POLICY_CAL_SUG_BLOCK_ACTION_CODE.set(float(_i(cfg2.get("policy_calibration_suggest_block_action_code"), 0)))
        GAUGE_POLICY_CAL_SUG_BLOCK_SEVERITY.set(_f(cfg2.get("policy_calibration_suggest_block_severity"), 0.0))
        GAUGE_POLICY_CAL_SUG_BLOCK_SHARE_24H.set(_f(cfg2.get("policy_calibration_suggest_block_share_24h"), 0.0))
        GAUGE_POLICY_CAL_SUG_UNKNOWN_SHARE_24H.set(_f(cfg2.get("policy_calibration_suggest_unknown_share_24h"), 0.0))

        sq_ts = _i(cfg2.get("signal_quality_last_ts_ms"), 0)
        GAUGE_SQ_LAST_TS_MS.set(float(sq_ts))
        if sq_ts > 0:
            GAUGE_SQ_STALENESS_SEC.set((_now - sq_ts) / 1000.0)
        else:
            GAUGE_SQ_STALENESS_SEC.set(0.0)
