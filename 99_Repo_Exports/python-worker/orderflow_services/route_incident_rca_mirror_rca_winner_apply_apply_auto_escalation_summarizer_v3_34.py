...
APP_NAME = "route_incident_rca_mirror_rca_winner_apply_apply_auto_escalation_summarizer_v3_34"
...
LAST_HASH = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_ESCALATIONS_LAST_HASH",
    "metrics:ml:route_incident_rca_mirror_rca_winner_apply_apply_escalations:last",
)
AUDIT_STREAM = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_ESCALATIONS_AUDIT_STREAM",
    "stream:ml:route_incident_rca_mirror_rca_winner_apply_apply_escalations_audit",
)
PORT = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_ESCALATIONS_PORT", "9961"))
MAXLEN = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_ESCALATIONS_MAXLEN", "20000"))
RUN_EVERY_SEC = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_ESCALATIONS_RUN_EVERY_SEC", "300"))
...
RUNS = _counter("ml_route_incident_rca_mirror_rca_winner_apply_apply_escalations_runs_total", "Winner-apply apply escalation runs", ("status", "severity"))
LAT = _hist("ml_route_incident_rca_mirror_rca_winner_apply_apply_escalations_latency_seconds", "Winner-apply apply escalation latency seconds")
UP = _gauge("ml_route_incident_rca_mirror_rca_winner_apply_apply_escalations_up", "Winner-apply apply escalation up")
LAST_RUN_TS = _gauge("ml_route_incident_rca_mirror_rca_winner_apply_apply_escalations_last_run_ts_seconds", "Winner-apply apply escalation last run ts")
...
async def main() -> None:  # pragma: no cover
    ...
    while True:
        ...
        try:
            ...
            await r.xadd(AUDIT_STREAM, {"event_type": "ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_ESCALATION_SUMMARY", "severity": severity, "ts_ms": str(now_ms())}, maxlen=MAXLEN, approximate=True)
            if LAST_RUN_TS:
                LAST_RUN_TS.set(time.time())
        except Exception as exc:
            status = "error"
            await r.xadd(AUDIT_STREAM, {"event_type": "ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_ESCALATION_FAILED", "error": str(exc), "ts_ms": str(now_ms())}, maxlen=MAXLEN, approximate=True)
        finally:
            if RUNS:
