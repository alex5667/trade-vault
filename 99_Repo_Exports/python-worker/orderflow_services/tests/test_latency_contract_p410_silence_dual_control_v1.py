...
class TestValidateApprovalForAck:
    # Use far-future timestamps so P4.11 freshness deadlines remain valid
    _T0 = 9_000_000_000_000  # ~year 2255 in ms
    ...
    def _approved_raw(self, r, prefix='appr', operator='alice', purpose='rollout',
        ...
        raw = prepare_override_approval(
            ...
            minutes=minutes, ttl_s=604800, ops_stream=None,
            prepared_freshness_s=7200,
            now_ms=self._T0,
        )
        rid = raw['request_id']
        approve_override_approval(
            r, prefix=prefix, request_id=rid, operator='bob',
            reason='ok', ttl_s=604800, ops_stream=None,
            prepared_freshness_s=7200,
            approved_freshness_s=1800,
            now_ms=self._T0 + 1,
        )
    ...
    def test_valid_approval_accepted(self):
        ...
        v = validate_approval_for_ack(raw, purpose='rollout', operator='alice', ticket='T-1', escalation_ticket='ESC-1', minutes=600, now_ms=self._T0 + 2)
        assert v.ok

    def test_missing_approval_rejected(self):
        v = validate_approval_for_ack({}, purpose='rollout', operator='alice', ticket='T-1', escalation_ticket='ESC-1', minutes=600, now_ms=self._T0 + 2)
        assert not v.ok
    ...
    def test_wrong_minutes_rejected(self):
        ...
        v = validate_approval_for_ack(raw, purpose='rollout', operator='alice', ticket='T-1', escalation_ticket='ESC-1', minutes=700, now_ms=self._T0 + 2)
        assert not v.ok
    ...
    def test_wrong_operator_rejected(self):
        ...
        v = validate_approval_for_ack(raw, purpose='rollout', operator='charlie', ticket='T-1', escalation_ticket='ESC-1', minutes=600, now_ms=self._T0 + 2)
        assert not v.ok
