...
def emit_train_report(symbol: str, cal_type: str, schema_version: int, raw_ece: float, cal_ece: float, raw_brier: float, cal_brier: float, raw_metrics: Optional[dict] = None, cal_metrics: Optional[dict] = None) -> None:
    ...
    try:
        ...
        # Extended calibration quality metrics (MCE, slope, intercept, sharpness, near-half pmass)
        def _set_opt(metric, value) -> None:
            try:
                if value is None:
                    return
                lm = _safe_labels(metric, symbol=s)
                if lm is not None:
                    lm.set(float(value))
            except Exception:
                pass

        raw_m = dict(raw_metrics or {})
        cal_m = dict(cal_metrics or {})
        _set_opt(confidence_cal_train_mce_raw_gauge,                  raw_m.get("mce"))
        _set_opt(confidence_cal_train_mce_cal_gauge,                  cal_m.get("mce"))
        _set_opt(confidence_cal_train_slope_raw_gauge,                raw_m.get("calibration_slope"))
        _set_opt(confidence_cal_train_slope_cal_gauge,                cal_m.get("calibration_slope"))
        _set_opt(confidence_cal_train_intercept_raw_gauge,            raw_m.get("calibration_intercept"))
        _set_opt(confidence_cal_train_intercept_cal_gauge,            cal_m.get("calibration_intercept"))
        _set_opt(confidence_cal_train_sharpness_mean_raw_gauge,       raw_m.get("sharpness_mean"))
        _set_opt(confidence_cal_train_sharpness_mean_cal_gauge,       cal_m.get("sharpness_mean"))
        _set_opt(confidence_cal_train_sharpness_entropy_raw_gauge,    raw_m.get("sharpness_entropy"))
        _set_opt(confidence_cal_train_sharpness_entropy_cal_gauge,    cal_m.get("sharpness_entropy"))
