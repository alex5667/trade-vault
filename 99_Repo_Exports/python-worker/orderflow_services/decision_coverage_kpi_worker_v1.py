...
def _process_one(
    ...
    dq_s_raw = fields.get("dq_state")
    dr_s_raw = fields.get("drift_state")

    # Fallback to map new schema fields if missing
    ...
    dq_state = _parse_json_maybe(dq_s_raw if dq_s_raw is not None else "unknown")
    drift_state = _parse_json_maybe(dr_s_raw if dr_s_raw is not None else "unknown")
