...
_ROLE_URL_ENV: dict[str, str] = {
    'writer': 'EXEC_HEALTH_REDIS_WRITER_URL',
    'audit': 'EXEC_HEALTH_REDIS_AUDIT_URL',
    'bootstrap': 'EXEC_HEALTH_REDIS_BOOTSTRAP_URL',
}

_FALLBACK_REDIS_URL = os.getenv('REDIS_URL', 'redis://redis-worker-1:6379/0')


def _default_redis_url_for_service(service: str) -> str:
    """Return the role-appropriate Redis URL env var for a given service.
    ...
    try:
        ...
        env_var = _ROLE_URL_ENV.get(expected.role, '')
        if env_var:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # redis-url is resolved AFTER --service is parsed so we can pick the role-correct URL.
    # The default below is a fallback; main() overrides it if --redis-url was not explicitly set.
...
def main() -> None:
    ...
    # Resolve the redis URL: if not explicitly provided, pick the role-correct URL.
    redis_url = args.redis_url or _default_redis_url_for_service(args.service)
    h = ChaosHarness(redis_url, service=args.service, wrong_user_url=args.wrong_user_url)
