...
try:
    from orderflow_services.auto_apply_guard import DEFAULT_REASONS as AUTO_APPLY_REASONS
except:
    AUTO_APPLY_REASONS = "tick_gate,enforce_bucket_promoter,meta_cov,prom_rules_bundle_smoke,prom_rules_loaded_probe,of_inputs_v3,of_inputs_exporters_smoke,of_gate_exporters_smoke"
...
def _iter_scan(r, pattern: str) -> Iterable[str]:
    # No longer used to prevent Redis CPU saturation. 
    # Use deterministic key generation instead.
    return []
...
def main() -> None:
    ...
    symbols_allow = _parse_csv(os.environ.get("OF_INPUTS_V3_CIRCUIT_EXPORTER_SYMBOLS", "") or ",".join(default_symbol_allowlist()), upper=True)
    reasons_allow = _parse_csv(os.environ.get("OF_INPUTS_V3_CIRCUIT_EXPORTER_REASONS", "") or AUTO_APPLY_REASONS, upper=False)
    ...
    while True:
        ...
        try:
            # ---- cfg disabled
            disabled_keys = [CFG_DISABLED_PREFIX + s for s in symbols_allow]
            ...
            # ---- downgrades zsets
            dg_keys = [STATE_DG_PREFIX + rsn + ":" + sym for sym in symbols_allow for rsn in reasons_allow]
            ...
            for rsn in reasons_allow:
                k = AP_GLOB_PREFIX + rsn
            ...
            for sym in symbols_allow:
                for rsn in reasons_allow:
                    k = AP_SYM_PREFIX + sym + ":" + rsn
                    ttl = _as_int(r.pttl(k), -2)
                    if ttl != -2:
