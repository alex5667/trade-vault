...
def main() -> int:
    ...
    allow_early_unquarantine = bool(int(os.environ.get("META_COV_ALLOW_EARLY_UNQUARANTINE", "0") or 0))
    ...
    # P34: Quarantine guard - do not allow raising share for a bucket that is still quarantined
    # unless the same patch explicitly clears the quarantine (or force=true).
    if not args.force:
        for _b in ("a", "b", "c", "d"):
            q_active = _i(cfg2.get(f"meta_cov_quarantine_{_b}"), 0)
            q_until = _i(cfg2.get(f"meta_cov_quarantine_until_ms_{_b}"), 0)
            if q_active != 1 or q_until <= now_ts:
                continue

            # Is this patch explicitly clearing quarantine now?
            clearing = False
            if (f"meta_cov_quarantine_{_b}" in patch2) or (f"meta_cov_quarantine_until_ms_{_b}" in patch2):
                pv = patch2.get(f"meta_cov_quarantine_{_b}")
                pu = patch2.get(f"meta_cov_quarantine_until_ms_{_b}")
                if _i(pv, 1) == 0 and _i(pu, 1) == 0:
                    clearing = True

            if clearing and (not allow_early_unquarantine):
                print(json.dumps({
                    "ok": 0,
                    "sid": sid,
                    "reason": "early_unquarantine_blocked",
                    "bucket": _b,
                    "quarantine_until_ms": q_until,
                }))
                return 0

            prop_share = patch2.get(f"meta_enforce_share_cov_{_b}")
            if prop_share is not None and _f(prop_share, 0.0) > 0.0 and (not clearing):
                print(json.dumps({
                    "ok": 0,
                    "sid": sid,
                    "reason": "bucket_quarantined",
                    "bucket": _b,
                    "quarantine_until_ms": q_until,
                    "proposed_share": _f(prop_share, 0.0),
                }))
                return 0
