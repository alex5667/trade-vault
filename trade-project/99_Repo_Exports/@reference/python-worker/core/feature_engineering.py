...
def apply_robust_scale(x: float, *, center: float, scale: float, eps: float = 1e-9) -> float:
    """Robust scaling: (x-center)/scale, where scale is typically MAD*1.4826 or IQR/1.349."""
    sc = float(scale)
    if not math.isfinite(sc) or abs(sc) < eps:
        sc = 1.0
    return (float(x) - float(center)) / sc
