...
Tests cover:
  - _load_order_state triages to _recover_state_from_exec_stream when key missing
  - rehydrated state is persisted back to Redis
  - _resume_open_from_state works correctly on rehydrated snapshots
  - exec_event emitted for rehydrate and resume_from_materialized_state
"""
...
# ── Path bootstrap ───────────────────────────────────────────────────────────
# Add python-worker to sys.path so that `services.*` imports resolve when the
# executor is loaded directly (outside Docker / package-install).
_WORKER_ROOT = Path(__file__).parent.parent / "python-worker"
if str(_WORKER_ROOT) not in sys.path:
    sys.path.insert(0, str(_WORKER_ROOT))


def _load_module(name: str, rel_path: str):
    """Load a python-worker/services module by name + relative path."""
...
# Pre-load the replay module so executor can import it in the try/except block.
_load_module("execution_state_replay", "services/execution_state_replay.py")
_load_module("services.execution_state_replay", "services/execution_state_replay.py")

# Load binance_executor directly (no package install required)
mod_path = _WORKER_ROOT / "services" / "binance_executor.py"
