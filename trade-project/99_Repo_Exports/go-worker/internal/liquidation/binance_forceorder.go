...
type binanceForceOrderMsg struct {
	EventType string               `json:"e"` // Go 1.26+: explicit field prevents case-insensitive collision with "E"
...
func ParseBinanceForceOrder(raw []byte, recvTsMs int64) (NormalizedEvent, error) {
	...
	if err := json.Unmarshal(raw, &msg); err != nil {
		cnt := atomic.AddUint64(&debugCounter, 1)
		if cnt <= 5 || cnt%1000 == 0 {
			fmt.Printf("[liq] parse error #%d: raw(%d bytes): %s err: %v\n", cnt, len(raw), string(raw[:min(len(raw), 500)]), err)
		}
