...
func newTestController() *stream.Controller {
	...
	c := stream.NewController(
		...
		func(symbols []string, logger *zap.SugaredLogger) stream.ExchangeManager {
			return NewFuturesMultiplexManager(symbols, logger)
		},
		&Normalizer{},
		nil,
	)
...
// Mocks tested generic stream logic.
func TestReconcile_NoOpOnSameSymbols(t *testing.T) {
	...
	defer c.Stop()
	// Next attempt... (would require breaking unexported fields on controller if testing internal behavior)
}
