#!/usr/bin/env python3
"""
Telegram Stream Reader - читает сигналы из notify:telegram и отправляет в Telegram bot
"""
import os
import time
import redis
...
CONSUMER_GROUP = "telegram-sender-group"
CONSUMER_NAME = "telegram-sender-1"

print("🚀 Telegram Stream Reader запуск...")
...
print(f"🔄 Начинаю чтение из {NOTIFY_STREAM}...")
    ...
    try:
        ...
        for stream, items in messages:  # noqa: B007, RUF100
            for msg_id, fields in items:
                try:
                    print(f"📨 Получен сигнал: {msg_id}")

                    # Извлекаем текст сообщения
                    text = fields.get('text', '')
                    if not text:
                        print("⚠️ Пустой текст сигнала")
                        continue
                    ...
                    if success:
                        ...
                        print(f"✅ Сигнал обработан: {msg_id}")
                    else:
                        print(f"❌ Не удалось отправить сигнал: {msg_id}")  # noqa: RUF001
