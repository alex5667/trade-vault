#!/bin/bash
# Start OBI Services with venv activation
...
echo "🚀 Starting OBI Event & PNG Services (venv)..."
echo ""
...
python -m services.notify_receiver > /tmp/notify_receiver.log 2>&1 &
NOTIFY_PID=$!
echo "   PID: $NOTIFY_PID"
sleep 2
...
echo "$NOTIFY_PID" > /tmp/notify_receiver.pid
