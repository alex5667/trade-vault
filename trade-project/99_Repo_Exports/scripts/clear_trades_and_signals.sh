...
# Скрипт для очистки всех данных по сделкам и сигналам
# Удаляет сигналы, ордера, события и связанные данные
...
# Паттерны ключей для очистки
KEY_PATTERNS=("signals:*" "signal:*" "order:*" "trade:*")
STREAM_PATTERNS=("events:trades" "trades:closed")
...
echo -e "${RED}🧹 ОЧИСТКА ДАННЫХ ПО СДЕЛКАМ И СИГНАЛАМ${NC}"
echo -e "${RED}========================================${NC}"
echo -e "${YELLOW}⚠️  ВНИМАНИЕ: Это удалит все данные по сделкам и сигналам!${NC}"
echo ""
echo -e "${YELLOW}Будут удалены:${NC}"
echo -e "  • Все сигналы (signals:*, signal:*)"
echo -e "  • Все ордера (order:*)"
echo -e "  • Все сделки (trade:*)"
echo -e "  • Все события (events:trades, trades:closed)"
echo ""
