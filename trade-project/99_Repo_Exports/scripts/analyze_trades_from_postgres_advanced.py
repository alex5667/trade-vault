# scripts/analyze_trades_from_postgres_advanced.py
...
# ----------------------------
# Рендер отчётов (plain/markdown-friendly)
# ----------------------------
...
def main() -> None:
    ...
    # собираем всё в одну строку для удобного копипаста в Telegram
    out_chunks: list[str] = []

    # сортируем группы по label
    for key in sorted(groups.keys()):
        gb = groups[key]
        out_chunks.append(render_global_report(gb.label, gb.global_stats))
        out_chunks.append("")  # пустая строка
        out_chunks.append(
            render_entry_tag_report(
                gb.by_tag,
                min_trades=args.min_trades_per_tag,
            )
        )
        out_chunks.append("")  # пустая строка между группами

    full_text = "\n".join(out_chunks).strip()

    if args.markdown:
        # Для Telegram — просто завернули в код-блок
        print("```")
        print(full_text)
        print("```")
    else:
