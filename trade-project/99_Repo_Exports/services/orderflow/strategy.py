...
class OrderFlowStrategy:
    ...
    async def process_tick(self, runtime: SymbolRuntime, tick: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            runtime.last_liq = {"score": liq.score, "regime": liq.regime}

            indicators["liq_score"] = float(liq.score)
            indicators["liq_regime"] = str(liq.regime)
            indicators["liq_depth_usd_min_5"] = float(depth_usd_min_5)
            indicators["liq_spread_bps"] = float(liq.spread_bps)
            indicators["liq_book_rate_hz"] = float(liq.book_rate_hz)
            indicators["liq_book_stale_ms"] = int(stale)
            if liq.score < 0.60:
        ...
        try:
            # 1. Feed trade (skip if side is unknown)
            qty_f = float(tick.get("qty") or 0.0)

            side_sign = 0
            bm = tick.get("is_buyer_maker")
            if bm is not None:
                # Binance semantics: isBuyerMaker=True => taker SELL, False => taker BUY
        ...
        try:
            if price > 0:
                ...
                if sym:
                    ...
                    asyncio.create_task(self.redis.set(f"cfg:last_px:{sym}", str(price), ex=ttl))
                    asyncio.create_task(self.redis.set(f"cfg:last_px_ts_ms:{sym}", str(now_ms), ex=ttl))
        except Exception:
            pass
            ...
            if os.getenv("ATR_SANITY_ENABLE", "1") == "1":
                ...
                try:
                    if int(res.bad) == 1:
                        ...
                        asyncio.create_task(self.redis.set(f"cfg:atr_bad:{runtime.symbol}", "1", ex=ttl))
                        asyncio.create_task(self.redis.sadd("cfg:atr_bad:symbols", str(runtime.symbol)))
                        asyncio.create_task(self.redis.expire("cfg:atr_bad:symbols", int(os.getenv("ATR_BAD_SYMBOLS_SET_TTL_SEC", "86400"))))
                        # SRE counter: atr_bad_total{symbol,reason} (hash field=reason)
                        try:
                            reason = str(res.reason or "na")
                            asyncio.create_task(self.redis.hincrby(f"metrics:atr_bad_total:{runtime.symbol}", reason, 1))
                            asyncio.create_task(self.redis.expire(f"metrics:atr_bad_total:{runtime.symbol}", int(os.getenv("METRICS_COUNTER_TTL_SEC", "604800"))))
                        except Exception:
                            pass
                    ...
                    if int(getattr(res, "jump_event", 0) or 0) == 1:
                        ...
                        asyncio.create_task(self.redis.incr(f"cfg:atr_jump_count:{runtime.symbol}"))
                        asyncio.create_task(self.redis.expire(f"cfg:atr_jump_count:{runtime.symbol}", win))
                        asyncio.create_task(self.redis.sadd("cfg:atr_jump:symbols", str(runtime.symbol)))
                        asyncio.create_task(self.redis.expire("cfg:atr_jump:symbols", int(os.getenv("ATR_JUMP_SYMBOLS_SET_TTL_SEC", "86400"))))
                        # SRE counter: atr_jump_total{symbol}
                        try:
                            asyncio.create_task(self.redis.incr(f"metrics:atr_jump_total:{runtime.symbol}"))
                            asyncio.create_task(self.redis.expire(f"metrics:atr_jump_total:{runtime.symbol}", int(os.getenv("METRICS_COUNTER_TTL_SEC", "604800"))))
                        except Exception:
                            pass
                except Exception:
                    pass
        ...
        try:
            if int(indicators.get("cvd_quarantine_active", 0) or 0) == 1:
                ...
                asyncio.create_task(self.redis.set(f"cfg:cvd_quarantine_meta:{runtime.symbol}", json.dumps(meta, ensure_ascii=False), ex=ttl_sec))
                asyncio.create_task(self.redis.sadd("cfg:cvd_quarantine:symbols", str(runtime.symbol)))
                asyncio.create_task(self.redis.expire("cfg:cvd_quarantine:symbols", int(os.getenv("CVD_QUAR_SYMBOLS_SET_TTL_SEC", "86400"))))
                # SRE counter: cvd_quarantine_activations_total{symbol}
                try:
                    asyncio.create_task(self.redis.incr(f"metrics:cvd_quarantine_activations_total:{runtime.symbol}"))
                    asyncio.create_task(self.redis.expire(f"metrics:cvd_quarantine_activations_total:{runtime.symbol}", int(os.getenv("METRICS_COUNTER_TTL_SEC", "604800"))))
                except Exception:
                    pass
        except Exception:
            pass
        ...
        try:
            ...
            # This prevents exec_risk_penalty=0 when data is missing
            try:
                spr = float(indicators.get("spread_bps", 0.0) or 0.0)
                if spr <= 0:
                    spr = float(getattr(runtime, "last_spread_bps", 0.0) or 0.0)
                ...
                if spr <= 0:
                    ...
                    # degrade data_health so book-evidence is vetoed if you already use it downstream
                    ...
                    # Add specific reason and explicitly degrade book_health
            ...
            try:
                ...
                if sym:
                    ...
                    if int(indicators.get("atr_bad", 0) or 0) == 1:
                        ...
                        asyncio.create_task(self.redis.set(f"cfg:atr_bad:{sym}", json.dumps(o, ensure_ascii=False), ex=ttl))
                        sset = os.getenv("ATR_BAD_SYMBOLS_SET", "cfg:atr_bad:symbols")
                        asyncio.create_task(self.redis.sadd(sset, sym))
                        asyncio.create_task(self.redis.expire(sset, ttl))
                    ...
                    if int(indicators.get("cvd_quarantine_active", 0) or 0) == 1:
                        ...
                        asyncio.create_task(self.redis.set(f"cfg:cvd_quarantine:{sym}", json.dumps(o, ensure_ascii=False), ex=ttl))
            ...
            if ofc:
                ...
                if enforce and ofc.ok == 0:
                    ...
                    if is_soft_pass:
                        ...
                        indicators["is_virtual"] = 1  # Ensure shadowed signals execute virtually
                    else:
                ...
                try:
                    ...
                    # Fallback: raw OBI snapshot (always available while books flow)
                    if "obi" not in indicators:
                        snap = getattr(runtime, "last_obi_snapshot", None)
                        if isinstance(snap, dict):
                            snap_ts = int(snap.get("ts_ms", 0) or 0)
                            snap_age = now_ms_det - snap_ts if snap_ts > 0 else 999999
                            book_stale_ms = int(runtime.config.get("book_stale_ms", 5000))
                            if snap_ts > 0 and 0 <= snap_age <= book_stale_ms:
                                indicators["obi"] = float(snap.get("obi", 0.0) or 0.0)
                                indicators["obi_z"] = float(snap.get("obi_z", 0.0) or 0.0)
                                indicators["obi_dir"] = str(snap.get("direction", "") or "")
                                indicators["obi_snapshot_age_ms"] = int(snap_age)
                                indicators["obi_src"] = "snapshot"
        ...
        try:
            if int(indicators.get("book_health_ok", 1) or 1) == 1:
                ...
                if "obi" not in indicators:
                    ...
                    if isinstance(snap, dict):
                        snap_ts = int(snap.get("ts_ms", 0) or 0)
                        snap_age = now_ms - snap_ts if snap_ts > 0 else 999999
                        book_stale_ms = int(runtime.config.get("book_stale_ms", 5000))
                        if snap_ts > 0 and 0 <= snap_age <= book_stale_ms:
                            indicators["obi"] = float(snap.get("obi", 0.0) or 0.0)
                            indicators["obi_z"] = float(snap.get("obi_z", 0.0) or 0.0)
        ...
        try:
            ...
            indicators["liq_depth_usd_min_5"] = float(depth_usd_min_5)
            indicators["liq_spread_bps"] = float(liq_ev.spread_bps)
            indicators["liq_book_rate_hz"] = float(liq_ev.book_rate_hz)
            indicators["liq_book_stale_ms"] = int(book_stale_ms)
            if liq_ev.score < 0.60:
                indicators["liq_why"] = f"score<={liq_ev.score:.2f}"
        except Exception:
            pass
