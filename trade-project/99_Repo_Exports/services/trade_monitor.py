...
TM_SIGNAL_BLOCKED_SYMBOL_GUARD = Counter(
    "tm_signal_blocked_symbol_guard_total",
    "Signals blocked because the symbol already has an open position",
    ["symbol"]
)
# --------------------------------------------------------------------
...
class TradeMonitorService:
    def __init__(
        ...
        *,
        ...
        # ✅ Per-symbol position guard: 1 symbol = 1 open position
        # When enabled, on_signal() blocks new signals for symbols that already
        # have open positions (in-memory check via open_by_symbol index).
        # When disabled (default=0), only sid-based dedup applies.
        self.exec_single_active_position_per_symbol = os.getenv(
        ...
        # Single source of truth: CRYPTO_SIGNAL_MIN_CONF (no per-symbol overrides)
    ...
    def on_signal(self, raw_signal: Dict[str, Any]) -> Optional[str]:
        ...
        # Single source of truth: global confidence threshold (CRYPTO_SIGNAL_MIN_CONF)
        ...
        with self._lock:
            ...
            # ✅ Per-symbol guard: 1 symbol = 1 open position
            ...
            if self.exec_single_active_position_per_symbol:
                ...
                if existing:
                    ...
                    try:
                        TM_SIGNAL_BLOCKED_SYMBOL_GUARD.labels(symbol=sym_up).inc()
                    except Exception:
                        pass
            ...
            # Mark ALL trades as virtual regardless of policy
            pos.is_virtual = True
                    ...
                    if kind == "append_event":
                        ...
                        try:
                            ...
                            if not _d:
                                ...
                                continue
                            self.repo.save_trailing_move(_d.get("pos"), float(_d.get("previous_sl", 0.0)), float(_d.get("new_sl", 0.0)), int(_d.get("ts_ms", 0)))
                        except Exception as e:
                        ...
                        try:
                            ...
                            if not _d:
                                ...
                                continue
                            self.repo.save_trailing_sync(_d.get("pos"), int(_d.get("ts_ms", 0)))
                        except Exception as e:
                        ...
                        try:
                            ...
                            if not _d:
                                ...
                                continue
                            self.repo.save_closed(_d.get("closed"), health_snapshot=(_d.get("health_snapshot") or {}))
                        except Exception as e:
