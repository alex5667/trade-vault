# services/trailing_size_recommender.py
# -*- coding: utf-8 -*-
"""
...
class TrailingSizeRecommendation(NamedTuple):
    ...
    stop_atr_mult: float            # использованный stop_atr_mult
...
def _compute_confidence(
    ...
    if len(mfe_r_win) >= 2:
        # Простая реализация std без numpy
        mean_mfe = sum(mfe_r_win) / len(mfe_r_win)
        std_mfe_r = (sum((x - mean_mfe) ** 2 for x in mfe_r_win) / len(mfe_r_win)) ** 0.5
    else:
        std_mfe_r = 0.0

    if len(giveback_ratio_win) >= 2:
        mean_giveback = sum(giveback_ratio_win) / len(giveback_ratio_win)
        std_giveback_ratio = (sum((x - mean_giveback) ** 2 for x in giveback_ratio_win) / len(giveback_ratio_win)) ** 0.5
    else:
...
def recommend_trailing_size(
    ...
    *,
    ...
    Args:
        ...
        min_wins: (Optional) явно заданный минимум выигрышных сделок. Если None, используется эвристика.
        ...
        escape_min_wins: минимальное число побед для "аварийного выхода" (если trades < min_trades, но confidence высокий).
        escape_confidence: порог confidence для срабатывания escape hatch.
    ...
    # Default min_wins to min_trades/3 if not provided (heuristic)
    ...
    # NOTE: The original code only checked `n < min_trades`.
    # We now enforce `n` (wins) >= effective_min_wins AND count_total >= min_trades.
    # But for backward compatibility with calls passing only min_trades, we treat min_trades as a rough filter.
    
    # Let's align with the goal: be robust.
    # If we have too few wins, we can't recommend anything unless escape hatch.
    
    sufficiency_met = (count_total >= min_trades) and (n >= effective_min_wins)
    ...
    if not sufficiency_met:
        ...
        if n < escape_min_wins:
            return None
        # If escape hatch candidate (enough wins but maybe not enough total trades),
        # we proceed to compute stats and check confidence at the end.
    ...
    lock_r = max(0.05, min(raw_lock_r, 1.0))
