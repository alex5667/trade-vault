...
def main():
    ...
    try:
        ...
        from services.periodic_reporter import hydrate_trade_closed
        seen_pairs = set()
        ...
        for _, fields in all_entries:
            if not fields:
                continue
            t = hydrate_trade_closed(redis_client, fields)
            source = t.get("source", "").strip()
            symbol = t.get("symbol", "").strip()
            if source and symbol:
                # Get the logical source for the strategy, fallback to executor source
                from domain.normalizers import source_from_strategy
                logical_source = source_from_strategy(t.get("strategy", ""), source)
                pair = (canon_source(logical_source), canon_symbol(symbol))
                seen_pairs.add(pair)
    ...
    test_symbol = "BTCUSDT"
    logger.info(f"\n🎯 Используем хардкод для теста: {test_source} / {test_symbol}")
