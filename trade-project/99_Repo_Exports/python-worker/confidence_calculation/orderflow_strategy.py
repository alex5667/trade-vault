...
from handlers.crypto_orderflow.utils.log_sampler import LogSamplerFactory, sampled_debug, sampled_info, sampled_warning
...
from services.orderflow.utils import _calc_pressure_sps, _cooldown_ms_for, _should_sample, hour_of_week_utc, session_utc
...
class OrderFlowStrategy:
    ...
    def __init__(self, redis: aioredis.Redis, ticks: aioredis.Redis, publisher: AsyncSignalPublisher,
        ...
        self._max_back_ms = int(os.getenv("TIME_MAX_BACK_MS", "2000"))
        self._warn_back_ms = int(os.getenv("TIME_WARN_BACK_MS", "500"))
        self._debug_deltas = os.getenv("CRYPTO_OF_DEBUG_DELTAS", "false").strip().lower() in ("1", "true", "yes", "on")
        self._last_px_ttl = int(os.getenv("LAST_PX_TTL_SEC", "600"))
    ...
    def _ensure_proof_state(self, now_ms: int):
        ...
        try:
            ...
            with open(self.conf_cal_proof_path) as f:
    ...
    async def process_tick(self, runtime: SymbolRuntime, tick: dict[str, Any]) -> dict[str, Any] | None:
        ...
        # --- PROACTIVE REGIME REFRESH ---
        current_regime = str(getattr(runtime, "last_regime", "na") or "na").lower()
        if current_regime in ("na", "unknown", "none", ""):
            # Throttle fetching to avoid Redis spam on missing keys
            if getattr(runtime, "tick_count", 0) % 50 == 0:
                try:
                    rg_val = await self.redis.get(f"regime:{runtime.symbol}")
                    if rg_val:
                        current_regime = rg_val.decode("utf-8") if isinstance(rg_val, bytes) else str(rg_val)
                        runtime.last_regime = current_regime
                except Exception:
                    pass

        indicators["regime"] = current_regime
        ...
        try:
            ...
            # Prefer last_spread_bps_l2 (set on every book update from BBO in book_processor)
            # over last_spread_bps (set only when microbar closes, may be stale/zero)
            spread_bps = float(getattr(runtime, "last_spread_bps_l2", 0.0) or 0.0)
            if spread_bps <= 0:
            ...
            if snap is not None:
                ...
                try:
                    ...
                    # Compute real spread from bid/ask when spread_bps not pre-calculated on snapshot
                    if spread_bps <= 0 and bb > 0 and ba > bb and mid > 0:
                        computed_spr = 10_000.0 * (ba - bb) / mid  # bps
                        if computed_spr > 0:
                            spread_bps = computed_spr
                            # Persist so line ~1251 picks it up without re-reading the book
        ...
        try:
            ...
            runtime.l3_queue.on_trade(
                side=1 if ((tick.get("side")).upper() == "BUY") else -1,
        ...
        try:
            ...
            atr_gate_audit_only = str(runtime.config.get("atr_gate_audit_only", os.getenv("ATR_GATE_AUDIT_ONLY", "false"))).lower() in ("true", "1", "yes")

            atr_cache = get_atr_cache()
            atr_val, atr_meta = atr_cache.get_with_meta(symbol=runtime.symbol, timeframe=None)

            px0 = float(price or indicators.get("price", 0.0) or 0.0)
            atr0 = float(atr_val) if atr_val is not None else float(getattr(runtime, "last_atr", 0.0) or 0.0)
            now_ms = int(tick_ts if tick_ts > 0 else get_ny_time_millis())

            res = self._atr_sanity.update(
                symbol=str(runtime.symbol),
                atr=float(atr0),
                px=float(px0),
                age_ms=int((atr_meta or {}).get("age_ms", 0)),
                now_ms=now_ms,
                tf=str((atr_meta or {}).get("picked_tf") or "5m"),
            )

            atr_used = float(res.atr_used)
            indicators["atr"] = atr_used
            ...
            if atr_meta:
                ...
                indicators["atr_age_ms"] = int(atr_meta.get("age_ms", 0))
            else:
                indicators["atr_src"] = str(getattr(runtime, "atr_src", "na"))
                indicators["atr_tf"] = str(getattr(runtime, "atr_tf", "na"))
                indicators["atr_age_ms"] = int(getattr(runtime, "atr_age_ms", 0) or 0)
            ...
            t0 = float(runtime.dynamic_cfg.get(DK.ATR_FLOOR_T0_BPS, 0.0) or runtime.config.get("atr_floor_t0_bps", 0.0) or 0.0)
            t1 = float(runtime.dynamic_cfg.get(DK.ATR_FLOOR_T1_BPS, 0.0) or runtime.config.get("atr_floor_t1_bps", 0.0) or 0.0)
            t2 = float(runtime.dynamic_cfg.get(DK.ATR_FLOOR_T2_BPS, 0.0) or runtime.config.get("atr_floor_t2_bps", 0.0) or 0.0)
            ...
            indicators["atr_bps_th"] = float(th)

            passed = (atr_bps >= th)

            if not passed:
                if atr_gate_audit_only:
                    if runtime.delta_log_sampler.should_log("atr_floor_audit"):
                        logger.info("AUDIT: [ATR-FLOOR] (%s) would veto: atr_bps=%.2f < th=%.2f (tier=%d)",
                                    runtime.symbol, atr_bps, th, tier)
                else:
                    if runtime.delta_log_sampler.should_log("atr_floor_veto"):
                        logger.warning("🛑 [ATR-FLOOR] (%s) VETO: atr_bps=%.2f < th=%.2f (tier=%d) - Blocked",
                                    runtime.symbol, atr_bps, th, tier)
                    return None

        except Exception as e:
            logger.error(f"ATR Floor Veto Error: {e}")
        ...
        try:
            ...
            # last_spread_bps_l2 is updated on every book message (BBO-based) → prefer it
            ...
            if spr <= 0 and runtime.last_book:
                snap_b = runtime.last_book
                spr = float(getattr(snap_b, "spread_bps", 0.0) or 0.0)
                if spr <= 0:
                    # Compute from best bid/ask — most reliable source
                    try:
                        bb = float(getattr(snap_b, "best_bid_px", 0.0) or 0.0)
                        ba = float(getattr(snap_b, "best_ask_px", 0.0) or 0.0)
                        if bb > 0 and ba > bb:
                            mid_b = (bb + ba) / 2.0
                            spr = 10_000.0 * (ba - bb) / mid_b  # bps
                            if spr > 0:
                                runtime.last_spread_bps = float(spr)
                                indicators["spread_bps_book_computed"] = 1
                    except Exception:
                        pass
        ...
        # --- ATR meta & sanity flags moved to early ATR-FLOOR veto ---
            ...
            try:
                ...
                if k > 0:
                    ...
                    if impact > 0:
                        ...
                        indicators['slippage_reason'] = (indicators.get('slippage_reason', 'na') or 'na') + f'|ofi+{impact:.3f}'
            except Exception:
                pass
        except Exception:
        ...
        try:
            ...
            if ofc:
                ...
                # ── DIAGNOSTIC (temporary): confirm of_confirm_ok written ──
                if runtime.tick_count % 50000 == 0:
                    self.logger.warning(
                        "🔍 [DIAG-OFC-SET] %s | of_confirm_ok=%d | id(indicators)=%d | ofc.ok=%s",
                        runtime.symbol, int(ofc.ok), id(indicators), ofc.ok,
                    )
                    ...
                    if OF_GATE_METRICS_ENABLE:
                        ...
                        if rate > 0 and _should_sample(int(tick_ts), rate):
                            ...
                            payload = {
                                ...
                                "meta_p": str(float(ev.get(MetaKeys.P, -1.0) or -1.0)),
                                "meta_veto": str(int(ev.get(MetaKeys.VETO, 0) or 0)),
                                "meta_enforce_applied": str(int(ev.get(MetaKeys.ENFORCE_APPLIED, 0) or 0)),
                                "meta_enforce_share": str(float(ev.get(MetaKeys.ENFORCE_SHARE, 1.0) or 1.0)),
                                "meta_enforce_bucket": (ev.get("meta_enforce_bucket", "other") or "other"),
                                "data_health": str(float(indicators.get("data_health", 1.0) or 1.0)),
                                "data_health_veto": str(int(indicators.get("data_health_veto_active", 0) or 0)),
                                "data_health_shadow": str(int(indicators.get("data_health_shadow_only", 0) or 0)),
                                ...
                                # LOB pressure (P91) - needed for LOB_PRESSURE_SMOKE check
                                "lob_qi_mean": str(float((runtime.last_lob_event or {}).get("qi_mean", 0.0) or 0.0)),
                                "lob_qi_max_abs": str(float((runtime.last_lob_event or {}).get("qi_max_abs", 0.0) or 0.0)),
                                "lob_qi_slope": str(float((runtime.last_lob_event or {}).get("qi_slope", 0.0) or 0.0)),
                                "lob_micro_mid_div_bps": str(float((runtime.last_lob_event or {}).get("micro_mid_div_bps", 0.0) or 0.0)),
                                "lob_micro_shift_bps": str(float((runtime.last_lob_event or {}).get("micro_shift_bps", 0.0) or 0.0)),
                                "lob_depth_slope_imb": str(float((runtime.last_lob_event or {}).get("depth_slope_imb", 0.0) or 0.0)),
                                "lob_depth_convexity_imb": str(float((runtime.last_lob_event or {}).get("depth_convexity_imb", 0.0) or 0.0)),
                                "lob_dw_obi": str(float((runtime.last_lob_event or {}).get("dw_obi", 0.0) or 0.0)),
                                "lob_dw_obi_z": str(float((runtime.last_lob_event or {}).get("dw_obi_z", 0.0) or 0.0)),
                                "lob_dw_obi_stability_score": str(float((runtime.last_lob_event or {}).get("dw_obi_stability_score", 0.0) or 0.0)),
                                "lob_dw_obi_stable_secs": str(float((runtime.last_lob_event or {}).get("dw_obi_stable_secs", 0.0) or 0.0)),
                                "lob_dw_obi_stable": str(int((runtime.last_lob_event or {}).get("dw_obi_stable", 0) or 0)),
                            }
                ...
                if dec and hasattr(dec, "need") and hasattr(dec, "have"):
                    ...
                    if bool(ev.get("ok_soft", 0)):
                        ...
                        reason_soft = (ev.get("soft_reason", "") or "")
                        ...
                        indicators["of_confirm_ok_soft"] = 0
                    ...
                    try:
                        ...
                        runtime.last_of_confirm_have_need_ratio = float(indicators.get("of_confirm_have_need_ratio", 0.0) or 0.0)
                ...
                if enforce and ofc.ok == 0:
                    ...
                    if is_soft_pass:
                        ...
                        self.logger.info(
                            ...
                            runtime.symbol, indicators.get("soft_fail_type"), indicators.get("soft_fail_reason")
                        )
...
#                             )
                        ...
                        if emit_v2:
                            ...
                            # Session one-hot (train==serve: deterministic from tick_ts_ms)
                            # session_onehot(tick_ts_ms) already called in A5 block of tick_processor.py.
                            # Exactly one of these will be 1; defaults to 0 if indicators not populated.
                            ofi_kwargs["session_asia"] = _i(indicators.get("session_asia", 0), 0)
                            ofi_kwargs["session_eu"]   = _i(indicators.get("session_eu", 0), 0)
                            ofi_kwargs["session_us"]   = _i(indicators.get("session_us", 0), 0)
                            ofi_kwargs["session_off"]  = _i(indicators.get("session_off", 0), 0)
        ...
        try:
             ...
             if div:
                  ...
                  # Div Match flag (Only trust if CVD is not quarantined)
                  div_match = 0
                  cvd_q = int(indicators.get("cvd_quarantine_active", getattr(runtime, "cvd_quarantine_active", 0) or 0) or 0)
                  if cvd_q != 1:
                      d_dir = (payload.get("direction", "")).upper()
        ...
        try:
            ...
            drift_state = (indicators.get("drift_state", "ok")).lower()
            dq_state = (indicators.get("dq_state", "ok")).lower()
    ...
    async def _emit_payload(self, runtime: SymbolRuntime, payload: dict[str, Any], now_ms: int) -> dict[str, Any] | None:
        """
    ...
    def _compute_confidence(
        ...
        *,
        ...
        ctx = SimpleNamespace(
            ...
            # ML Scorer V2: L3 orderflow features for MLScoringGate
            l3_spread_bps=float(_get("l3_spread_bps", _get("spread_bps", 0.0))),
            l3_microprice_shift_bps_20=float(_get("l3_microprice_shift_bps_20", _get("microprice_shift_bps_20", 0.0))),
            l3_microprice_velocity_bps=float(_get("l3_microprice_velocity_bps", 0.0)),
            l3_obi_5=float(_get("l3_obi_5", 0.0)),
            l3_obi_20=float(_get("l3_obi_20", _get("obi_20", 0.0))),
            l3_obi_50=float(_get("l3_obi_50", 0.0)),
            l3_obi_persistence_score=float(_get("l3_obi_persistence_score", 0.0)),
            l3_cancel_to_trade_bid_5s=float(_get("l3_cancel_to_trade_bid_5s", _get("cancel_to_trade_bid", 0.0))),
            l3_cancel_to_trade_ask_5s=float(_get("l3_cancel_to_trade_ask_5s", _get("cancel_to_trade_ask", 0.0))),
            l3_cancel_to_trade_bid_20s=float(_get("l3_cancel_to_trade_bid_20s", 0.0)),
            l3_cancel_to_trade_ask_20s=float(_get("l3_cancel_to_trade_ask_20s", 0.0)),
            l3_queue_pressure_bid=float(_get("l3_queue_pressure_bid", 0.0)),
            l3_queue_pressure_ask=float(_get("l3_queue_pressure_ask", 0.0)),
            l3_market_depth_imbalance=float(_get("l3_market_depth_imbalance", 0.0)),
            conf_score=0.0,
            # ML Scorer V2: feature flag for USE_UNIFIED_SCORING gate
        ...
        try:
            conf, parts = self.conf_scorer.score(kind=kind or "custom", side=side, ctx=ctx, ff=ctx)
            ...
            # ML Scorer V2: propagate shadow fields for observability
            if "ml_shadow_conf01" in parts:
                indicators["ml_shadow_conf01"] = parts["ml_shadow_conf01"]
                indicators["ml_shadow_predicted_r"] = parts.get("ml_shadow_predicted_r", 0.0)
                indicators["ml_shadow_status"] = parts.get("ml_shadow_status", "")
            if "scorer_mode" in parts:
                indicators["scorer_mode"] = parts["scorer_mode"]
            ...
            try:
                if int(runtime.config.get("confidence_shadow_enable", 0) or 0) == 1:
                    ...
                    conf2, parts2 = self.conf_scorer.score(kind=kind or "custom", side=side, ctx=ctx2, ff=ctx2)
    ...
    async def _on_microbar_closed(self, runtime: SymbolRuntime, bar: MicroBar) -> None:
        ...
        try:
             ...
             if rg_val:
                 new_regime = rg_val.decode("utf-8") if isinstance(rg_val, bytes) else str(rg_val)
        ...
        try:
            ...
            if math.isfinite(dn_usd) and dn_usd > 0:
                ...
                if now_ms - runtime.last_dn_how_report_ts_ms > 3600_000:
                    ...
                    # Metrics
                    from services.orderflow.metrics import of_dn_how_ratio_t1_gauge, of_dn_how_scale_gauge
                    with contextlib.suppress(Exception):
                    ...
                    if runtime._calib_bars_since_persist >= min_bars:
                        ...
                        if self.calib_svc:
                            await asyncio.gather(
                                self.calib_svc.persist_dn(runtime, regime=rg, ts_ms=int(bar.end_ts_ms)),
                                self.calib_svc.persist_tick_dn(runtime, regime=rg, ts_ms=int(bar.end_ts_ms))
                            )
        ...
        try:
            ...
            adx_raw = await self.redis.hget(f"adx:{runtime.symbol}", "adx")  # type: ignore[misc]
