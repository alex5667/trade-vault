...
def main() -> None:
    if os.getenv("ALERTS_ENABLE", "1") != "1":
        return
    ...
    atr_bad_pct = float(os.getenv("ALERT_ATR_BAD_PCT", "30"))
    ...
    while True:
        try:
            ...
            if 100.0 * (atr_bad_n / float(universe)) >= atr_bad_pct and _cooldown_ok(r, "alerts:cooldown:atr_bad", cooldown):
                top = _read_set(r, "cfg:atr_bad:symbols", max_n=20)
                msgs.append(
                    f"ATR bad pct={atr_bad_n}/{universe}={100.0*atr_bad_n/universe:.1f}% >= {atr_bad_pct:.0f}%. Top: {', '.join(top)}"
                )

            if 100.0 * (cvd_q_n / float(universe)) >= cvd_q_pct and _cooldown_ok(r, "alerts:cooldown:cvd_q", cooldown):
                top = _read_set(r, "cfg:cvd_quarantine:symbols", max_n=20)
                msgs.append(
                    f"CVD quarantine pct={cvd_q_n}/{universe}={100.0*cvd_q_n/universe:.1f}% >= {cvd_q_pct:.0f}%. Top: {', '.join(top)}"
                )
