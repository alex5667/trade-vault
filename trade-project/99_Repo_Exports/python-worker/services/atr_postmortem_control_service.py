...
def create_postmortem_for_incident(incident_id: str, severity: str, reason_code: str, title: str) -> str | None:
    ...
    try:
        with get_conn() as conn, conn.cursor() as cur:
            ...
            r.xadd(RS.NOTIFY_TELEGRAM, {"channel": "ops", "type": "postmortem_required", "payload": json.dumps(payload)}, maxlen=STREAM_RETENTION[RS.NOTIFY_TELEGRAM], approximate=True)
...
def change_postmortem_status(pm_id: str, new_status: str, actor: str) -> bool:
    ...
    try:
        with get_conn() as conn, conn.cursor(cursor_factory=__import__('psycopg2').extras.RealDictCursor) as cur:
            ...
            r.xadd(RS.NOTIFY_TELEGRAM, {"channel": "ops", "type": "postmortem_status_change", "payload": json.dumps(payload)}, maxlen=STREAM_RETENTION[RS.NOTIFY_TELEGRAM], approximate=True)
...
def create_corrective_action(pm_id: str, action_type: str, priority: str, title: str, reason_code: str, due_at_ms: int, owner: str, payload_json: dict) -> str | None:
