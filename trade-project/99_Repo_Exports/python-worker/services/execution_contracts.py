...
def build_materialized_state_view(state: dict[str, Any]) -> dict[str, Any]:
    ...
    # Scale-in: include TP qty allocation and leg history in materialized view
    tp_qtys = _json_field(doc, 'tp_qtys_requested_json')
    if tp_qtys is not None:
        doc.setdefault('position', {})['tp_qtys_requested'] = tp_qtys
    ...
    if scale_in_meta:
        doc.setdefault('position', {})['scale_in_meta'] = scale_in_meta
    return doc
