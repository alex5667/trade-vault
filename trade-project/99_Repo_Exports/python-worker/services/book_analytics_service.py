...
class MicrostructureFeatures:
    """VWAP deviation + bid-ask spread dynamics."""
    @staticmethod
    def compute(tick: TickMessage, vwap: float) -> dict:
        spread = tick.ask - tick.bid
        spread_bps = 0.0
        if tick.last_price > 0:
            spread_bps = spread / tick.last_price * 10_000

        vwap_dev = 0.0
        if vwap > 0:
            vwap_dev = (tick.last_price - vwap) / vwap * 10_000  # в bps

        return {
            "spread_bps": spread_bps,
            "vwap_dev_bps": vwap_dev,
            # Широкий спред при движении вниз = panic selling
            "spread_pressure": spread_bps * (-1 if vwap_dev < 0 else 1),
            # Нормализованная позиция цены в bid-ask
            "price_position": (tick.last_price - tick.bid) / spread if spread > 0 else 0.5,
        }
