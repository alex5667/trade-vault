    ...
    def process_book(self, runtime: SymbolRuntime, payload: Dict[str, Any], ingest_ts_ms: int) -> bool:
        ...
        try:
            ...
            if book_ts_ms > 0:
                ...
                # that decision-snapshot (strategy.py:4986-4987) reads real state.
                # book_health is computed in strategy.py → compute_data_health() and
                # written into indicators["book_health*"]. We mirror it to runtime here
                # so the decision snapshot always reflects the actual observed state.
                # Note: indicators may not be passed to book_processor; we use safe
                # defaults that match the dataclass defaults so cold-start is safe.
                # The canonical update happens via _sync_book_health_to_runtime() called
                # from strategy.py after compute_data_health (see below).
                # This assignment ensures that even before the first tick the value is
                # at least the correct warm-up safe default (1 / "OK").
            ...
            # P1 audit: surface fail-open errors via Prometheus so they don't hide for a week.
            # reason = exception class name (low cardinality: TypeError, ValueError, KeyError, …)
            try:
