...
class LeaseManager:
    ...
    async def try_acquire_lease(self, msg_id: str) -> bool:
        """
        ...
        try:
            ...
            ok = await self.redis.set(key, "1", nx=True, px=self.config.msg_lease_ttl_ms)
            if ok:
                return True
        except Exception:
    ...
    async def release_lease(self, msg_id: str) -> None:
    ...
    async def try_acquire_sid_lease(self, sid: str) -> str | None:
        """
        ...
        try:
            ok = await self.redis.set(key, token, nx=True, px=int(self.config.sid_lease_ttl_ms))
            if ok:
    ...
    async def release_sid_lease(self, sid: str, token: str) -> None:
        with contextlib.suppress(Exception):
            await self.lua_scripts.execute("release_lease", keys=[self._sid_lease_key(sid)], args=[token])

    async def maybe_extend_sid_lease(self, sid: str, token: str, last_extend_ms: int) -> int:
        """
        ...
        try:
            ok = await self.lua_scripts.execute(
                "extend_lease",
