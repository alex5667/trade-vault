...
def test_schema_legacy_downgrade(mock_engine):
    ...
    with patch("core.of_confirm_engine.OFConfirmEngine._load_meta_model_slot", return_value=mm_mock):
        ...
        assert ofc.evidence[MetaKeys.MODE] == "SHADOW"
        assert "SCHEMA_MISMATCH" in ofc.evidence[MetaKeys.REASON]
        assert ofc.evidence[MetaKeys.MODEL_SCHEMA_NAME] == "legacy"
...
def test_schema_compatible_enforce(mock_engine):
    ...
    with patch("core.of_confirm_engine.OFConfirmEngine._load_meta_model_slot", return_value=mm_mock):
        ...
        assert ofc.evidence[MetaKeys.MODE] == "ENFORCE"
        assert ofc.evidence[MetaKeys.MODEL_SCHEMA_NAME] == META_FEAT_V1_NAME
