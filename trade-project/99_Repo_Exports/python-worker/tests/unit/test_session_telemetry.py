...
from services.orderflow.utils import fmt_utc_dow_hour, hour_of_week_utc, session_utc
...
def test_utc_helpers():
    ...
    # Mon = 0
    # 0 * 24 + 10 = 10
    ts = int(datetime(2024, 1, 22, 10, 0, tzinfo=UTC).timestamp() * 1000)
    ...
    # Sun = 6
    # 6 * 24 + 23 = 144 + 23 = 167
    ts_sun = int(datetime(2024, 1, 21, 23, 59, tzinfo=UTC).timestamp() * 1000)
...
def test_how_scale_tracker():
    ...
    ts_other = int(datetime(2024, 1, 22, 11, 0, tzinfo=UTC).timestamp() * 1000)
    for _ in range(10):
        tracker.update(ts_other, 200.0) # Bucket 11

    ts_other2 = int(datetime(2024, 1, 22, 12, 0, tzinfo=UTC).timestamp() * 1000)
    for _ in range(100):
