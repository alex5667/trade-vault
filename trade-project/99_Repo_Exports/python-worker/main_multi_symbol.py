#!/usr/bin/env python3
"""
Multi-Symbol OrderFlow Handler — LEGACY STATIC ENTRY POINT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⚠️  ВНИМАНИЕ: ЭТОТ ФАЙЛ НЕ ЯВЛЯЕТСЯ КАНОНИЧЕСКОЙ ТОЧКОЙ ВХОДА ДЛЯ PRODUCTION.

PRODUCTION:  main_multi_symbol_dynamic.py  (запускается при DYNAMIC_SYMBOLS=true)
LEGACY/TEST: main_multi_symbol.py          ← текущий файл (DYNAMIC_SYMBOLS=false)

Этот файл сохранён для:
  1. Тестовых окружений без dynamic symbol management.
  2. Backward-compatibility с регрессионными тестами.
  3. HealthCheck: pgrep -f main_multi_symbol.py срабатывает на ОБА файла
     (поскольку dynamic-файл импортирует этот пакет), healthcheck НЕ ЗАВИСИТ
     от того, какой файл является точкой входа — это намеренно.
    ...
    def run(self, health_check_interval: int = 60) -> None:
        ...
        try:
            while self.is_running:
                ...
                # P4 SLO contract baseline flush
                try:
                    if hasattr(self.health_metrics, "r") and self.health_metrics.r:
                        for scope in ("edge", "pipeline", "entry_policy"):
                            flush_exec_health_contract_state_sync(redis_client=self.health_metrics.r, scope=scope)
                except Exception:
                    pass
