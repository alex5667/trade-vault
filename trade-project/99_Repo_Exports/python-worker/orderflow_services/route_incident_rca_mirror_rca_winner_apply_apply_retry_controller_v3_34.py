...
APP_NAME = "route_incident_rca_mirror_rca_winner_apply_apply_retry_controller_v3_34"
...
LAST_HASH = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_LAST_HASH",
    "metrics:ml:route_incident_rca_mirror_rca_winner_apply_apply_retry:last",
)
AUDIT_STREAM = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_AUDIT_STREAM",
    "stream:ml:route_incident_rca_mirror_rca_winner_apply_apply_retry_audit",
)
STATE_PREFIX = os.getenv(
    "ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_STATE_PREFIX",
    "state:ml:route_incident_rca_mirror_rca_winner_apply_apply_retry:",
)
...
PORT = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_PORT", "9960"))
MAXLEN = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_MAXLEN", "20000"))
RUN_EVERY_SEC = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_RUN_EVERY_SEC", "180"))
LOOKBACK_COUNT = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_LOOKBACK_COUNT", "50"))
MAX_ATTEMPTS = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_MAX_ATTEMPTS", "2"))
BACKOFF_SEC = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_BACKOFF_SEC", "120"))
STATE_TTL_SEC = int(os.getenv("ML_ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_STATE_TTL_SEC", "86400"))
...
RUNS = _counter("ml_route_incident_rca_mirror_rca_winner_apply_apply_retry_runs_total", "Winner-apply apply retry runs", ("status", "decision"))
LAT = _hist("ml_route_incident_rca_mirror_rca_winner_apply_apply_retry_latency_seconds", "Winner-apply apply retry latency seconds")
UP = _gauge("ml_route_incident_rca_mirror_rca_winner_apply_apply_retry_up", "Winner-apply apply retry up")
LAST_RUN_TS = _gauge("ml_route_incident_rca_mirror_rca_winner_apply_apply_retry_last_run_ts_seconds", "Winner-apply apply retry last run ts")
    ...
    while True:
        ...
        try:
            ...
            if resolved == 1:
                ...
                shadow_arms = "[]" if rollback_mode == "SINGLE_ARM" else json.dumps([a for a in ("deterministic", "vertex_candidate", "local_fallback_candidate") if a != rollback_primary])
                await r.hset(
                    EXPERIMENT_POLICY_KEY,
                    mapping={
                        "mode": rollback_mode,
                        "primary_arm": rollback_primary,
                        "shadow_arms_json": shadow_arms,
                        "last_mode_switch_ts_ms": str(now_ms()),
                        "last_mode_switch_reason_code": "WINNER_APPLY_APPLY_RETRY_REAPPLY",
            ...
            await r.xadd(
                AUDIT_STREAM,
                {"event_type": "ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_EVALUATED", **result},
                maxlen=MAXLEN,
                approximate=True,
            )
            await r.hset(
            ...
            status = "error"
            await r.xadd(AUDIT_STREAM, {"event_type": "ROUTE_INCIDENT_RCA_MIRROR_RCA_WINNER_APPLY_APPLY_RETRY_FAILED", "error": str(exc), "ts_ms": str(now_ms())}, maxlen=MAXLEN, approximate=True)
        finally:
            if RUNS:
