...
APP_NAME = "offline_debug_handoff_rewire_adapter_v3_3"
INPUT_STREAM = os.getenv(
    "ML_OFFLINE_DEBUG_HANDOFF_SOURCE_STREAM",
...
DECISIONS_STREAM = os.getenv(
    "ML_OFFLINE_DEBUG_HANDOFF_REWIRE_DECISIONS_STREAM",
    "stream:ml:offline_debug_handoff_rewire_decisions",
)
AUDIT_STREAM = os.getenv(
    "ML_OFFLINE_DEBUG_HANDOFF_REWIRE_AUDIT_STREAM",
    "stream:ml:offline_debug_handoff_rewire_audit",
)
LAST_HASH = os.getenv(
    "ML_OFFLINE_DEBUG_HANDOFF_REWIRE_LAST_HASH",
    "metrics:ml:offline_debug_handoff_rewire:last",
)
GLOBAL_POLICY_KEY = os.getenv(
    "ML_OFFLINE_DEBUG_HANDOFF_REWIRE_GLOBAL_POLICY_KEY",
    "cfg:ml:offline_debug_handoff_rewire:global",
)
GROUP = os.getenv("ML_OFFLINE_DEBUG_HANDOFF_REWIRE_GROUP", APP_NAME)
CONSUMER = os.getenv("HOSTNAME", APP_NAME)
PORT = int(os.getenv("ML_OFFLINE_DEBUG_HANDOFF_REWIRE_PORT", "9919"))
MAXLEN = int(os.getenv("ML_OFFLINE_DEBUG_HANDOFF_REWIRE_MAXLEN", "20000"))

DEFAULT_ENABLED = int(os.getenv("ML_OFFLINE_DEBUG_HANDOFF_REWIRE_ENABLED", "1"))
DEFAULT_MODE = os.getenv("ML_OFFLINE_DEBUG_HANDOFF_REWIRE_MODE", "ENABLED").upper()
DEFAULT_MAX_PAYLOAD_BYTES = int(os.getenv("ML_OFFLINE_DEBUG_HANDOFF_REWIRE_MAX_PAYLOAD_BYTES", "131072"))
DEFAULT_MAX_PROMPT_CHARS = int(os.getenv("ML_OFFLINE_DEBUG_HANDOFF_REWIRE_MAX_PROMPT_CHARS", "16000"))
...
RUNS = _counter(
    "ml_offline_debug_handoff_rewire_runs_total",
    "Offline debug handoff rewire runs",
    ("status", "decision"),
)
LAT = _hist(
    "ml_offline_debug_handoff_rewire_latency_seconds",
    "Offline debug handoff rewire latency seconds",
)
UP = _gauge(
    "ml_offline_debug_handoff_rewire_up",
    "Offline debug handoff rewire adapter up",
)
LAST_RUN_TS = _gauge(
    "ml_offline_debug_handoff_rewire_last_run_ts_seconds",
    "Offline debug handoff rewire adapter last run timestamp",
)
ROUTED = _counter(
    "ml_offline_debug_handoff_rewire_routed_total",
    "Offline debug handoff rewire routed rows",
    ("decision",),
)
...
def build_payload_json(row: dict[str, Any]) -> str:
    ...
    snapshot_json = maybe_json(row.get("snapshot_json"), {})
    prompt = (row.get("prompt") or "")
    context = maybe_json(row.get("context_json"), {})
    if snapshot_json or prompt or context:
        return stable_json({
            "prompt": prompt,
...
def build_handoff_row(row: dict[str, Any], policy: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "task_type": "offline_debug",
        "severity": severity,
        "source": "offline_debug_handoff_rewire_v3_3",
        "payload_json": build_payload_json(row),
            ...
            for msg_id, payload in messages:
                ...
                try:
                    ...
                    await r.xadd(
                        AUDIT_STREAM,
                        {
                            "event_type": "OFFLINE_DEBUG_HANDOFF_REWIRE_DECIDED",
                            **decision_payload,
                        }, maxlen=MAXLEN,
                        approximate=True,
                    )
                    ...
                    await r.xadd(
                        AUDIT_STREAM,
                        {
                            "event_type": "OFFLINE_DEBUG_HANDOFF_REWIRE_FAILED",
                            "error": str(exc),
                            "ts_ms": str(now_ms()),
                        }, maxlen=MAXLEN,
                        approximate=True,
