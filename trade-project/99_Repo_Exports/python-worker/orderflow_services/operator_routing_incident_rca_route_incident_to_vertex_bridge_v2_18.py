...
APP_NAME = "operator_routing_incident_rca_route_incident_to_vertex_bridge_v2_18"
PORT = int(os.getenv("ML_OPERATOR_ROUTING_INCIDENT_RCA_ROUTE_BRIDGE_PORT", "9900"))

IN_STREAM = "stream:ml:operator_routing_incident_rca_route_incident_bundle_results"
OUT_STREAM = "stream:ml:operator_routing_incident_rca_route_rca_requests"

GROUP = "operator_routing_incident_rca_route_bridge_v2_18"
...
RUNS = _counter("ml_operator_routing_incident_rca_route_rca_bridge_runs_total", "Runs", ("status",))
LAT = _hist("ml_operator_routing_incident_rca_route_rca_bridge_latency_seconds", "Latency")
LAST_RUN = _gauge("ml_operator_routing_incident_rca_route_rca_bridge_last_run_ts_seconds", "Last timestamp")
            ...
            for msg_id, payload in records:
                try:
                    ...
                    req = {
                        ...
                        "primary_reason_codes": row.get("primary_reason_codes", ""),
                        "payload_json": json.dumps(row),
                        "request_type": "VERTEX_RCA_GOVERNANCE",
                        "ts_ms": now_ms()
                    }

                    await r.xadd(OUT_STREAM, req, maxlen=MAXLEN, approximate=True)
