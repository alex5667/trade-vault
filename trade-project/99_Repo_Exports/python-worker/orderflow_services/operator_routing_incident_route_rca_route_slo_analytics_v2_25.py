...
APP_NAME = "operator_routing_incident_route_rca_route_slo_analytics_v2_25"
PORT = int(os.getenv("ML_OPERATOR_ROUTING_INCIDENT_ROUTE_RCA_ROUTE_SLO_PORT", "9912"))

IN_STREAM = "stream:ml:operator_routing_incident_route_rca_routing_verify_results"
OUT_STREAM = "stream:ml:operator_routing_incident_route_rca_route_slo_rollups"
LAST_METRIC = "metrics:ml:operator_routing_incident_route_rca_route_slo:last"

GROUP = "operator_routing_incident_route_rca_route_slo_v2_25"
...
RUNS = _counter("ml_operator_routing_incident_route_rca_route_slo_runs_total", "Runs", ("status",))
LAT = _hist("ml_operator_routing_incident_route_rca_route_slo_latency_seconds", "Latency")
LAST_RUN = _gauge("ml_operator_routing_incident_route_rca_route_slo_last_run_ts_seconds", "Last timestamp")
            ...
            for msg_id, payload in records:
                try:
                    ...
                    rollup = {
                        "incident_id": inc_id,
                        "total": "1",
                        "success": "1" if row.get("status") == "SUCCESS" else "0",
                        "failed": "1" if row.get("status") != "SUCCESS" else "0",
                        "success_rate": "1.0" if row.get("status") == "SUCCESS" else "0.0",
                        "mttr_ms": "300000", # Dummy 5 min
                        "ts_ms": now_ms()
                    }
                    await r.xadd(OUT_STREAM, rollup, maxlen=MAXLEN, approximate=True)
                    await r.hset(LAST_METRIC, mapping=rollup)
