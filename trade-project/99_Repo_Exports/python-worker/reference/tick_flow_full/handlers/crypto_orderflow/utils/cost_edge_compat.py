...
def attach_cost_edge_veto_fields(ctx: Any, dec: Any) -> None:
    ...
    """
    try:
        ctx.veto_reason_code = str(getattr(dec, "reason_code", "") or "VETO_EDGE_COST")
        ctx.veto_reason = f"COST_EDGE: {getattr(dec, 'reason_code', 'VETO_EDGE_COST')}"
...
def maybe_dual_emit_legacy_thin_cost(*, emit_veto_metric: Any, kind: str, ctx: Any, reason_code: str) -> list[str]:
    ...
    emitted: list[str] = []
    try:
        rc = (reason_code or "VETO_EDGE_COST")
        try:
