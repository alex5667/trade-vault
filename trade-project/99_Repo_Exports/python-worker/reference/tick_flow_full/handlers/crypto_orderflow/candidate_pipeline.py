...
class GateRunner:
    ...
    def run(self, *, ctx: Any, **kwargs: Any) -> tuple[bool, str, list[StageTraceItem]]:
        ...
        for name, gate in self._gates:
            ...
            trace.append(StageTraceItem(stage=name, ok=bool(ok), reason=(reason or ""), meta=dict(meta or {})))
            if not ok:
...
class CandidateEmitPipeline:
    ...
    def process_candidate(self, *, cand: Any, **kwargs: Any) -> CandidatePipelineResult:
        ...
        if not ok:
            return CandidatePipelineResult(sent=False, veto_reason=(reason or "GATE_VETO"), trace=trace)
