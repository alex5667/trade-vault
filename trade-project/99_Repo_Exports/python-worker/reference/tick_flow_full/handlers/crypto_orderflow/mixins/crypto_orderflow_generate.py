...
class CryptoOrderFlowGenerateMixin:
    ...
    def _generate_signals(self, ctx: Any) -> bool:
        ...
        for cand in candidates:
            ...
            try:
                _rg = getattr(ctx, "market_regime", None) or getattr(ctx, "regime", None)
                if _rg is not None:
            ...
            # rc: uint16 numeric; rc16: base64-packed u16 (optional).
            ...
            payload["rc"] = rc_u16
            if pack_rc16:
                payload["rc16"] = pack_u16(rc_u16)
            if keep_reason_str:
                payload["reason_code"] = str(getattr(res, "reason_code", ReasonCode.OK.value))

            # FINAL: store compact uint16 codes in payload; string labels are expanded ONLY in formatter/publisher.
            qf_codes = getattr(res, "quality_codes", None) or ()
            if qf_codes and not isinstance(qf_codes, list):
                qf_codes = list(qf_codes)
            if qf_codes:
                payload["qf"] = qf_codes  # list[int] for debugging/inspectability
                # smaller wire format (base64 packed u16). formatter prefers qf16 if present.
                if cfg.qf_pack_u16:
                    payload["qf16"] = pack_u16_list(qf_codes)
