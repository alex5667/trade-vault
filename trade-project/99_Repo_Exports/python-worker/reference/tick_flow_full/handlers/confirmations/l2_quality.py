...
class L2QualityPolicy:
    ...
    def assess(self, *, kind: str, ctx: Any, l2: Any | None) -> L2Assessment:
        ...
        if len(bids) < min_levels or len(asks) < min_levels:
            ...
            if fail_open:
                return L2Assessment(0.45, False, flags, "l2_thin_fail_open")
        ...
        if is_crossed(bb, ba):
            ...
            if fail_open:
                return L2Assessment(0.35, False, flags, "l2_crossed_fail_open")
        ...
        if sp is None:
            ...
            if fail_open:
                return L2Assessment(0.40, False, flags, "l2_no_spread_fail_open")
        ...
        if sp > float(max_spread_bps):
            ...
            if fail_open:
                ...
                score = max(0.15, 1.0 - float(sp) / max(float(max_spread_bps) * 3.0, 1e-9))
        ...
        if wall_n < self.min_top_wall_notional:
            ...
            if fail_open:
                return L2Assessment(0.55, False, flags, "l2_no_wall_fail_open")
            return L2Assessment(0.0, True, flags, "l2_no_wall_fail_closed")

        soft = max(0.0, min(1.0, 1.0 - (float(sp) / max(float(max_spread_bps), 1e-9)) * 0.35))
