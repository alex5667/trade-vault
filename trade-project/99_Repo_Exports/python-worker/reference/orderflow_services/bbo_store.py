...
class BBOStoreCfg:
    ...
    def from_env() -> BBOStoreCfg:
        ...
        stream_maxlen = int(os.getenv("BBO_TS_STREAM_MAXLEN", "500000") or 500000)
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            sink = _Sink(str(cfg.stream), int(cfg.stream_maxlen))

        await publisher.xadd_json(
            sink=sink,
            payload={"payload": json.dumps(payload, ensure_ascii=False, separators=(",", ":"))},
            symbol=symbol,
            approximate=True,
        )
    except Exception:
