...
class SymbolRuntime:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            self.dynamic_cfg["atr_tf_selected"] = str(self.config.get("atr_tf", "1m") or "1m")
        except Exception:
            self.dynamic_cfg["atr_tf_selected"] = "1m"
        ...
        try:
            ms = int(self.config.get("atr_sanity_min_samples", int(os.getenv("ATR_SANITY_MIN_SAMPLES", "50"))) or 50)
            max_age = int(self.config.get("atr_sanity_max_age_ms", int(os.getenv("ATR_SANITY_MAX_AGE_MS", "120000"))) or 120000)
        except Exception:
            ms = 50
            max_age = 120000
    ...
    def get_atr_tf_selected(self) -> str:
        ...
        Single source of truth: dynamic_cfg["atr_tf_selected"] -> config -> fallback.
    ...
    async def ensure_specs_fresh(self, r) -> None:
        ...
        try:
            ...
            try:
                ...
                for _b, _v in zip(_slip_buckets, _slip_vals or []):
                    ...
                    try:
                        ...
                        pass
                if _coeff_map:
                    self.dynamic_cfg["slippage_decomp_impact_coeff_bps"] = _coeff_map
            except Exception:
                pass
            ...
            try:
                ...
                if sd:
                    try:
                        self.dynamic_cfg["slippage_decomp_enforce_buckets"] = str(sd)
                    except Exception:
                        pass
                    try:
                ...
                if tf:
                    try:
                        self.dynamic_cfg["taker_flow_gate_enforce_buckets"] = tf
                    except Exception:
                        pass
                    try:
                        if isinstance(self.config, dict):
