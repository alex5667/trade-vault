...
def ensure_trade_levels_once(
    *, ctx: Any, symbol: str, side: str, kind: str, cfg: dict[str, Any] | None,
    ...
    # --- SLQ dynamic stop override (fail-open) ---
    # IMPORTANT: must happen BEFORE cfg_hash/key to keep ctx-level caching consistent.
    try:
        from services.slq_risk_adjust import maybe_apply_slq_to_risk_cfg
        # normalize side: keep original side param; helper accepts Any
...
class GateRunner:
    ...
    def _emit_entry_policy_diag_best_effort(
        ...
        *,
        ...
        Policy:
          - VETO/DIAG events must NOT go to the tradeable outbox stream.
          - They go ONLY to ENTRY_POLICY_DIAG_STREAM (if configured).
        ...
        try:
            ...
            if r is None:
                return
            sid = str(getattr(getattr(f, "cand", None), "signal_id", "") or getattr(getattr(f, "ctx", None), "sid", "") or "").strip()
            # Correlation id is still useful even if SID is missing (legacy).
            trace_id = str(getattr(getattr(f, "ctx", None), "trace_id", "") or getattr(getattr(f, "ctx", None), "correlation_id", "") or sid or "").strip()
            if not trace_id:
                trace_id = sid
...
class ScoringRunner:
    ...
    def compute(self, f: CandidateFrame, res: Any) -> tuple[float, float, float, float, dict[str, Any]]:
        ...
        try:
            ...
            candidates_total(f.handler, kind=k)
...
class ConfidenceGateRunner:
    ...
    def check(self, f: CandidateFrame, *, confidence_pct: float, conf_factor01: float) -> tuple[bool, str]:
        ...
        min_cf = self._env_float(f"MIN_CONF_FACTOR_{sym_u}", self._env_float("MIN_CONF_FACTOR_DEFAULT", 0.45))
...
class PayloadBuilder:
    ...
    def build(
        ...
        *,
        ...
        # ------------------------------------------------------------------
        # FIX: Inject market regime into payload so downstream consumers
        # (create_position → entry_regime, stats_aggregator → regime_stats)
        # receive the actual computed regime instead of defaulting to "na".
        #
        # Source of truth priority:
        #   1) ctx.market_regime (set by RegimeDetector.detect())
        #   2) ctx.regime (legacy alias)
        #   3) of.regime (OrderFlow sub-context)
        # Canonicalization: MarketRegime enum → .value/.name; str → lower().
        # ------------------------------------------------------------------
        try:
            _rg = getattr(f.ctx, "market_regime", None) or getattr(f.ctx, "regime", None)
            if _rg is None:
                _of = getattr(f.ctx, "of", None)
                if _of is not None:
                    _rg = getattr(_of, "regime", None)
            if _rg is not None:
                _rg_str = str(getattr(_rg, "value", None) or getattr(_rg, "name", None) or _rg).strip().lower()
                if _rg_str:
                    payload["regime"] = _rg_str
        except Exception:
            pass
        ...
        try:
            ...
            if mt5_enabled:
                ...
                pos_size = float(getattr(f.ctx, "position_size", 0.0) or 0.01)
...
class OutboxWriter:
    ...
    def emit(
        ...
        *,
        ...
        try:
            # ------------------------------------------------------------
            # MT5 Payload Passing Strategy:
            # We temporarily embed mt5_payload in the payload dict with a special key.
            # SignalPublisher will extract it and pass to build_outbox_envelope.
            # This avoids changing the emitter interface signature.
            # ------------------------------------------------------------
            if mt5_payload is not None:
                try:
                    payload["__mt5_payload__"] = mt5_payload
                except Exception:
                    pass
...
class CandidateEmitPipelineV2:
    ...
    def __init__(self, handler: Any):
        ...
        # News enricher - Zero-IO shadow cache architecture (no Redis calls in tick-loop)
        try:
            from news_pipeline.enricher_shadow import NewsEnricherShadow

            redis_client = getattr(handler, "redis_news", None) or getattr(handler, "redis", None)
            ...
            if redis_client:
                ...
                self._news_enricher = self.news_enricher
            else:
    ...
    def emit(self, *, ctx: Any) -> bool:
        ...
        # News enrichment - fail-open, before processing candidates
        try:
            if self.news_enricher:
                # Extract deterministic timestamp from tick data
                ts_ms = int(getattr(ctx, "ts_ms", 0) or 0) or get_ny_time_millis()
                self.news_enricher.attach(ctx, now_ts_ms=ts_ms)
        except Exception:
        ...
        for f in frames:
            ...
            try:
                candidates_total(self.h, kind=str(getattr(f, "kind_str", "") or getattr(f, "kind", "") or ""))
            except Exception:
                pass
            ...
            if res is None:
                ...
                try:
                    ...
                    veto_total(self.h, kind=kind_s, reason_code="VETO_CONFIRMATIONS_NONE")
                except Exception:
                    pass
            ...
            if bool(getattr(res, "veto", False)):
                ...
                try:
                    stage_ms_hist(self.h, stage="gates", ms=float(sp_g2.ms()), kind=kind_s, symbol=sym)
                    veto_total(self.h, kind=kind_s, reason_code=veto_rc)
                except Exception:
                    pass
            ...
            # Stage 6: outbox
            sp_emit = Span()
            sent = self.writer.emit(f, payload, meta_extra=payload_meta, mt5_payload=mt5_payload)
            try:
            ...
            if sent:
                try:
                    emit_ok_total(self.h, kind=kind_s)
                except Exception:
                    pass
...
class ConfidenceGateRunner:
    ...
    def check(
        ...
        *,
        ...
        # rt приходит из RuntimeRefresher (мягкий refresh раз в N секунд).
        # ------------------------------------------------------------------
        if rt is None:
            # fallback (совместимость) — но желательно всегда передавать rt
