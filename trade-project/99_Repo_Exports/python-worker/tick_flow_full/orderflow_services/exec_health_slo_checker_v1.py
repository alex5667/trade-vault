...
def _scan_keys(r, prefix: str) -> list[str]:
    ...
    try:
        cur = 0
        while True:
            cur, batch = r.scan(cursor=cur, match=patt, count=10000)
            out.extend([str(x) for x in batch or []])
            if int(cur) == 0:
                break
    except Exception:
        return []
...
def main() -> int:
    ...
    ap.add_argument("--stale-ms", type=int, default=int(os.getenv("EXEC_HEALTH_SLO_STALE_MS", "90000")))
    ...
    try:
        keys = _scan_keys(r, args.state_prefix)
        rows = _read_rows(r, keys)

        now = _now_ms()
        # Auto-cleanup: if key is older than 1 hour, delete it
        VERY_STALE_MS = 3600 * 1000
        for row in rows:
            key = row.get("_key")
            upd = _i(row.get("updated_ts_ms"), 0)
            if key and upd > 0 and (now - upd) > VERY_STALE_MS:
                logger.warning("exec health slo: cleaning up very stale key %s (age > 1h)", key)
                r.delete(key)

        summary = build_summary(rows, now_ms=now, stale_ms=max(1000, int(args.stale_ms)))
