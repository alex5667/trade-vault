...
def compute_absorption_level_score(
    *,
    ...
    """
    bias = str(_get(bar, "fp_absorption_bias", "NONE") or "NONE").upper()
    direction_u = str(direction).upper()
    dir_match = (bias in ("LONG", "SHORT") and bias == direction_u)

    low_len = _i(_get(bar, "fp_ladder_low_len", 0), 0)
    high_len = _i(_get(bar, "fp_ladder_high_len", 0), 0)
    ladder_len = max(low_len, high_len)

    poc_on_edge = _i(_get(bar, "fp_poc_on_edge", 0), 0)
    # Prefer portable eff_quote; fallback to legacy eff_delta if missing.
    eff_q = _f(_get(bar, "fp_eff_quote", 0.0), 0.0)
    eff = eff_q if eff_q > 0.0 else _f(_get(bar, "fp_eff_delta", 0.0), 0.0)
