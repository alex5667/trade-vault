...
# inside _fit_model() when GPU training is actually needed.
# ---------------------------------------------------------------------------

def _parse_bool_env_fs(val: str) -> bool:
    return str(val).strip().lower() in ("1", "true", "yes", "on")

_GPU_ENABLED_FS_ENV = _parse_bool_env_fs(os.getenv("GPU_ENABLED", "1") or "1")
_GPU_FS_CHECKED = [None]  # mutable cache


def _check_gpu_xgb_fs() -> bool:
    """Lazy GPU+XGBoost check for feature selection."""
    if _GPU_FS_CHECKED[0] is not None:
        return _GPU_FS_CHECKED[0]
    if not _GPU_ENABLED_FS_ENV:
        _GPU_FS_CHECKED[0] = False
        return False
    try:
...
def _fit_model(
    ...
    *,
    ...
    if m in ("gbdt", "hgb", "hist"):
        # GPU XGBoost: 10-50x faster for N > 5000, seamless CPU fallback
        if _check_gpu_xgb_fs() and len(X_tr) > 5_000:
            try:
                ...
                pass  # fallback to sklearn below
