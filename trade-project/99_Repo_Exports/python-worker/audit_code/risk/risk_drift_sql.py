...
class RiskDriftSqlSink:
    ...
    def from_env(cls) -> RiskDriftSqlSink:
        ...
        enabled = os.getenv('TRADE_RISK_DRIFT_LEDGER_ENABLE', '1').strip().lower() in {'1', 'true', 'yes', 'on'}
        ...
        try:
            with psycopg.connect(self.dsn) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        """
                        (
                            (doc.get('decision_id') or '') or None,
                            (doc.get('sid') or '') or None,
                            (doc.get('signal_id') or '') or None,
                            (doc.get('symbol') or ''),
                            (doc.get('tier') or ''),
                            ...
                            (doc.get('source') or 'risk_consistency_checker'),
                            (doc.get('quarantine_action') or 'REPEATED_MISMATCH_QUARANTINED'),
                            now_ms,
                        ),
                    )
                conn.commit()
            if TRADE_RISK_DRIFT_LEDGER_WRITE_TOTAL:
                TRADE_RISK_DRIFT_LEDGER_WRITE_TOTAL.labels(
                    action=(doc.get('quarantine_action') or 'REPEATED_MISMATCH_QUARANTINED')
                ).inc()
            return True
        except Exception:
