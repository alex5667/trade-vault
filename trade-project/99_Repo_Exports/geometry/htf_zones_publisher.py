...
def _hash_payload(s: str) -> str:
    """Lightweight stable hash (hashlib imported at module level)."""
...
class HTFZonesPublisher:
    ...
    def _init_htf_service(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.warning("HTF Redis provider unavailable, falling back to no-provider: %s", e)
                self._htf = HTFLevelsService()
        except Exception as e:
            logger.error("Failed to initialize HTFLevelsService: %s", e)
            self._htf = None
    ...
    def _levels_to_zones(self, levels: List[Any], symbol: str, ts_ms: int) -> Dict[str, Any]:
        ...
        for lv in levels:
            try:
                ltype = _safe_str(
                    getattr(lv, "level_type", None)
                    or getattr(lv, "type", None)
                    or getattr(lv, "kind", None),
                    "NA",
                )
                px = _safe_float(
                    getattr(lv, "price", None)
                    or getattr(lv, "px", None)
                    or getattr(lv, "value", None),
                    0.0,
                )
        ...
        return {"v": 1, "symbol": symbol, "ts_ms": int(ts_ms), "zones": zones}
    ...
    async def _compute_levels(self, symbol: str, ts_ms: int) -> List[Any]:
        ...
        try:
            ...
            logger.warning("_compute_levels(%s) error: %s", symbol, e)
            return []
    ...
    async def tick_once(self) -> int:
        ...
        n = 0
        now = _now_ms()
        min_gap_ms = int(self.cfg.min_change_publish_sec * 1000.0)
        for sym in self.cfg.symbols:
            # symbols are already normalised in PubCfg.from_env()
            lp = self._last_pub_ts_ms.get(sym, 0)
            if lp > 0 and (now - lp) < min_gap_ms:
                continue
            ...
            try:
                ...
                logger.warning("Redis GET failed for %s: %s", hkey, e)
                prev = None
            if prev == h:
                continue
            try:
