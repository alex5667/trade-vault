...
class Trade:
    ...
    Ожидается, что все источники (Redis/Postgres) будут приводить записи к этому виду.
    """
...
class TagStats:
    ...
    Используется одинаково для:
      - глобальной сводки (tag="__ALL__"),
      - любых групп: entry_tag, strategy, source+symbol и т.п.
    """
    ...
    # -------------------------
    # Агрегация одной сделки
    # -------------------------
    def add_trade(self, t: Trade) -> None:  # noqa: PLR0912
        self.n += 1
        ...
        # Всё, что зависит от 1R — вычисляем один раз и переиспользуем
        r_m: float | None = None
        r_b: float | None = None
        ...
        # Трейлинг — переиспользуем уже вычисленные r_m / r_b
        trailing_flag = t.trailing_started or t.trailing_active
        if trailing_flag:
            self.n_trailing += 1
            if r_m is not None and r_b is not None:
        ...
        is_trailing_close = "TRAILING" in cr or "TRAILING" in crd
    ...
    def finalize(self) -> dict[str, Any]:
        ...
        res: dict[str, Any] = {"tag": self.tag, "n": self.n}
        ...
        # Win-rate
        res["wr_managed"] = self.n_win / self.n
        res["wr_baseline"] = self.n_win_fixed / self.n
        ...
        res["payoff_r"] = (avg_win_r / abs(avg_loss_r)) if avg_loss_r < 0 else 0.0
        ...
        res["payoff_usd"] = (avg_win_usd / abs(avg_loss_usd)) if avg_loss_usd < 0 else 0.0
        ...
        res["payoff_fixed_usd"] = (avg_win_fixed_usd / abs(avg_loss_fixed_usd)) if avg_loss_fixed_usd < 0 else 0.0
        ...
        res["payoff_fixed_r"] = (avg_win_fixed_r / abs(avg_loss_fixed_r)) if avg_loss_fixed_r < 0 else 0.0
        ...
        res["giveback_share"] = self.n_giveback_pos / self.n
        ...
        res["missed_share"] = self.n_missed_pos / self.n
        ...
        # Трейлинг
        res["trailing_share"] = self.n_trailing / self.n
