...
def _fields_to_dict(fields: Any) -> Dict[str, Any]:
    """
    Normalizes field representation.
    redis-py usually returns dict, but execute_command/raw RESP can return list [k, v, k, v, ...].
    """
    if fields is None:
        return {}
        ...
        *,
        ...
        if not success and res is None:
            ...
            # If it IS a NOGROUP error, auto-create the consumer group
                ...
                # If we successfully read after group creation, break from outer loop
                if success:
                    break
...
class AsyncRedisStreamHelper:
        ...
        while retry_count < max_retries:
            try:
                ...
                if "loading" in error_msg:
                    ...
                    raise
        raise RuntimeError(f"Redis still loading after {max_retries} attempts")
    ...
    async def read(
        ...
        *,
        ...
        try:
            ...
            if not isinstance(exc, ResponseError) or "nogroup" not in exc_str:
                raise exc
            for stream_name in normalized.keys():
                try:
    ...
    async def claim_pending(
        ...
        """Async XAUTOCLAIM wrapper to recover pending entries (PEL).

        Returns:
            (next_start_id, [StreamMsg...])
        """
        if not stream:
            return start_id, []
        ...
        try:
            ...
            if "NOGROUP" in err_s:
                ...
                try:
                    await self.client.xgroup_create(
                        stream, self.group, id=create_group_start_id, mkstream=True
                    )
                except ResponseError as create_exc:
                    if "BUSYGROUP" not in str(create_exc):
                        logger.warning("Failed to create consumer group for %s: %s", stream, create_exc)
                # Retry after group creation
                try:
                    res = await self.client.execute_command(
                        "XAUTOCLAIM",
                        stream,
                    ...
                    if _is_unsupported_xautoclaim(e2):
                        logger.warning("XAUTOCLAIM unsupported after NOGROUP recovery for %s", stream)
                        return start_id, []
                    raise
            elif _is_unsupported_xautoclaim(exc):
                logger.warning("XAUTOCLAIM unsupported for %s", stream)
                return start_id, []
            else:
                raise
        except Exception as e:
            if _is_unsupported_xautoclaim(e):
                logger.warning("XAUTOCLAIM unsupported (generic catch) for %s", stream)
                return start_id, []
            raise

        # Parse result: [next_id, [(id, fields), ...], [deleted_ids...]]
        if not res or not isinstance(res, (list, tuple)) or len(res) < 2:
            return start_id, []
        ...
        # If scan finished (next_id="0-0") and we got no messages, keep start_id
        if next_id == "0-0" and not msgs:
            return start_id, []
        return next_id, msgs

    async def pending_len(self, stream: str) -> int:
        """Async XPENDING summary length for (stream, group)."""
        if not stream:
            return 0
        try:
    ...
    async def autoclaim_pending(
        ...
        *,
        ...
        """Claim stale pending messages (PEL) for this consumer.

        Uses XAUTOCLAIM when available (Redis >= 6.2). Falls back to XPENDING+XCLAIM on older servers.
        Returns a flat list of (message_id, fields_dict) compatible with xreadgroup output entries.
        """
        ...
        for _ in range(max_iters):
            try:
                ...
                if _is_unsupported_xautoclaim(exc):
                    return await self._claim_via_xpending(stream, min_idle_ms=min_idle_ms, count=count)
                raise

            if not res or len(res) < 2:
                break

            next_id = _decode_any(res[0])
            entries = res[1] or []
            ...
            for ent in entries:
                try:
                    if isinstance(ent, (list, tuple)) and len(ent) >= 2:
                        mid = _decode_any(ent[0])
                        fields = _fields_to_dict(ent[1])
                        if mid:
                            batch.append((mid, fields))
                except Exception:
                    continue
            ...
            # If server doesn't advance cursor, avoid infinite loops.
            if not next_id or next_id == sid:
                break
            sid = next_id

        return out

    async def _claim_via_xpending(
        self,
        stream: str,
        *,
        min_idle_ms: int,
        ...
        """Fallback for Redis without XAUTOCLAIM: XPENDING range -> XCLAIM."""
        try:
            pending = await self.client.execute_command(
                "XPENDING",
                stream,
                self.group,
                "-",
        ...
        for it in pending or []:
            try:
                if isinstance(it, (list, tuple)) and len(it) >= 3:
                    mid = _decode_any(it[0])
                    idle = int(it[2] or 0)
                    if mid and idle >= int(min_idle_ms):
                        ids.append(mid)
            except Exception:
                continue
        ...
        try:
            claimed = await self.client.execute_command(
                "XCLAIM",
                stream,
                self.group,
                self.consumer,
        ...
        out: List[Tuple[str, Dict[str, Any]]] = []
        for ent in claimed or []:
            try:
                if isinstance(ent, (list, tuple)) and len(ent) >= 2:
                    out.append((_decode_any(ent[0]), _fields_to_dict(ent[1])))
            except Exception:
                continue
        return out
