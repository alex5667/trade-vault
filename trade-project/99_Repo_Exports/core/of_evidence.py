...
def compute_sweep_recent(
    *,
    ...
    ts = _i(getattr(last_sweep, "ts_ms", 0), 0)
    ...
    try:
        indicators["sweep_kind"] = _s(getattr(last_sweep, "kind", ""), "")
    except Exception:
        pass
    try:
        indicators["sweep_dir_bias"] = _s(getattr(last_sweep, "direction_bias", ""), "")
    except Exception:
        pass
    return True
...
def compute_reclaim_recent(
    *,
    ...
    ts = _i(getattr(last_reclaim, "ts_ms", 0), 0)
    age = (now_ts_ms - ts) if ts > 0 else 10**9
    ...
    bias = _s(getattr(last_reclaim, "direction_bias", ""), "").upper()
    if bias and bias != str(direction).upper():
        return False, 0

    hold_bars = _i(getattr(last_reclaim, "hold_bars", 0), 0)
    if hold_bars <= 0:
        hold_bars = _i(cfg.get("reclaim_hold_bars", 2), 2)

    # Optional diagnostic fields
    try:
        indicators["reclaim_level"] = float(getattr(last_reclaim, "level", 0.0) or 0.0)
    except Exception:
        pass
    try:
        indicators["reclaim_pool_id"] = _s(getattr(last_reclaim, "pool_id", ""), "")
    except Exception:
        pass
