...
class ATRTfCalibrator:
    ...
    def __init__(
        ...
        *,
        candidates: List[str],
        max_age_ms_by_tf: Optional[Dict[str, int]] = None,
    ...
    def choose(
        ...
        *,
        ...
        now_ms: int,
        atr_cache: Any,
    ) -> Optional[ATRTfChoice]:
        sym = str(symbol or "").upper()
        px = float(price or 0.0)
        if px <= 0:
            return None

        best: Optional[ATRTfChoice] = None
    ...
    @staticmethod
    def load_choice(state: Dict[str, Any]) -> Optional[ATRTfChoice]:
        try:
            if not isinstance(state, dict):
                return None
            tf = str(state.get("tf") or "").strip()
            if not tf:
                return None
            return ATRTfChoice(
                tf=tf,
                src=str(state.get("src") or "none"),
