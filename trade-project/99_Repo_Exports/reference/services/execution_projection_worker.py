...
try:  # pragma: no cover
    ...
    try:
        from execution_metrics import EXECUTION_ACTIVE_SYMBOL_GUARD_CAS_TOTAL
    except Exception:
...
class ExecutionProjectionWorker:
    ...
    def _persist_active_symbol_state(self, state_doc: Mapping[str, Any]) -> None:
        ...
        # P6: evaluate terminal-ish state once and reuse for both branches
        is_terminalish = self._state_is_terminalish(doc)
        if is_terminalish and not self.exchange_truth_release:
            # P7: use release tombstone instead of physical deletion
            try:
                res = self._active_symbol_guard_store().mark_released(
                    symbol=symbol,
                    expected_sid=sid,
                    release_reason="projection_terminal_clear",
                    writer="projection"
                )
    ...
    def run_once(self) -> ProjectionBatchResult:
        ...
        for stream_id, fields in rows:
            try:
                ...
                print(f"Error projecting row {stream_id}: {e}", flush=True)
                if TRADE_EXECUTION_PROJECTION_EVENT_TOTAL:
    ...
    def health_snapshot(self, *, lag_readyz_max_ms: int = 30000) -> Dict[str, Any]:
        ...
        # Fix: True lag is the distance to the tip of the stream, not wall clock age.
        # If we are exactly at the tip, we have 0 lag, even if the last event was hours ago.
    ...
    def reset_derived_state(self, *, dry_run: bool = True) -> int:
        ...
        try:
            ...
            cursor_val = 0
            while True:
                cursor_val, keys = self.r.scan(cursor_val, match=pattern, count=200)
                if keys:
                    self.r.delete(*keys)
                    count += len(keys)
                if cursor_val == 0:
...
def _worker_from_env(r: Any) -> ExecutionProjectionWorker:  # pragma: no cover
    ...
    return ExecutionProjectionWorker(
        ...
        exchange_truth_release=os.getenv('EXEC_SINGLE_ACTIVE_POSITION_EXCHANGE_TRUTH_RELEASE', '1').strip() == '1',
...
def main() -> int:  # pragma: no cover
    ...
    while True:
        try:
            ...
            print(f"Exception in worker.run_once(): {e}", flush=True)
            import traceback
            traceback.print_exc()
            time.sleep(1)
            continue
