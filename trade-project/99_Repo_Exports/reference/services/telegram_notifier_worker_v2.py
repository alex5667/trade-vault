...
# Constants
STREAM_KEYS = {
    RS.NOTIFY_TELEGRAM: "notify_telegram_group",      # Main info stream — handles ok:INFO SLO counters
    RS.NOTIFY_TELEGRAM_CRIT: "notify_telegram_crit_group",
    RS.NOTIFY_TELEGRAM_PAGE: "notify_telegram_page_group"
}
...
def process_message(r, stream_key, message_id, message_data):
    ...
    try:
        ...
        if stream_key in COUNTER_ONLY_STREAMS:
            ...
            r.set("notify:last_ok_ts_ms", int(time.time() * 1000))
            r.set(f"notify:last_ok_ts_ms:{severity.upper()}", int(time.time() * 1000))
        ...
        if sent:
            ...
            r.set("notify:last_err_ts_ms", int(time.time() * 1000))
            r.set(f"notify:last_err_ts_ms:{severity.upper()}", int(time.time() * 1000))
            ...
            # Strategy: we ACK even on failure to avoid blocking forever, 
            # monitoring will catch the high burn rate.
            return True
                    ...
                    for _, messages in items:
                        for message_id, message_data in messages:
                            ...
                            try:
                                ...
                                lag_ms = int(time.time() * 1000) - msg_ts_ms
