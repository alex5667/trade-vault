...
class OFConfirmService:
        ...
        while self.running:
            try:
                ...
                for sym, raw in zip(symbols, values):
                    ...
                    ts_ms = _i(_f(ts_sec, 0.0) * 1000) if ts_sec is not None else _i(time.time() * 1000)
                    book_for_detector = {
                        "bids": book.get("bids", []),
                        "asks": book.get("asks", []),
                        "ts_ms": ts_ms,
    ...
    async def _check_spike(self, symbol: str, state: SymbolState, spike: Dict[str, Any]):
        ...
        try:
            ...
            ts_ms = spike.get("ts_ms", int(time.time()*1000))
            ...
            if of_confirm and of_confirm.ok:
                ...
                out_payload["generated_at"] = int(time.time() * 1000)
    ...
    async def _ensure_microbar_groups(self) -> List[str]:
        ...
        if "{sym}" not in self.stream_bars_template:
            ...
            seen = 0
            while True:
                cursor, batch = await self.redis.sscan(self.microbar_symbols_set, cursor=cursor, count=500)
                for s in batch or []:
