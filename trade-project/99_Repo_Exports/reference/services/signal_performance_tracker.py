...
- Чтение сигналов из Redis Streams (signals:orderflow:XAUUSD, signals:ta:XAUUSD)
- Чтение тиков из stream:tick_XAUUSD
...
class IncomingSignal(BaseModel):
    ...
    def normalize_direction(cls, v):
        ...
        if v_upper in ("BUY", "LONG"):
            return "LONG"
        if v_upper in ("SELL", "SHORT"):
            return "SHORT"
        return v_upper

    model_config = ConfigDict(
        extra='ignore',  # Ignore extra fields in payload
...
class SignalPerformanceTracker:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        try:
            pg_dsn = os.getenv("DATABASE_URL", "postgresql://trading:trading_password@localhost:5434/trade")
        ...
        try:
            ...
            if stream == self.crypto_raw_stream:
                ...
                try:
                    ...
                    lag = int(time.time() * 1000) - int(ts_part)
            ...
            def _run():
                ...
                try:
                    ...
                    if entry_ts > 0:
                        lat_ms = int(time.time() * 1000) - entry_ts
    ...
    def _process_event_message(self, stream: str, msg_id: str, data: Dict[str, Any]) -> bool:
        ...
        if event_type == "SL_HIT":
            ...
            ts_value = normalize_ts_ms(ts_raw) or int(time.time() * 1000)
    ...
    def _handle_tp_event(self, position: PositionState, tp_event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            trailing_result = self.trailing_orchestrator.start_trailing(
                ...
                event_ts=normalize_ts_ms(tp_event.get("timestamp")) or int(time.time() * 1000)
            )
            ...
            applied = self.trade_monitor.apply_trailing_sl_sync(
                ...
                ts_ms=int(tp_event.get("timestamp") or time.time() * 1000),
    ...
    def _create_consumer_groups(self) -> None:
        ...
        while retry_count < max_retries:
            try:
                self.redis.xgroup_create("events:trades", self.events_group, id='$', mkstream=True)
    ...
    def _ticks_listener_thread(self) -> None:
        ...
        Читает stream:tick_XAUUSD, stream:tick_BTCUSDT и т.д.
