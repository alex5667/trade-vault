...
class SignalTargetWorker:
    ...
    def __init__(self, target: str):
        ...
        self.notify_counter_key = os.getenv("NOTIFY_SIGNAL_COUNTER_KEY", RK.NOTIFY_SIGNAL_COUNTER)
        try:
            self.notify_every_n = max(1, int(os.getenv("CRYPTO_NOTIFY_SIGNAL_EVERY_N", "1")))
        except Exception:
    ...
    def _schedule_retry(self, sid: str, task: Dict[str, Any], attempt: int, err: Exception) -> None:
        ...
        due_ms = int(time.time() * 1000) + int(delay)
        task2 = dict(task)
        task2["attempt"] = a
    ...
    def _send_dlq(self, reason: str, data: Any) -> None:
        payload = {"ts": int(time.time() * 1000), "reason": reason, "target": self.target, "data": data}
        try:
            self.redis.xadd(self.dlq_stream, {"data": json.dumps(payload, ensure_ascii=False)}, maxlen=50000, approximate=True)
