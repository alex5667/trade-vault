...
class PeriodicReporter:
    ...
    def _generate_and_send_report_internal(self, source: str, symbol: str, window_seconds: Optional[int] = None):
        ...
        try:
            ...
            if trades:
                ...
                # User requested to base the main report on ALL closed trades, 
                # without distinguishing between real and virtual in the main section.
                metrics = m_all
                ...
                metrics["real_metrics"] = m_real
                metrics["real_trades_total"] = real_total
                metrics["virtual_trades_total"] = virtual_total
                metrics["report_virtual_only"] = False
                metrics["total_trades"] = combined_total
    ...
    def _gather_window_metrics_stream(self, source: str, symbol: str, window_seconds: Optional[int] = None) -> Dict[str, any]:
        ...
        cutoff_ms = 0 if trade_window_count else int(time.time() * 1000) - target_window * 1000
        min_id = f"{cutoff_ms}-0"
    ...
    def _send_report(self, source: str, symbol: str, m: Dict[str, any], window_seconds: Optional[int] = None) -> None:
        try:
            ...
            virtual_total = int(m.get("virtual_trades_total", 0))
            combined_total = real_total + virtual_total
            ...
            sampled_info(
                ...
                f"📨 _send_report вызван для {source}/{symbol}: real={real_total}, virtual={virtual_total}, send_empty={send_empty}"
            )
            ...
            if combined_total <= 0 and not send_empty:
                logger.debug(f"⏭️ Пропуск пустого отчета для {source}/{symbol} (combined_total=0)")
                return

            if not is_scheduled and combined_total < REPORT_TRIGGER_COUNT and REPORT_TRIGGER_COUNT > 0:
                logger.debug(f"⏭️ Пропуск отчета для {source}/{symbol} (всего {combined_total} сделок, нужно {REPORT_TRIGGER_COUNT})")
                return
            ...
            if combined_total < min_trades_for_report:
                sampled_info(
                    logger,
                    "PERIODIC_REPORTER_SKIP_INSUFFICIENT",
                    f"⏭️ Пропуск отчета для {source}/{symbol}: недостаточно сделок ({combined_total} < {min_trades_for_report})"
                )
                return
            ...
            # Unified: always show both Virtual and Real lines
            # Get real metrics from embedded real_metrics bucket
            real_m = m.get("real_metrics") or {}
            r_total = int(real_m.get("total_trades", 0))
            r_wins = int(real_m.get("wins", 0))
            r_pnl = float(real_m.get("total_pnl", 0.0))
            r_wr = (r_wins / r_total * 100.0) if r_total > 0 else 0.0

            signal_shadow_lines = [
                "<b>📊 Signal & Shadow Analytics</b>",
                f"Виртуальные сделки: <b>{v_all_c}</b> | WR: <b>{v_all_wr:.1f}%</b> | PnL: <b>{v_all_pnl:+.2f}</b>",
                f"Реальные сделки: <b>{r_total}</b> | WR: <b>{r_wr:.1f}%</b> | PnL: <b>{r_pnl:+.2f}</b>",
            ...
            title_text = f"Отчет: {html.escape(str(source))}"
            sections: list[str] = [
    ...
    def _gather_trades_for_trailing_analysis(self, source: str, symbol: str, limit: int = 500) -> List[ClosedTradeSnapshot]:
        ...
        cutoff_ms = 0 if trade_window_count else int(time.time() * 1000) - RECENT_WINDOW_SECONDS * 1000
        min_id = f"{cutoff_ms}-0"
    ...
    def _get_validation_stats(self, source: str, symbol: str, window_seconds: Optional[int] = None) -> Tuple[float, int, Dict[str, float], int, int]:
        ...
        try:
            ...
            _score_veto_min: float = float(os.getenv("OF_SCORE_MIN", "0.40"))
            ...
            if len(pairs) < 500:  # Limit increased
                try:
                    ...
                    for pattern in ["signals:orderflow:*", "signals:cryptoorderflow:*", "signals:ta:*", "signals:aggregated:*"]:
                        ...
                        keys = list(self.redis.scan_iter(match=pattern, count=1000))[:500]
