...
def normalize_liq_event(fields: Dict[str, object]) -> Tuple[Optional[LiqEventV1], Optional[str]]:
    ...
    ts_event_ms = _safe_int(fields.get("ts_event_ms"), -1)
    ts_ingest_ms = _safe_int(fields.get("ts_ingest_ms"), -1)

    venue = _safe_str(fields.get("venue")).strip()
    symbol = _safe_str(fields.get("symbol")).strip().upper()
    order_side = _safe_str(fields.get("order_side")).strip().upper()
