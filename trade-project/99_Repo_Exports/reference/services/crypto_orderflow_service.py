...
class CryptoOrderflowService:
    def __init__(self, redis_dsn: str, ticks_dsn: Optional[str] = None) -> None:
        ...
        sock_to = float(os.getenv("REDIS_SOCKET_TIMEOUT", "30"))
        ...
        self.score_calibrator = RollingPercentileCalibrator(
            cfg=ConfidenceCalibratorCfg(
                window=int(os.getenv("CONF_CALIB_WINDOW", "2000")),
                min_history=int(os.getenv("CONF_CALIB_MIN_HISTORY", "30")),
                fallback_k=float(os.getenv("CONF_CALIB_FALLBACK_K", "8.0"))
            )
        )
        ...
        self.trade_risk_engine_v2_enable = (
            str(os.getenv("TRADE_RISK_ENGINE_V2_ENABLE", os.getenv("RISK_ENGINE_V2_ENABLE", "1"))).strip().lower()
            in {"1", "true", "yes", "on"}
        )
        ...
        self.raw_signal_stream = os.getenv("CRYPTO_RAW_STREAM", RS.CRYPTO_RAW)
        # MT5 orders queue. Binance uses orders:queue:binance (binance_executor).
        self.orders_queue = os.getenv("ORDERS_QUEUE_MT5") or os.getenv("ORDERS_QUEUE") or RS.ORDERS_QUEUE_MT5
        ...
        logger.info("   Telegram every_n: %s", os.getenv("CRYPTO_NOTIFY_SIGNAL_EVERY_N", "1"))
    ...
    def _build_portfolio_risk_input(self, runtime: Any, signal: Dict[str, Any]) -> Optional["PortfolioRiskInput"]:
        ...
        return PortfolioRiskInput(
            ...
            expected_edge_bps=float(signal.get("expected_edge_bps") or signal.get("edge_bps") or 0.0),
            fee_bps=float(signal.get("fee_bps") or signal.get("estimated_fee_bps") or 0.0),
            ...
            net_beta=float(signal.get("net_beta") or signal.get("beta") or 1.0),
            leader_symbol=str(signal.get("leader_symbol") or 'BTCUSDT'),
            leader_drawdown_bps=float(signal.get("leader_drawdown_bps") or 0.0),
            news_blackout=bool(signal.get("news_blackout") or signal.get("high_vol_blackout") or signal.get("news_blackout_active")),
        ...
        while not self._shutdown:
            ...
            try:
                ...
                if "NOGROUP" in str(exc):
                    ...
                    if is_transient_redis_error(exc):
                        delay = backoff.get_delay()
                        logger.warning("⚠️ (%s) Transient ошибка чтения стрима [%s]: %s (backoff=%.2fs)", symbol, str(stream), exc, delay)
                        await asyncio.sleep(delay)
                        continue
                    else:
                ...
                if is_transient_redis_error(exc):
                    delay = backoff.get_delay()
                    logger.warning("⚠️ (%s) Transient ошибка чтения стрима [%s]: %s (backoff=%.2fs)", symbol, str(stream), str(exc), delay)
                    await asyncio.sleep(delay)
                    continue
                else:
                    logger.error("❌ (%s) Критическая ошибка чтения стрима [%s]: %s", symbol, str(stream), exc)
                    delay = backoff.get_delay()
                    await asyncio.sleep(delay)
                    continue
