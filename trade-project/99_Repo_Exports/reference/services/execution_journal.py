...
class ExecutionJournalSink:
            ...
            if writer is not None:
                try:
                    writer.enqueue({
                        ...
                        "event_ts_ms": _i(payload.get("ts_ms") or int(time.time() * 1000)),
    ...
    def upsert_order_snapshot(self, state: Dict[str, Any]) -> bool:
        ...
        doc = dict(state or {})
        now_ms = _i(doc.get('ts_ms') or int(time.time() * 1000))
        try:
            with self._get_conn_ctx() as conn:
                with conn.cursor() as cur:
                    cur.execute(sql, (
            ...
            with self._get_conn_ctx() as conn:
                with conn.cursor() as cur:
                    cur.execute(sql, (
                        ...
                        _i(s.get('updated_at_ms') or int(time.time() * 1000)),
                    ))
                conn.commit()
            return True
        except Exception:
