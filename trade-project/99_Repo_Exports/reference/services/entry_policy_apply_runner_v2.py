...
class EntryPolicyApplyRunnerV2:
    def __init__(self) -> None:
        ...
        # Bounded pool: apply runner is low-throughput (periodic, not tick-loop)
        _max_conn = int(os.getenv("ENTRY_POLICY_REDIS_MAX_CONN", "10"))
        ...
        self.telegram_stream = os.getenv("TELEGRAM_NOTIFY_STREAM", RS.NOTIFY_TELEGRAM)
        ...
        if kind in ("meta_model_freeze", "meta_model_unfreeze"):
            ...
            for op in ops:
                ...
                if cmd == "HSET" and key and field:
                    ...
                    pipe.set(key, val)
            pipe.set(f"{self.applied_prefix}:{sid}", str(int(time.time() * 1000)))
            await pipe.execute()
    ...
    async def run_once(self, kind: str = "active_arm") -> int:
        ...
        try:
            ...
            if kind == "overrides_v1":
                ...
                cur = 0
                while True:
                    cur, keys = await self.r.scan(cur, match="cfg:suggestions:entry_policy:latest:*", count=200)
                    for k in keys or []:
