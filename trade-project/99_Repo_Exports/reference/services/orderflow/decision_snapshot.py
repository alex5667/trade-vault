...
class DecisionSnapshotContractDTO(BaseModel):
    """Pydantic Type-Checked payload for decision_snapshot."""
...
def build_decision_snapshot_event(
    *,
    ...
    # Validate output through strict Pydantic model before returning
    try:
        validated = DecisionSnapshotContractDTO(**out)
        return validated.model_dump(exclude_none=False)
    except Exception as e:
        import logging
        logging.getLogger("decision_snapshot").warning("DecisionSnapshot Pydantic validation failed: %s", e)
