...
class SignalPipeline:
    ...
    async def publish_signal(self, runtime: SymbolRuntime, signal: Dict[str, Any]) -> None:
        ...
        gate_mode = os.getenv("ATR_GATE_MODE", os.getenv("FEES_AWARE_GATE_MODE", "ENFORCE")).upper()
        ...
        try:
            deriv_profile = str(os.getenv("DERIV_CTX_PROFILE", os.getenv("GATE_PROFILE", "default")) or "default").strip().lower()
        ...
        if atr and entry and atr > 0 and entry > 0:
            try:
                # Prioritize explicit calibrator overrides if they exist
                trail_calib = getattr(runtime, "calibrated_specs", {}).get("trailing", {})
                calib_cb_mult = float(trail_calib.get("trail_atr_mult") or os.getenv("BINANCE_TRAIL_ATR_MULT", "1.0"))
        ...
        # In range, we don't trail the market, we "unwrap inventory in doses".
        # We drop TP3 (trimming tp_levels) and force SL to break-even only (protective profile).
        ...
        if is_range_regime_flag:
            enriched_signal["tp_ratio"] = [0.60, 0.40]
        ...
        if notify_enabled and self.publisher.r and telegram_payload:
             ...
             notify_signal_every_n = int(os.getenv("CRYPTO_NOTIFY_SIGNAL_EVERY_N", "1"))
             msg_id = int(ts_ms)
        ...
        *,
        ...
        try:
            ...
            if mirror_all:
                ...
                binance_queue = os.getenv("ORDERS_INTENT_BINANCE", "orders:intent:binance")
