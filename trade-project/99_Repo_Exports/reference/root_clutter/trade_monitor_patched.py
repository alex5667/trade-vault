...
from core.autopilot_fields import extract_autopilot_fields
...
class TradeMonitorService:
    ...
    def _safe_save_trade_to_db(self, closed: TradeClosed) -> None:
        """
        Helper explicitly for running in _db_executor.
        Catches exceptions to prevent thread pool crash (though TPE swallows them usually).
        """
        try:
    ...
    def _safe_trigger_report(self, source: str, symbol: str, counter_type: str, order_id: str) -> None:
        ...
        try:
            ...
            check_and_trigger_report(source, symbol, counter_type=counter_type, order_id=order_id)
        except Exception as e:
    ...
    # --------------------
    # RegimeGuard async persist helpers
    # --------------------
    def _rg_pending_add(self, delta: int) -> None:
        try:
            ...
            return
        # gauge-like observe (fail-open)
        try:
            self._m_obs("tm_regime_guard_persist_pending", float(pending), None)
        except Exception:
            return

    def _submit_regime_guard_persist_task(
        self,
        task: Callable[[], None],
        tags: Optional[Dict[str, Any]] = None,
    ) -> None:
        # If disabled, run inline (legacy behavior).
        if not getattr(self, "_rg_async_persist", False):
            try:
                task()
            except Exception as e:
                logger.warning("regime guard persist failed (sync): %s", e)
            return
        ...
        if not sem:
            ...
            try:
                ...
                logger.warning("regime guard persist failed (sync/sem-missing): %s", e)
            return

        # Backpressure: do not block trading loop. If queue is full -> drop persist, record metric.
        acquired = False
        try:
            acquired = sem.acquire(blocking=False)
        except Exception:
            acquired = True  # fail-open: do not lose persist if semaphore is broken

        if not acquired:
            try:
                self._m_inc("tm_regime_guard_persist_dropped_total", 1, tags)
            except Exception:
                pass
            logger.warning(
                "regime guard persist dropped: pending limit reached (max=%s)",
                getattr(self, "_rg_max_pending", "?"),
            )
            return
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                self._m_inc("tm_regime_guard_persist_submit_failed_total", 1, tags)
            except Exception:
                pass
            logger.warning("Failed to submit regime guard persist task: %s", e)
            return

        try:
            self._m_inc("tm_regime_guard_persist_submitted_total", 1, tags)
        except Exception:
            pass
        ...
        def _cb(f):
            try:
                ...
                if exc:
                    try:
                        self._m_inc("tm_regime_guard_persist_failed_total", 1, tags)
                    except Exception:
                        pass
                    logger.error("Async regime guard persist failed: %s", exc)
            except Exception:
                pass
            finally:
                try:
        ...
        try:
            ...
            # If callback cannot be attached, release immediately to avoid deadlock.
            try:
                self._rg_pending_add(-1)
                sem.release()
            except Exception:
    ...
    def _update_stats_from_dicts(self, pos_dict: Dict[str, Any], closed_dict: Dict[str, Any]) -> None:
        ...
        if self.regime_guard:
            try:
                ...
                pnl = float(closed_dict.get("pnl_net") or closed_dict.get("pnl") or 0.0)
                risk = float(pos_dict.get("risk_amount") or closed_dict.get("risk") or 0.0)
                r_value = pnl / risk if risk > 0 else 0.0

                closed_at = closed_dict.get("closed_at") or closed_dict.get("exit_ts") or None
                if closed_at is None:
                    from datetime import datetime, timezone
                    closed_at = datetime.now(timezone.utc)
                elif isinstance(closed_at, (int, float)):
                    # Convert timestamp (ms or seconds) to datetime
                    from datetime import datetime, timezone
                    ts_sec = float(closed_at)
                    # If timestamp is in milliseconds (> year 2000 in seconds)
                    if ts_sec > 946684800000:
                        ts_sec = ts_sec / 1000.0
                    closed_at = datetime.fromtimestamp(ts_sec, tz=timezone.utc)
                elif not hasattr(closed_at, 'tzinfo'):
                    # If it's some other type, use current time
                    from datetime import datetime, timezone
                    closed_at = datetime.now(timezone.utc)
                ...
                if callable(persist_task):
                    self._submit_regime_guard_persist_task(
                        persist_task,
                        tags={"family": family, "venue": venue, "timeframe": timeframe},
                    )
            except Exception as e:
                logger.warning("regime guard update failed: %s", e)
    ...
    def _resolve_trailing_tp1_offset_atr(self, pos: PositionState, spec: SymbolSpec) -> float:
        ...
        # 1) spec override (конфиг по символу)
        try:
    ...
    def on_signal(self, raw_signal: Dict[str, Any]) -> Optional[str]:
        ...
        if not is_policy_entry and sig_conf < (self.shadow_conf_threshold / 100.0):
            # Ignore raw signals below threshold
            return None
        ...
        with self._lock:
            ...
            if sig.sid and sig.sid in self.pos_by_sid:
                ...
                pos = self.open_positions.get(pos_id)
                if pos and pos.is_virtual and is_policy_entry:
                    # Upgrade virtual to real if policy entry arrived
                    logger.info("⚡ Upgrading virtual position to REAL (sid=%s)", sig.sid)
                    pos.is_virtual = False
                    pos.v_gate_status = "passed"
                    # We continue to let it save etc.
                else:
            ...
            # Mark as virtual if not a direct policy entry
            if not is_policy_entry:
                pos.is_virtual = True
                pos.v_gate_status = "na"
            else:
                pos.is_virtual = False
                pos.v_gate_status = "passed"
    ...
    def _normalize_signal(self, raw: Dict[str, Any]) -> Optional[SignalNorm]:
        try:
            ...
            is_crypto = symbol_up.endswith(('USDT', 'USDC', 'USD', 'BUSD')) and symbol_up in ('BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'SOLUSDT', 'ADAUSDT')
            is_margin_fx = symbol_up in ("XAUUSD", "XAGUSD")
            ...
            max_margin_percent = float(os.getenv("MAX_MARGIN_PERCENT", os.getenv("RISK_PERCENT", "5.0")))
            ...
            if is_margin_based:
                ...
                if position_size_usd <= 0 and signal_lot > 0 and entry_price > 0:
                    # Fallback: если передан только lot, считаем его номиналом и нормализуем до маржи
                    position_size_usd = signal_lot
                # Кэп маржи от депозита
                position_size_usd = min(position_size_usd, margin_cap)
                # Маржинальный расчет: notional = margin * leverage; lot = notional / entry_price
                ...
                logger.info(
                    f"✅ Margin-based sizing: {symbol} margin=${position_size_usd:.2f}, lev={leverage_env:.0f}x, "
                    f"notional=${notional_usd:.2f}, entry_price=${entry_price:.2f}, lot={lot:.6f}"
                )
            else:
    ...
    def _finalize_orphan_closures(self, closures: List[Tuple[PositionState, float, int, str]]) -> None:
        ...
        for pos, exit_price, exit_ts_ms, close_reason_raw in closures:
            try:
                ...
                report_triggers.append((pos.source, pos.symbol, pos.id))
            except Exception as e:
                logger.warning("⚠️ Orphan forced-close failed: %s", e)

        # триггер отчётов вне lock
        for src, sym, oid in report_triggers:
            try:
                from services.periodic_reporter import check_and_trigger_report
        ...
        with self._lock:
            ...
            try:
                for pos_id, pos in list(self.open_positions.items()):
                    ...
                    if not sym:
                        continue
                    by_sym.setdefault(sym, []).append(str(pos_id))
            except Exception:
                return
            ...
            with ctx:
                ...
                with self._lock:
                    ...
                    for pos_id in by_sym.get(sym, []):
                        ...
                        local_triggers.append((str(getattr(pos, "source", "")), str(getattr(pos, "symbol", "")), str(pos.id)))
    ...
    def on_tick(self, raw_tick: Dict[str, Any]) -> None:
        ...
        with sym_ctx:
            ...
            # 2) snapshot pos_ids under _lock (maps/indexes are shared)
            with self._lock:
                pos_ids = list(self.open_by_symbol.get(symbol, set()))

            # 3) Collect IO steps OUTSIDE _lock to reduce contention.
            ...
            for pos_id in pos_ids:
                # fetch pos pointer under _lock; do not hold _lock for CPU work / IO
                with self._lock:
                    pos = self.open_positions.get(pos_id)
                ...
                for ev in (events or []):
                    ...
                    if ev.event_type == "TP_HIT":
                        ...
                        if int(p.get("tp_level", 0)) == 1 and getattr(pos, "sid", ""):
                            ...
                            try:
                                if self._is_trailing_after_tp1_enabled(pos, spec):
    ...
    def apply_external_sl_hit(
        ...
        with self._symbol_lock_ctx(sym):
            ...
            report_trigger = (pos.source, pos.symbol, pos.id)
    ...
    def _log_ab_closed_event(self, pos: PositionState, closed: TradeClosed, close_reason: str) -> None:
        ...
        try:
            ...
            meta_enforce_applied = int(getattr(pos, "meta_enforce_applied", -1))
            ...
            try:
                ...
                if not meta_enforce_cov_bucket and isinstance(sp, dict):
                    meta_enforce_cov_bucket = str(sp.get("meta_enforce_cov_bucket", "") or "")
                if meta_enforce_applied == -1 and isinstance(sp, dict):
                    meta_enforce_applied = int(sp.get("meta_enforce_applied", -1))
                if not meta_enforce_cov_bucket and isinstance(ind, dict):
                    meta_enforce_cov_bucket = str(ind.get("meta_enforce_cov_bucket", "") or "")
                if meta_enforce_applied == -1 and isinstance(ind, dict):
                    meta_enforce_applied = int(ind.get("meta_enforce_applied", -1))
            ...
            self.events_logger.log_position_closed(
                ...
                payload={
                    ...
                    "meta_enforce_cov_bucket": str(meta_enforce_cov_bucket),
                    "meta_enforce_applied": int(meta_enforce_applied),
                },
                extra_payload=extra,
            )
        except Exception:
