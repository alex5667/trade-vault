...
def patch_file(path):
    ...
    def insert_trade_kpi_liqmap_v1(self, rows) -> int:
        ...
        return len(rows)
"""
    if "ensure_trade_kpi_liqmap_v1_table" not in content:
...
"""
    if "POST_SL_LIQMAP_KPI_ARCHIVE_ENABLED" not in content:
...
"""
    if "def post_sl_liqmap_kpi_row" not in content:
...
"""
    if "pg.ensure_trade_kpi_liqmap_v1_table" not in content:
        content = content.replace("        tasks = []", run_lines + "\n        tasks = []\n" + tasks_append)

    with open(path, "w") as f:
        f.write(content)

patch_file("reference/services/archivers/stream_archiver.py")
patch_file("python-worker/services/archivers/stream_archiver.py")
