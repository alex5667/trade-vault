...
from domain.calculators import pnl_money
...
class TestTradeClosureIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.spec_mock.pnl_money = Mock(side_effect=lambda e, p, l, d, s=None: (p - e) * l if d == "LONG" else (e - p) * l)
    ...
    def test_long_position_tp1_hit(self):
        ...
        signal = SignalNorm(
            ...
            tp_levels=[50500.0, 51000.0, 51500.0],  # TP выше entry
            payload={}
        )
        ...
        self.assertEqual(len(events), 2, "Должно быть 2 события: TP_HIT и CLOSE")
        self.assertEqual(events[0].event_type, "TP_HIT", "Первое событие должно быть TP_HIT")
        self.assertEqual(events[1].event_type, "CLOSE", "Второе событие должно быть CLOSE")
        ...
        self.assertEqual(closed.close_reason, "TP1", "Причина закрытия должна быть TP1")
    ...
    def test_short_position_tp1_hit(self):
        ...
        # Проверяем закрытую сделку
        self.assertEqual(closed.close_reason, "TP1")
    ...
    def test_partial_tp_closures(self):
        ...
        self.assertIsNotNone(closed_1, "Должно быть частичное закрытие по TP1")
        ...
        # Проверяем второе частичное закрытие
        self.assertIsNotNone(closed_2, "Должно быть второе частичное закрытие по TP2")
        self.assertEqual(pos.tp_hits, 2)
        self.assertEqual(pos.tp2_hit, True)
        self.assertEqual(pos.remaining_qty, 0.2, "Должно остаться 20% от исходного объема")
