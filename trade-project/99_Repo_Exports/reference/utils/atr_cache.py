...
class ATRCache:
    ...
    def get_with_meta(
        ...
        Args:
            ...
            now_ms:     Current epoch-ms. Defaults to ``time.time() * 1000``.
            prefer_src: Force-select a specific candidate source name.
        """
        sym = str(symbol)
        nm = int(now_ms) if now_ms is not None else int(time.time() * 1000)
    ...
    def clear_all(self) -> int:
        ...
        try:
            keys = list(self.redis_client.scan_iter(match="atr:*", count=100))
