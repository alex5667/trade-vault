# -*- coding: utf-8 -*-
"""
Tests for enforce_bucket_state_exporter_v1.py (P77/P89/P90)
...
def test_exporter_self_health_gauges_exported():
    """P90: of_enforce_state_exporter_poll_ts_ms and of_enforce_state_exporter_errors_total
    must be importable module-level gauge objects (not None).
    This verifies the P90 gauge declarations are present in the module.
    """
    import orderflow_services.enforce_bucket_state_exporter_v1 as mod

    assert hasattr(mod, "of_enforce_state_exporter_poll_ts_ms"), \
        "P90 gauge of_enforce_state_exporter_poll_ts_ms not found in module"
    assert hasattr(mod, "of_enforce_state_exporter_errors_total"), \
        "P90 gauge of_enforce_state_exporter_errors_total not found in module"
    # Verify they are real Prometheus Gauge objects (have .set method)
    assert callable(getattr(mod.of_enforce_state_exporter_poll_ts_ms, "set", None)), \
        "of_enforce_state_exporter_poll_ts_ms does not have a .set() method"
    assert callable(getattr(mod.of_enforce_state_exporter_errors_total, "set", None)), \
        "of_enforce_state_exporter_errors_total does not have a .set() method"
