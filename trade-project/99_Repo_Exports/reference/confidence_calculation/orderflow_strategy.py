...
class OrderFlowStrategy:
    ...
    async def process_tick(self, runtime: SymbolRuntime, tick: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            now_ms = int(tick_ts if tick_ts > 0 else time.time() * 1000)
            ...
            res = self._atr_sanity.update(
                ...
                now_ms=now_ms,
                tf=str((atr_meta or {}).get("picked_tf") or "1m"),
            )
        ...
        try:
            if int(indicators.get("cvd_quarantine_active", 0) or 0) == 1:
                now_ms = int(indicators.get("now_ts_ms", 0) or tick_ts or int(time.time() * 1000))
        ...
        try:
            ...
            cfg2["of_score_min"] = float(cfg2.get("of_score_min", os.getenv("OF_SCORE_MIN", "0.40")))
    ...
    def _apply_confidence_calibration(self, runtime, indicators: Dict[str, Any], conf_raw: float, ctx: Dict[str, Any]) -> float:
        ...
        if champ_rt:
            champ_rt.maybe_reload(int(time.time() * 1000)) 
        ...
        if chall_rt:
             ...
             if need_challenger:
                 chall_rt.maybe_reload(int(time.time() * 1000))
