...
try:
    ...
    Counter = Gauge = None
    edge_time_skew_ms = edge_bad_time_total = None
...
try:
    ...
    decide_inline_exec_health = None  # type: ignore
    read_inline_exec_rollup_sync = None  # type: ignore
    read_perm_impact_rollup_sync = None  # type: ignore
    resolve_inline_exec_mode = None  # type: ignore
    inline_exec_edge_last_p95_bps = None  # type: ignore
    inline_exec_edge_tighten_total = None  # type: ignore
    inline_exec_edge_veto_total = None  # type: ignore
...
def _env_bool(name: str, default: bool = False) -> bool:
    """Безопасное извлечение bool из ENV."""
...
def estimate_slippage_bps(
    ...
    *,
    ...
    # ---------------------------------------------------------------------
    policy = (os.getenv("EDGE_TS_BAD_POLICY", "correct_skip_ema") or "").strip().lower()

    try:
        max_skew_ms = int(float(os.getenv("EDGE_TS_MAX_SKEW_MS", "21600000")))  # 6h
    except Exception:
        max_skew_ms = 21600000
    try:
        veto_bps = float(os.getenv("EDGE_TS_BAD_VETO_BPS", "1000000"))
    except Exception:
...
def _load_slippage_ema_bps(
    ...
    try:
        ...
        min_n = int(float(os.getenv("EDGE_SLIP_EMA_MIN_SAMPLES", "20")))
...
def _load_drift_active(
    ...
    *,
    ...
    include_kind = (os.getenv("FEATURE_DRIFT_INCLUDE_KIND", "0") or "").strip().lower() in {"1", "true", "yes", "on"}
...
class EdgeCostGate:
    ...
    def from_env(cls) -> "EdgeCostGate":
        ...
        apply_kinds = _parse_csv_set(os.getenv("EDGE_COST_APPLY_KINDS", "") or "")
        ...
        if os.getenv("EDGE_FEES_BPS_DEFAULT") is None and os.getenv("CRYPTO_COMMISSION_RATE") is not None:
            try:
                one_way_bps = float(os.getenv("CRYPTO_COMMISSION_RATE", "0")) * 10_000.0
        ...
        return cls(
            ...
            #   hard         -> veto
            # ------------------------------------------------------------------
            inline_exec_enabled=bool(_env_bool("INLINE_EXEC_HEALTH_ENABLE", False)),
            inline_exec_mode=str(os.getenv("INLINE_EXEC_HEALTH_MODE", "auto") or "auto").strip().lower(),
            inline_exec_warn_p95_bps=float(_env_float("INLINE_EXEC_HEALTH_WARN_P95_BPS", 0.0)),
            inline_exec_crit_p95_bps=float(_env_float("INLINE_EXEC_HEALTH_CRIT_P95_BPS", 0.0)),
            inline_exec_min_count=int(_env_int("INLINE_EXEC_HEALTH_MIN_COUNT", 3)),
            inline_exec_tighten_add_mult=float(_env_float("INLINE_EXEC_HEALTH_TIGHTEN_ADD_MULT", 1.0)),
            inline_exec_tighten_add_cap_bps=float(_env_float("INLINE_EXEC_HEALTH_TIGHTEN_ADD_CAP_BPS", 8.0)),
            inline_exec_max_perm_impact_p95_bps=float(_env_float("INLINE_EXEC_HEALTH_MAX_PERM_IMPACT_P95_BPS", 0.0)),
            inline_exec_tca_delta_sec=int(_env_int("INLINE_EXEC_HEALTH_TCA_DELTA_SEC", 1)),
        )
    ...
    def _inline_exec_feedback(
        ...
        *,
        ...
        tf: str,
        redis_client: Any,
        exec_profile: str = "default",
    ) -> Tuple[float, bool, str]:
        """Read inline IS rollup and decide: tighten_add_bps, veto, reason.

        Returns (tighten_add_bps, veto, reason_code).

        Fall-open: any failure returns (0.0, False, "").
        """
        if not self.inline_exec_enabled:
            return 0.0, False, ""
        if decide_inline_exec_health is None or read_inline_exec_rollup_sync is None:
            return 0.0, False, ""

        try:
            rollup = read_inline_exec_rollup_sync(
                redis_client,
                symbol=str(symbol or "").upper(),
                side=str(side or "").upper(),
                session=str(session or "na").lower(),
                kind=str(kind or "na").lower(),
                tf=str(tf or "na").lower(),
                min_count=int(self.inline_exec_min_count),
            )
            if not rollup:
                return 0.0, False, ""

            p95 = float(rollup.get("p95_bps", float("nan")))
            if not math.isfinite(p95):
                return 0.0, False, ""

            # Report p95 to Prometheus (best-effort)
            try:
                if inline_exec_edge_last_p95_bps is not None:
                    inline_exec_edge_last_p95_bps.labels(symbol=str(symbol).upper(), side=str(side).upper()).set(float(p95))
            except Exception:
                pass

            # Profile-aware mode resolution:
            #   default/soft -> monitor
            #   strict       -> tighten
            #   hard         -> veto
            mode = str(self.inline_exec_mode or "auto").strip().lower()
            if resolve_inline_exec_mode is not None:
                mode = resolve_inline_exec_mode(mode, profile=exec_profile)

            # Optional perm impact for double-veto confirmation
            perm_impact_p95 = float("nan")
            if read_perm_impact_rollup_sync is not None and self.inline_exec_max_perm_impact_p95_bps > 0:
                try:
                    v = read_perm_impact_rollup_sync(
                        redis_client,
                        symbol=str(symbol or "").upper(),
                        side=str(side or "").upper(),
                        session=str(session or "na").lower(),
                        kind=str(kind or "na").lower(),
                        tf=str(tf or "na").lower(),
                        delta_sec=int(self.inline_exec_tca_delta_sec),
                    )
                    if v is not None and math.isfinite(float(v)):
                        perm_impact_p95 = float(v)
                except Exception:
                    pass
            ...
            dec = decide_inline_exec_health(
                ...
                perm_impact_p95_bps=float(perm_impact_p95),
                max_perm_impact_p95_bps=float(self.inline_exec_max_perm_impact_p95_bps),
                mode=mode,
            )

            tighten_add = 0.0
            if dec.tighten_add_bps > 0:
                raw_add = float(dec.tighten_add_bps) * float(self.inline_exec_tighten_add_mult)
                tighten_add = float(min(raw_add, max(0.0, float(self.inline_exec_tighten_add_cap_bps))))

            if dec.veto:
                try:
                    if inline_exec_edge_veto_total is not None:
                        inline_exec_edge_veto_total.labels(symbol=str(symbol).upper(), side=str(side).upper()).inc()
                except Exception:
                    pass
                return float(tighten_add), True, str(dec.reason_code)

            if dec.apply and tighten_add > 0:
                try:
                    if inline_exec_edge_tighten_total is not None:
                        inline_exec_edge_tighten_total.labels(symbol=str(symbol).upper(), side=str(side).upper()).inc()
                except Exception:
                    pass

            return float(tighten_add), False, str(dec.reason_code if dec.apply else "")
    ...
    def _costs_bps(self, ctx: Any, *, kind: str, symbol: str, tf: Optional[str] = None) -> Tuple[float, float]:
        ...
        if tighten:
            ...
            if float(drift_factor) > 1.0:
                try:
                    cap = float(os.getenv("EDGE_DRIFT_SLIPPAGE_CAP_MULT", "3.0"))
                except Exception:
                    cap = 3.0
    ...
    def evaluate(self, *, ctx: Any, kind: str, symbol: str) -> EdgeCostGateDecision:
        ...
        # ------------------------------------------------------------------
        try:
            tighten = bool(getattr(ctx, "_edge_drift_tighten", False))
            if tighten and float(drift_factor) > 1.0:
                k_cap = float(os.getenv("EDGE_DRIFT_K_CAP_MULT", "2.5"))
        ...
        # P6: exec-health TCA rollup overlay (fail-open, single SoT via exec_health_rollups)
        exec_dec = None
        try:
            redis_client = getattr(self, "redis", None) or getattr(ctx, "redis", None)
            exec_rollups: dict = {}
        ...
        # ------------------------------------------------------------------
        # P1 Inline execution-health feedback (warm-path, fail-open).
        # ------------------------------------------------------------------
        # Reads per-symbol IS percentiles from recent fills (updated by tca_worker).
        # In ``tighten`` mode: adds slippage. In ``veto`` mode: returns veto
        # only if IS_p95 > crit AND permanent impact p95 is also bad.
        try:
            inl_side = str(getattr(ctx, "side", None) or getattr(ctx, "direction", None) or "").upper() or "LONG"
            inl_sess = str(getattr(ctx, "session", None) or "na").lower()
            inl_kind = str(kind or getattr(ctx, "kind", None) or "na").lower()
            inl_tf = str(tf or getattr(ctx, "tf", None) or "na").lower()
            inl_add, inl_veto, inl_reason = self._inline_exec_feedback(
                ctx=ctx,
                symbol=str(symbol or "").upper(),
                side=str(inl_side),
                session=str(inl_sess),
                kind=str(inl_kind),
                tf=str(inl_tf),
                redis_client=redis_client,
                exec_profile=str(exec_profile or "default"),
            )

            setattr(ctx, "inline_exec_tighten_add_bps", float(inl_add))
            setattr(ctx, "inline_exec_veto", bool(inl_veto))
            setattr(ctx, "inline_exec_reason", str(inl_reason))

            if inl_veto:
                d = EdgeCostGateDecision(
                    apply=True,
                    veto=True,
                    reason_code=str(inl_reason or self.REASON_INLINE_EXEC_VETO),
                    expected_move_bps=float("nan"),
                    threshold_bps=float(thr),
                    fees_bps=float(fees_bps),
                    slippage_bps=float(slip_bps),
                    k=float(k_eff),
                    mode=self.mode,
                    notes=f"inline_exec_p1 veto | add_bps={inl_add:.2f}",
                    drift_factor=float(drift_factor),
                    drift_score=float(drift_score),
                    drift_feature=str(drift_feat or ""),
                    total_costs_bps=float(fees_bps) + float(slip_bps),
                    buffer_bps=0.0,
                    edge_source="inline_exec_veto",
                )
                trace_gate(ctx, stage="gates", name="edge_cost_gate", passed=False, veto=True, reason_code=str(d.reason_code), metrics={"thr": d.threshold_bps})
                return d

            if inl_add > 0.0:
                slip_bps = float(slip_bps) + float(inl_add)
                # Recompute threshold after slippage adjustment
                thr = float(k_eff) * (float(fees_bps) + float(slip_bps) + float(buffer_bps))
        except Exception:
            pass  # never block hot path on inline exec feedback failure
