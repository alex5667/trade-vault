...
from handlers.base_orderflow_handler import Tick, PublishResult, MarketRegime  # type: ignore
...
def normalize_ts_ms(ts: int) -> int:
    """
    Эвристика нормализации ts в миллисекунды:
      - секунды: ~1e9..1e10 -> *1000
      - мс:      ~1e12..1e13 -> без изменений
      - мкс:     ~1e15..1e16 -> //1000
    """
    if ts <= 0:
        return 0
    # microseconds (Binance aggTrades иногда дают us в сторонних прокси)
    if ts >= 10_000_000_000_000:  # 1e13+
        # 1e15.. -> likely us, 1e13.. -> likely ms; разделим по 1e14
        if ts >= 100_000_000_000_000:  # 1e14
            return int(ts // 1000)
        return int(ts)
    # seconds
    if ts < 10_000_000_000:  # < 1e10
        return int(ts * 1000)
    # fallback
    return int(ts)
...
class MicrostructureEngine:
    def __init__(
        ...
        *,
        ...
        self._mode_counts = {"momentum": 0, "meanrev": 0, "mixed": 0}
