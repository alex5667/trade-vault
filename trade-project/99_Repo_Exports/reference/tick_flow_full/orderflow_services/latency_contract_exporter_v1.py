...
async def _scrape_required_coverage(redis_client: Any, prefix: str, stale_s: int, slo_summary_key: str) -> None:
    """P4.1: read required stage hashes and SLO gate summary, update Prometheus gauges."""
    now = time.time()
...
async def run_exporter_loop(
    ...
    while True:
        try:
            ...
            await _scrape_required_coverage(redis_client, prefix, stale_s, slo_summary_key)
        except Exception as exc:
...
async def main() -> None:
    ...
    try:
        await run_exporter_loop(redis_client, prefix, interval_s, stale_s, budgets, slo_summary_key)
    finally:
